        ...
        public static void CommandCreateSendanBoltHokyouzai(UIDocument uidoc)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    double pieceHaba = ClsRevitUtil.ConvertDoubleGeo2Revit(Master.ClsHiuchiCsv.GetHaraGroundLength(extractedName));
                    ...
                    tmpStPoint = new XYZ(tmpStPoint.X, tmpStPoint.Y, locationPoint.Point.Z);
                    tmpEdPoint = new XYZ(tmpEdPoint.X, tmpEdPoint.Y, locationPoint.Point.Z);
                    XYZ tmpMidPoint = new XYZ((tmpStPoint.X + tmpEdPoint.X) / 2, (tmpStPoint.Y + tmpEdPoint.Y) / 2, (tmpStPoint.Z + tmpEdPoint.Z) / 2);

                    // 腹起ベースと火打受けピースの交点を取得
                    //XYZ ptInsert = ClsSendanBoltHokyouzai.GetIntersectionPointWithLine(doc, tmpStPoint, tmpEdPoint, idPiece);
                    XYZ[] ptInserts = ClsSendanBoltHokyouzai.GetIntersectionPointsWithLine(doc, tmpStPoint, tmpEdPoint, idPiece);
                    ...
                    clsSendanBoltHokyouzai.CreateSendanBoltHokyouzai(doc, ptInsert, instBase.Host.Id, tmpStPoint, tmpEdPoint, offset, idPiece, hokyouzaiHaba);
                }
                catch
                {
                    break;
