        ...
        public static bool CommandChangeKiribariTsunagizaiBase(UIDocument uidoc)
        {
            ...
            for (int i = 0; i < idList.Count(); i++)
            {
                ...
                else
                {
                    ...
                    //元データを取得
                    templateBase.m_SteelType = ClsRevitUtil.CompareValues(ClsRevitUtil.GetParameter(doc, id, "分類"), templateBase.m_SteelType);
                    templateBase.m_SteelSize = ClsRevitUtil.CompareValues(ClsRevitUtil.GetParameter(doc, id, "サイズ"), templateBase.m_SteelSize);
                    templateBase.m_ToritsukeHoho = ClsRevitUtil.CompareValues(templateBase.GetToritsukeHoho(ClsRevitUtil.GetParameter(doc, id, "取付方法")), templateBase.m_ToritsukeHoho);
                    templateBase.m_BoltType1 = ClsRevitUtil.CompareValues(ClsRevitUtil.GetParameter(doc, id, "ボルトタイプ1"), templateBase.m_BoltType1);
                    templateBase.m_BoltType2 = ClsRevitUtil.CompareValues(ClsRevitUtil.GetParameter(doc, id, "ボルトタイプ2"), templateBase.m_BoltType2);
                    templateBase.m_BoltNum = ClsRevitUtil.CompareValues(ClsRevitUtil.GetParameterInteger(doc, id, "ボルト本数"), templateBase.m_BoltNum);
                }
            }

            //ダイアログを表示
            DLG.DlgCreateKiribariTsunagizaiBase kiribariTsunagizaiBase = new DLG.DlgCreateKiribariTsunagizaiBase(templateBase);
            ...
            foreach (var id in idList)
            {
                ...
                //分割がある場合の処理
                (List<XYZ> stList, List<XYZ> edList) = clsKiribariTsunagizaiBase.ChangeSplitKiribariTsunagizaiBase(doc, id);
