...
namespace YMS.Command
{
    class ClsCommandYokoyaita
    {
        public static void CommandYokoyaita(UIDocument uidoc)
        {
            ...
            List<List<ElementId>> handGroupList = ClsYokoyaita.GroupingOyakui(doc, idList, out List<ElementId> cornerGroupOriginalList);
            foreach(var handGroup in handGroupList)
            {
                ...
                for (int i = 0; i < handGroup.Count - 1; i++)
                {
                    ...
                    double dVoid = ClsVoid.GetVoidDep(doc, id1);
                    if (clsYokoyaita.m_type == "木矢板")
                        clsYokoyaita.CreateMokuyaita(doc, id1, id2, ClsRevitUtil.CovertFromAPI(lenght), ClsRevitUtil.CovertToAPI(dVoid), levelId, dHTop);
                    else
                        clsYokoyaita.CreateYokoyaita(doc, id1, id2, ClsRevitUtil.CovertFromAPI(lenght), ClsRevitUtil.CovertToAPI(dVoid), levelId, dHTop);
                }
        ...
        public static void CommandChangeYokoyaita(UIDocument uidoc)
        {
            ...
            for (int i = 0; i < yokoyaList.Count; i++)
            {
                if (!yokoyaList[i].Create)
                    continue;
                if (clsYokoyaita.m_type == "木矢板")
                    clsYokoyaita.CreateMokuyaita(doc, yokoyaList[i].Point, yokoyaList[i].Angle, 
                        yokoyaList[i].Length, yokoyaList[i].Kussaku, levelId, yokoyaList[i].CASE, yokoyaList[i].HTop);
                else
                    clsYokoyaita.CreateYokoyaita(doc, yokoyaList[i].Point, yokoyaList[i].Angle,
                        yokoyaList[i].Length, yokoyaList[i].Kussaku, levelId, yokoyaList[i].CASE, yokoyaList[i].HTop);
            }
        }
    }
