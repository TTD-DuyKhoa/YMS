    ...
    class ClsCommandSumibuPiece
    {
        public static void CommandSumibuPiece(UIDocument uidoc)
        {
            ...
            if (clsSP.m_CreateType == ClsSumibuPiece.CreateType.Manual)
            {
                ...
                {
                    if ((ClsRevitUtil.GetParameter(doc, id1, "縦本数") != "シングル") || (ClsRevitUtil.GetParameter(doc, id2, "縦本数") != "シングル"))
                    {
                    ...
                    XYZ moveVectorA = vecA * ClsRevitUtil.ConvertDoubleGeo2Revit(dOffset2);// ClsRevitUtil.ConvertDoubleGeo2Revit(dOffset1);
                    XYZ moveVectorB = vecB * ClsRevitUtil.ConvertDoubleGeo2Revit(dOffset1);// ClsRevitUtil.ConvertDoubleGeo2Revit(dOffset2);
                    ...
                    if (haraDan1 == "上段" && haraDan2 == "下段")
                    {
                        ...
                        if (yoko1 == "ダブル")
                        {
                            ...
                            if (!clsSP.CreateSumibuPiece(doc, id1, crossPoint + moveVectorB * 3, haraSize1, radians, false, true))
                            {
                                MessageBox.Show("隅部ピースの配置に失敗しました");
                            }
                        }
                        ...
                        {
                            clsSP.m_BasePoint = crossPoint + moveVectorA * 3;
                            if (!clsSP.CreateSumibuPiece(doc, id2, crossPoint + moveVectorA * 3, haraSize2, radians, false, false))
                            {
                                MessageBox.Show("隅部ピースの配置に失敗しました");
                            }
                        }
                    ...
                    else if (haraDan1 == "下段" && haraDan2 == "上段")
                    {
                        ...
                        {
                            clsSP.m_BasePoint = crossPoint + moveVectorB * 3;
                            if (!clsSP.CreateSumibuPiece(doc, id2, crossPoint + moveVectorB * 3, haraSize1, radians, false, false))
                            {
                                MessageBox.Show("隅部ピースの配置に失敗しました");
                            }
                        }
                        ...
                        {
                            clsSP.m_BasePoint = crossPoint + moveVectorA * 3;
                            if (!clsSP.CreateSumibuPiece(doc, id1, crossPoint + moveVectorA * 3, haraSize2, radians, false, true))
                            {
                                MessageBox.Show("隅部ピースの配置に失敗しました");
                            }
                        }
            ...
            else if (clsSP.m_CreateType == ClsSumibuPiece.CreateType.Auto)
            {
                ...
                for (int i = 0; i < lstHaraBase.Count; i++)
                {
                    ...
                    for (int k = 0; k < lstHaraBase.Count(); k++)
                    {
                        ...
                        // 既に処理済みか確認
                        bool exists = myList.Any(item => item.Item1 == idHaraokoshiBase1 && item.Item2 == idHaraokoshiBase2) || myList.Any(item => item.Item1 == idHaraokoshiBase2 && item.Item2 == idHaraokoshiBase1);
                        if (exists) { continue; }
