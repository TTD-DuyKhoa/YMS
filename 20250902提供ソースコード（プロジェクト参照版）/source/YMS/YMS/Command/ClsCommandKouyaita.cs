        ...
        public static void CommandKouyaita(UIApplication uiapp)
        {
            ...
            for (; ; )
            {
                try
                {
                    if (clsKouyaita.m_way == 0)
                    {
                        ...
                        while (true)
                        {
                            for (int i = 0; i < resList.Count; i++)
                            {
                                ...
                                createKouyaitaList.AddRange(clsKouyaita.CreateKouyaita1(doc, picPointS, picPointE, levelId, voidVec, ref lastVoidVec, true, bIrizumi, selShinId).ToList());
                            ...
                            if (0 < targetDesumiList.Count)
                            {
                                for (int i = 0; i < targetDesumiList.Count; i++)
                                {
                                    ...
                                    //出隅部専用のコーナー矢板を作成する
                                    createKouyaitaList.Add(clsKouyaita.CreateSingleKouyaita(doc, insertPoint, dir, levelId, targetDesumilastVoidVecList[i], selShinId));
                    ...
                    //    //作図に失敗した時のコメント
                    //}
                    createKouyaitaList.AddRange(clsKouyaita.CreateKouyaita1(doc, picPointS, picPointE, levelId, voidVec, ref lastVoidVec, true, true, selShinId).ToArray());
                    //反転否か
                    //YMS.DLG.DlgOppKouyaita oppKouyaita = new DLG.DlgOppKouyaita();
                    oppKouyaita.ShowDialog();
                    dResult = oppKouyaita.dialogResult;
