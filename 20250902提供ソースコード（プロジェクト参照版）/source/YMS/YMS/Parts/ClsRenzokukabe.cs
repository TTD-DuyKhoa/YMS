...
namespace YMS.Parts
{
    public class ClsRenzokukabe
    {
        ...
        /// <returns></returns>
        public bool CreateRenzokukabe(Document doc, XYZ picPointS, XYZ picPointE, ElementId levelId, bool bIrizumi = true, ElementId kabeShinId = null, bool defVoid = true)
        {
            try
            {
                ...
                string familyPathKabe = System.IO.Path.Combine(symbolFolpath, "山留壁関係\\08_" + RENZOKUKABE + "\\" + RENZOKUKABE + ".rfa");
        ...
        /// <param name="length"></param>
        /// <returns></returns>
        public bool CreateSingleRenzokukabe(Document doc, XYZ picPointS, XYZ soilDirection, ElementId levelId, bool bIrizumi = true, bool kabeCreate = true, ElementId kabeShinId = null)
        {
            try
            {
                string symbolFolpath = ClsZumenInfo.GetYMSFolder();
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    ////補助線の作成
                    double dH = ClsRevitUtil.CovertToAPI(dEx);
                    outLine = CreatePointHojyoLine(doc, picPointS, -inst.FacingOrientation, soilDirection, dH, ClsRevitUtil.CovertToAPI(dEx / 2));//外側
                    inLine = CreatePointHojyoLine(doc, picPointS, -inst.FacingOrientation, soilDirection, -dH, ClsRevitUtil.CovertToAPI(dEx / 2));//内側
