...
namespace YMS.Parts
{
    public class ClsKiribariTsunagizaiBase
    {
        #region 定数
        public const string baseName = "切梁繋ぎ材ベース";
        #endregion
        ...
        #region コンストラクタ
        public ClsKiribariTsunagizaiBase()
        {
            Init();
        }
        #endregion
        ...
        public bool CreateKiribariTsunagizaiBase(Document doc, List<ElementId> modelLineIdList, ElementId levelID)
        {
        ...
        public bool ChangeKiribariTsunagizaiBase(Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, string dan, ElementId levelID)
        {
            string symbolFolpath = ClsZumenInfo.GetYMSFolder();
            string shinfamily = System.IO.Path.Combine(symbolFolpath, "ベース関係\\" + baseName + ".rfa");
            //シンボル読込
        ...
        public bool ChangeKiribariTsunagizaiBase(Document doc, XYZ tmpStPoint, XYZ tmpEdPoint)
        {
        ...
        public (List<XYZ>, List<XYZ>) ChangeSplitKiribariTsunagizaiBase(Document doc, ElementId id)
        {
        ...
        /// <param name="id">選択した 切梁ツナギ材ベース のID</param>
        ...
        /// <summary>
        /// 図面上の切梁ツナギ材ベースを全て取得する
        /// </summary>
        /// <param name="doc"></param>
        /// <returns></returns>
        public static List<ElementId> GetAllKiribariTsunagizaiBaseList(Document doc)
        {
            //図面上の切梁ツナギ材ベースを全て取得
        ...
        /// <summary>
        ///  図面上の切梁ツナギ材ベースを全てクラスで取得する
        /// </summary>
        /// <param name="doc"></param>
        /// <returns></returns>
        public static List<ClsKiribariTsunagizaiBase> GetAllClsKiribariTsunagizaiBaseList(Document doc)
        {
            List<ClsKiribariTsunagizaiBase> lstBase = new List<ClsKiribariTsunagizaiBase>();

            List<ElementId> lstId = GetAllKiribariTsunagizaiBaseList(doc);
            foreach (ElementId id in lstId)
            {
                ClsKiribariTsunagizaiBase clsKT = new ClsKiribariTsunagizaiBase();
        ...
        public void SetParameter(Document doc, ElementId id)
        {
            ...
            if (m_ToritsukeHoho == ClsKiribariTsunagizaiBase.ToritsukeHoho.Bolt)
            {
        ...
        public bool CreateTanbuParts(Document doc)
        {
            ...
            if (m_ToritsukeHoho == ToritsukeHoho.Buruman)
            {
                if (m_SteelType != "チャンネル")
                {
                    ...
                    sym = (ClsRevitUtil.GetFamilySymbol(doc, buruFamName, "切梁繋ぎ"));
                }
                else
                {
            ...
            switch (m_SteelType)
            {
                ...
                case "H形鋼 細幅":
                    {
                        double sunpou2 = ClsCommonUtils.ChangeStrToDbl(Master.ClsYamadomeCsv.GetKouzaiSizeSunpou(m_SteelSize, 1));
            ...
            foreach (ElementId id in lstHari)
            {
                ...
                if (insec != null)
                {
                    string danA = ClsRevitUtil.GetParameter(doc, id, "段");
                    ...
                    using (Transaction tx = new Transaction(doc, "Load Family"))
                    {
                        ...
                        if (sym != null)
                        {
                            double dVectAngle = -(dir.AngleOnPlaneTo(XYZ.BasisX, XYZ.BasisZ) + ClsGeo.Deg2Rad(150));// * dir.X + 30 * dir.Y));// dir.AngleOnPlaneTo(direction2, XYZ.BasisZ) + 
