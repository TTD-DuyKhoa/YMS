...
namespace YMS.Parts
{
    public class ClsCoverPlate
    {
        ...
        public bool CreateCoverPlate(Document doc, XYZ ptSt, XYZ ptEd, FamilyInstance instBase, string size, string baseName, bool isKussaku, XYZ vecOffset = null, double baseoffset = 0.0)
        {
            ...
            using (Transaction t = new Transaction(doc, "Create First Cover Plate"))
            {
                ...
                else
                {
                    ...
                    // シンボルを回転
                    double radians = XYZ.BasisX.AngleOnPlaneTo(vec, normal); //inst.HandOrientation.AngleOnPlaneTo(vec, normal);
                ...
                if (isKussaku)
                {
                    if(rPlane)
                    {
                        ...
                        ClsRevitUtil.SetParameter(doc, createdId, "ホストからのオフセット", baseoffset - kouzaiSizeHalf - ClsRevitUtil.CovertToAPI(coverPlateThickness));
                    }
                    else if (baseName.Contains("切梁")|| baseName.Contains("隅火打"))
                    {
                        if (!ClsRevitUtil.SetParameter(doc, createdId, "基準レベルからの高さ", baseoffset - kouzaiSizeHalf - ClsRevitUtil.CovertToAPI((coverPlateThickness))))
                            ClsRevitUtil.SetParameter(doc, createdId, "ホストからのオフセット", baseoffset - kouzaiSizeHalf - ClsRevitUtil.CovertToAPI(coverPlateThickness));// / 2)));
                    }
                    else if (baseName.Contains("腹起"))
                    {
                        ...
                        if (isLeft)
                        {
                            ElementTransformUtils.MoveElement(doc, createdId, (kouzaiSizeHalf + (ClsRevitUtil.CovertToAPI(coverPlateThickness))) * -vecOffset);
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    {
                        if (!ClsRevitUtil.SetParameter(doc, createdId, "基準レベルからの高さ", baseoffset + kouzaiSizeHalf))// + ClsRevitUtil.CovertToAPI((coverPlateThickness / 2)));
                            ClsRevitUtil.SetParameter(doc, createdId, "ホストからのオフセット", baseoffset + kouzaiSizeHalf);
                    }
                    else if (baseName.Contains("腹起"))
                    {
