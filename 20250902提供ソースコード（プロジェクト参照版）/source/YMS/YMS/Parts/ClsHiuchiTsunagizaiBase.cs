...
namespace YMS.Parts
{
    public class ClsHiuchiTsunagizaiBase
    {
        ...
        public bool ChangeHiuchiTsunagizaiBase(Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, string dan, ElementId levelID)
        {
            string symbolFolpath = ClsZumenInfo.GetYMSFolder();
            string shinfamily = System.IO.Path.Combine(symbolFolpath, "ベース関係\\" + baseName + ".rfa");
            //シンボル読込
        ...
        public bool ChangeHiuchiTsunagizaiBase(Document doc, XYZ tmpStPoint, XYZ tmpEdPoint)
        {
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                t.Start(); FailureHandlingOptions failOpt = t.GetFailureHandlingOptions();
            ...
            foreach (ElementId id in lstHari)
            {
                ...
                if (insec != null)
                {
                    ...
                    using (Transaction tx = new Transaction(doc, "Load Family"))
                    {
                        ...
                        if (sym != null)
                        {
                            double angle = dVectAngle - (dir.AngleOnPlaneTo(XYZ.BasisX, XYZ.BasisZ) + ClsGeo.Deg2Rad(150));// * dir.X + 30 * dir.Y));// dir.AngleOnPlaneTo(direction2, XYZ.BasisZ) + 
