...
namespace YMS.DLG
{
    public partial class DlgCASE : System.Windows.Forms.Form
    {
        ...
        private void SetColorAndReadonyRow(DataGridView dgw, DataGridViewRow targetRow)
        {
            ...
            {
                if(targetRow.Index == row.Index) { continue; }
                SetColorAndReadonyRow(row);
            }
        }
        ...
        private bool CheackNumericValueKoyaita()
        {
            foreach (DataGridViewRow row in dgvKoyaita.Rows)
            {
                foreach (DataGridViewCell cell in row.Cells)
                {
                    ...
                    if (cellIndex == KouyaitaColumnLength.Index ||
                        cellIndex == KouyaitaColumnRemainLen.Index ||
                        cellIndex == KouyaitaColumnJointNum.Index ||
                        cellIndex == KouyaitaColumnStake1.Index ||
                        cellIndex == KouyaitaColumnStake2.Index ||
                        cellIndex == KouyaitaColumnStake3.Index ||
                        cellIndex == KouyaitaColumnStake4.Index ||
                        cellIndex == KouyaitaColumnStake5.Index ||
                        cellIndex == KouyaitaColumnStake6.Index ||
                        cellIndex == KouyaitaColumnStake7.Index ||
                        cellIndex == KouyaitaColumnStake8.Index ||
                        cellIndex == KouyaitaColumnStake9.Index ||
                        cellIndex == KouyaitaColumnStake10.Index)
                    {
                        int intNum = 0; double doubleNum = 0;
                        if (!int.TryParse(GetCellVal(cell), out intNum) && !double.TryParse(GetCellVal(cell), out doubleNum))
                        {
                            return false;
                        }
                    }
        ...
        private bool CheackNumericValueOyakui()
        {
            foreach (DataGridViewRow row in dgvOyakui.Rows)
            {
                ...
                {
                    int cellIndex = cell.ColumnIndex;
                    if (cellIndex == OyakuiColumnLength.Index ||
                        cellIndex == OyakuiColumnRemainLen.Index ||
                        cellIndex == OyakuiColumnJointNum.Index ||
                        cellIndex == OyakuiColumnStake1.Index ||
                        cellIndex == OyakuiColumnStake2.Index ||
                        cellIndex == OyakuiColumnStake3.Index ||
                        cellIndex == OyakuiColumnStake4.Index ||
                        cellIndex == OyakuiColumnStake5.Index ||
                        cellIndex == OyakuiColumnStake6.Index ||
                        cellIndex == OyakuiColumnStake7.Index ||
                        cellIndex == OyakuiColumnStake8.Index ||
                        cellIndex == OyakuiColumnStake9.Index ||
        ...
        private bool CheackNumericValueSMW()
        {
            foreach (DataGridViewRow row in dgvSMW.Rows)
            {
                ...
                {
                    int cellIndex = cell.ColumnIndex;
                    if (cellIndex == SMWColumnLength.Index ||
                        cellIndex == SMWColumnSoilDia.Index ||
                        cellIndex == SMWColumnSoilLen.Index ||
                        cellIndex == SMWColumnRemainLen.Index ||
                        cellIndex == SMWColumnJointNum.Index ||
                        cellIndex == SMWColumnStake1.Index ||
                        cellIndex == SMWColumnStake2.Index ||
                        cellIndex == SMWColumnStake3.Index ||
                        cellIndex == SMWColumnStake4.Index ||
                        cellIndex == SMWColumnStake5.Index ||
                        cellIndex == SMWColumnStake6.Index ||
                        cellIndex == SMWColumnStake7.Index ||
                        cellIndex == SMWColumnStake8.Index ||
                        cellIndex == SMWColumnStake9.Index ||
        ...
        private bool CheackNumericValueConWall()
        {
            foreach (DataGridViewRow row in dgvConWall.Rows)
            {
                ...
                {
                    int cellIndex = cell.ColumnIndex;
                    if (cellIndex == ConWallColumnLength.Index ||
                        cellIndex == ConWallColumnWallLen.Index ||
                        cellIndex == ConWallColumnWallThick.Index ||
                        cellIndex == ConWallColumnRemainLen.Index ||
                        cellIndex == ConWallColumnJointNum.Index ||
                        cellIndex == ConWallColumnStake1.Index ||
                        cellIndex == ConWallColumnStake2.Index ||
                        cellIndex == ConWallColumnStake3.Index ||
                        cellIndex == ConWallColumnStake4.Index ||
                        cellIndex == ConWallColumnStake5.Index ||
                        cellIndex == ConWallColumnStake6.Index ||
                        cellIndex == ConWallColumnStake7.Index ||
                        cellIndex == ConWallColumnStake8.Index ||
                        cellIndex == ConWallColumnStake9.Index ||
        ...
        private bool ChangeCASE(DataGridView dgv, DataGridViewRow row)
        {
            ...
            {
                DialogResult dr = MessageBox.Show("CASE" + newCASE + "は既に存在しています。CASE" + newCASE + "の情報を適用しますか？", "CASE変更", MessageBoxButtons.YesNo);
                if (dr == DialogResult.Yes)
                {
                    ...
                    if (CheackExistCASEAndEdaban(dgv, newCASE, edaban, row.Index, out existRowIndex))
                    {
                        ...
                        int jointNumIndex = dgv.Name == dgvKoyaita.Name || dgv.Name == dgvOyakui.Name ? jointNumIndexKO : jointNumIndexSR;
        ...
        private bool ChangeEdaban(DataGridView dgv, DataGridViewRow row)
        {
            ...
            int newEdabanIndex = dgv.Name == dgvKoyaita.Name || dgv.Name == dgvOyakui.Name ? newEdabanIndexKO : newEdabanIndexSR;
            ...
            {
                DialogResult dr = MessageBox.Show("CASE" + nowCASE + "-" + newEdaban + "は既に存在しています。CASE" + nowCASE + "-" + newEdaban + "の情報を適用しますか？", "枝番号変更", MessageBoxButtons.YesNo);
                if (dr == DialogResult.Yes)
                {
                    DataGridViewRow sameCASERow = dgv.Rows[existRowIndex];
        ...
        private bool ChangeTypeToZanchiLen(DataGridView dgv, DataGridViewRow row)
        {
            ...
            if(row.Cells.Count != m_editPreviousDGV.Cells.Count) { return false; }
            bool isOk = false;
            for (int i = startIndex; i <= endIndex; i++)
            {
            ...
            {
                DataGridViewRow sameCASERow = dgv.Rows[existRowIndex];
                DialogResult dr = MessageBox.Show("CASE" + GetCellVal(sameCASERow.Cells[0]) + "の設定値と同一です。変更してよろしいですか?", "CASE変更", MessageBoxButtons.YesNo);
                if (dr == DialogResult.Yes)
                {
                    return true;
                }
        ...
        private bool CheackExistCASEAndEdaban(DataGridView dgv, string CASE, string edaban, int targetRowIndex, out int existRowIndex)
        {
            int edabanIndex = dgv.Name == dgvKoyaita.Name || dgv.Name == dgvOyakui.Name ? edabanIndexKO : edabanIndexSR;
            foreach (DataGridViewRow row in dgv.Rows)
            {
        ...
        private bool CheackExistTypeToZanchiLen(DataGridView dgv, DataGridViewRow targetRow, out int existRowIndex, bool isExceptSameCase = false)
        {
            ...
            {
                bool isExist = true; ;
                if(row.Index == targetRow.Index) { continue; }
                for(int i = startIndex; i <= endIndex; i++)
                {
        ...
        private void btnAddEdaban_Click(object sender, EventArgs e)
        {
            ...
            if (dgvKoyaita.CurrentRow != null && dgvKoyaita.Visible) { dataGridView = dgvKoyaita; }
            else if (dgvOyakui.CurrentRow != null && dgvOyakui.Visible) { dataGridView = dgvOyakui; }
            else if (dgvSMW.CurrentRow != null && dgvSMW.Visible) { dataGridView = dgvSMW; }
            else if (dgvConWall.CurrentRow != null && dgvConWall.Visible) { dataGridView = dgvConWall; }
            ...
            if (dataGridView != null)
            {
                ...
                //枝番号は現状では適当な値を設定する
                int edabanIndex = dataGridView.Name == dgvKoyaita.Name || dataGridView.Name == dgvOyakui.Name ? edabanIndexKO : edabanIndexSR;
                int newEdabanIndex = dataGridView.Name == dgvKoyaita.Name || dataGridView.Name == dgvOyakui.Name ? newEdabanIndexKO : newEdabanIndexSR;
                ...
                int kuiIndexStart = dataGridView.Name == dgvKoyaita.Name || dataGridView.Name == dgvOyakui.Name ? KuiIndexStartKO : KuiIndexStartSR;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (dgvKoyaita.Visible) { dataGridView = dgvKoyaita; }
            else if (dgvOyakui.Visible) { dataGridView = dgvOyakui; }
            else if (dgvSMW.Visible) { dataGridView = dgvSMW; }
            else if (dgvConWall.Visible) { dataGridView = dgvConWall; }
