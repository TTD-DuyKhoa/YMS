...
namespace YMS
{
    class ClsTest
    {
        ...
        public static bool CreateIntersectionFamily(UIDocument uidoc)
        {
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                try
                {
                    ...
                    DialogResult dResult = MessageBox.Show("位置をずらしますか？",
                        "確認",
        ...
        #region "TypeIDを変更"
        public static FamilySymbol ChangeTypeID(Document doc, FamilySymbol familySymbol, ElementId targetId, string strNewType)
        {
            // 複製元のタイプを取得
        ...
        /// <returns></returns>
        public static bool CreateHaraokoshiShin(Document doc, List<ElementId> ids, string shinName, DLG.DlgCreateHaraokoshiBase haraokoshiBase)
        {
            //string symbolFolpath = ClsZumenInfo.GetFamilyFolder();
            //string shinfamily = System.IO.Path.Combine(symbolFolpath, "山留壁ファミリ\\" + shinName + ".rfa");
            //using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
        ...
        /// <param name="nDan">1:上段 0:同段 2:下段</param>
        /// <returns></returns>
        public static bool CreateHaraokoshiShin(UIDocument uidoc, ElementId id, string shinName, DLG.DlgCreateHaraokoshiBase haraokoshiBase)
        {
            //Document doc = uidoc.Document;
            //string symbolFolpath = ClsZumenInfo.GetFamilyFolder();
            //string shinfamily = System.IO.Path.Combine(symbolFolpath, "山留壁ファミリ\\" + shinName + ".rfa");
        ...
        public static bool CreateKiribariShin(Document doc, List<ElementId> ids, string shinName, ElementId haraokoshiShin, string sameDan)
        {
            //string symbolFolpath = ClsZumenInfo.GetFamilyFolder();
            //string shinfamily = System.IO.Path.Combine(symbolFolpath, "山留壁ファミリ\\" + shinName + ".rfa");
            //using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
        ...
        public static bool CreateKariKouzai(Document doc)
        {
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                ...
                foreach (ElementId id in targetFamilies)
                {
                    ...
                    double kouzaiSize = ClsRevitUtil.CovertToAPI(ClsCommonUtils.ChangeStrToDbl(sunpou)) / 2;//GetTypeParameter(kozaiSym, "エンドプレートD") / 2;
                    double kouzaiSizeD = ClsRevitUtil.CovertToAPI(ClsCommonUtils.ChangeStrToDbl(sunpou)) / 2;//GetTypeParameter(kozaiSym, "エンドプレートW") / 2;
                    ...
                    if (buzaiName == "腹起")//腹起の場合は外側に仮鋼材を配置
                    {
                            ...
                            for (int i = 0; i < insecList.Count; i++)
                            {
                                ...
                                if (distanceToStart < distanceToEnd)
                                {
                                    ...
                                    tmpStPoint = new XYZ(insecList[i].X - (kouzaiSizeD * 2 * Direction.X),
                                             insecList[i].Y - (kouzaiSizeD * 2 * Direction.Y),
                                             insecList[i].Z);
                                }
                                else
                                {
                        ...
                        tmpStPoint = new XYZ(tmpStPoint.X + (kouzaiSizeD * Direction.Y),
                                             tmpStPoint.Y - (kouzaiSizeD * Direction.X),
                                             tmpStPoint.Z);
                        tmpEdPoint = new XYZ(tmpEdPoint.X + (kouzaiSizeD * Direction.Y),
                                             tmpEdPoint.Y - (kouzaiSizeD * Direction.X),
                                             tmpEdPoint.Z);
                    }
        ...
        public static bool GetBaseCount(UIDocument uidoc)
        {
            ...
            MessageBox.Show("腹起ベースが" + haraList.Count.ToString() + "本、" +
                            "切梁ベースが" + kiriList.Count.ToString() + "本、" +
                            "計" + (haraList.Count + kiriList.Count).ToString() + "本です。");
            return true;
        }
        ...
        public static bool ChangeKariKouzai(UIDocument uidoc)
        {
            ...
            MessageBox.Show("仮鋼材" + kariList.Count.ToString() + "本のパラメータ" +
                            "「コメント」の値を" + para1 + "から" +
                            para2 + "に変更しました。");
            return true;
        }
        ...
        /// <returns></returns>
        public static ElementId CreateWall(Document doc, XYZ startPt, XYZ endPt, ElementId baseLevelId, WallKind kind, string typeName, bool flip = false, bool structural = true)
        {
        ...
        /// <param name="structural">構造のon/off</param>
        /// <returns></returns>
        public static ElementId CreateWall(Document doc, XYZ startPt, XYZ endPt, ElementId baseLevelId, WallType symW, bool flip = false, bool structural = true)
        {
            if (symW == null)
            {
                return null;
        ...
        public static bool PickObjects(UIDocument uidoc, string shinName, string filterName, ref List<ElementId> ids)
        {
            ...
            try
            {
                ...
                IList<Reference> pickedElements = selection.PickObjects(ObjectType.Element, selFilter, shinName + "を配置する芯を選択してください。");
        ...
        public static bool PickObjectsFilters(UIDocument uidoc, string shinName, List<string> filterNameList, ref List<ElementId> ids)
        {
            Document doc = uidoc.Document;
            ids = new List<ElementId>();
            ...
            {
                //IList<Reference> pickedElements = selection.PickObjects(ObjectType.Element, selFilter, "切梁を配置する芯を選択してください。");
                IList<Reference> pickedElements = selection.PickObjects(ObjectType.Element, selFilters, shinName + "を配置する芯を選択してください。");
        ...
        public static bool PickObjectLoop(UIDocument uidoc, string shinName, string filterName, ref List<ElementId> ids)
        {
            ...
            for (; ; )
            {
                try
                {
                    Reference pickedElement = selection.PickObject(ObjectType.Element, selFilter, shinName + "を配置する芯を選択してください。");
        ...
        public static ElementId Create(Document doc, Curve cv, ElementId levelId, FamilySymbol symbol)
        {
            ...
            try
            {
                ...
                FamilyInstance newInstance = doc.Create.NewFamilyInstance(cv, symbol, lv, Autodesk.Revit.DB.Structure.StructuralType.NonStructural);
        ...
        /// <returns></returns>
        public static FamilySymbol GetFamilySymbol(Document doc, string familyName, string typeName, string categoryName = "")
        {
            if (doc == null)
            {
                return null;
        ...
        public static ElementId Create(Document doc, XYZ point, ElementId levelId, FamilySymbol symbol)
        {
            ...
            {
                Level lv = doc.GetElement(levelId) as Level;
                FamilyInstance newInstance = doc.Create.NewFamilyInstance(point, symbol, lv, Autodesk.Revit.DB.Structure.StructuralType.NonStructural);
        ...
        public bool CommandTest_Kurane2(UIApplication uniapp, bool needHinmeiCode)
        {
            ...
            foreach (string targetFolder in targetFolders)
			{
                ...
                string[] allSubFolders = System.IO.Directory.GetDirectories(path, "*", System.IO.SearchOption.AllDirectories);
        ...
        public List<Element> GetVoidsInFamily(Family family)
        {
            ...
            ICollection<Element> elements = collector.OfCategory(BuiltInCategory.OST_GenericModel).OfClass(typeof(FamilyInstance)).ToElements();
