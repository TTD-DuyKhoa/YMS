        ...
        public Result Excute()
        {
            using (Transaction tr = new Transaction(_doc))
            {
                ...
                try
                {
                    ...
                    Curve flatCurve = Line.CreateBound(new XYZ(pntFirst.X, pntFirst.Y, 0), new XYZ(pntSecond.X, pntSecond.Y, 0));

                    Taikeikou taikeikou = new Taikeikou();
                    taikeikou.m_Size = frm.CmbSize.Text;
                    taikeikou.m_KodaiName = frm.CmbKoudainame.Text;
                    taikeikou.m_JointType = frm.RbtAttachWelding.Checked ? DefineUtil.eJoinType.Welding : DefineUtil.eJoinType.Bolt;
                    taikeikou.m_BoltType = frm.RbtAttatchBolt.Checked ? (frm.RbtBolt.Checked ? DefineUtil.eBoltType.Normal : DefineUtil.eBoltType.HTB) : DefineUtil.eBoltType.None;
                    ...
                    foreach (ElementId id in nedaIds)
                    {
                        ...
                        Curve flatNedaC = Line.CreateBound(new XYZ(nedaC.GetEndPoint(0).X, nedaC.GetEndPoint(0).Y, 0), new XYZ(nedaC.GetEndPoint(1).X, nedaC.GetEndPoint(1).Y, 0));
                    ...
                    for (int i = 0; i < crossPnts.Count - 1; i++)
                    {
                        ...
                        if (target1.m_H>=0&&target1.m_H == target2.m_H) { continue; }
                        ...
                        taikeikou.m_TeiketsuType = frm.RbtAttachWelding.Checked ? DefineUtil.eJoinType.Welding : DefineUtil.eJoinType.Bolt;
                        ...
                        string result = Taikeikou.CreateTaikeikou(_doc, target1, target2, taikeikou, onP1, onP2, (double)frm.NmcClearlance.Value, frm.ChkNormalSize.Checked, frm.RbtStiffenerY.Checked);
                        if (result!="完了")
                        {
