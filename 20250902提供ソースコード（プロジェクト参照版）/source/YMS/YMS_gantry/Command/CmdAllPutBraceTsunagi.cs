    ...
    internal class CmdAllPutBraceTsunagi
    {
            ...
            using (var transaction = new Transaction(doc))
            {
...
#endif
                {
                    ...
                    var highestOhbikiBottomFt = MaterialSuper
                        ...
                        .Where(x => x != null)
                        .Select(x => ClsRevitUtil.GetParameterDouble(doc, x.m_ElementId, DefineUtil.PARAM_BASE_OFFSET) - 0.5 * (x.SteelSize?.HeightFeet ?? 0.0))
                        .Max();
                    ...
                    if (viewModel.HrzJointHasK || viewModel.HrzJointHasH)
                    {
                        ...
                        var familyPath = Directory.EnumerateFiles(PathUtil.GetYMSFolder(), Path.GetFileName(viewModel.HrzJointSize.FamilyPath), SearchOption.AllDirectories).FirstOrDefault();
                        ...
                        {
                            var pillarMaster = masterHBeam.FirstOrDefault(x => x.Size == kodaiData.pilePillerData.PillarSize.Replace("-", ""));
                            // 水平つなぎ
                            for (int indexDan = 0; indexDan < danData.Count; indexDan++)
                            {
                                ...
                                var danRefPlane = doc.Create.NewReferencePlane(danPointOnPlane, danPointOnPlane + XYZ.BasisY, XYZ.BasisX, doc.ActiveView);
                                danRefPlane.Maximize3DExtents();



                                // 0:橋長方向, 1:幅員方向
                                var requireFlagSet = new bool[]
                                {
                                    viewModel.HrzJointHasK,
                                    viewModel.HrzJointHasH
                                };
                                var pillarKSetSet = new double[][]
                                {
                                    pillarKSet,
                                    pillarHSet
                                };
                                var pillarHSetSet = new double[][]
                                {
                                    pillarHSet,
                                    pillarKSet,
                                };
                                ...
                                switch (metalShape)
                                {
                                    ...
                                    case MetalShape.C:
                                        ...
                                        {
                                            0.5*pillarMaster.HFt,
                                            -0.5*pillarMaster.BFt,
                                        };
                                        break;
                                    case MetalShape.H:
                                        pillarWidthSet = new double[]
                                        {
                                            0.5 * (pillarMaster.HFt + hbeamB),
                                            -0.5*(pillarMaster.BFt + hbeamB),
                                        };
                                        break;
                                }
                                ...
                                switch (metalShape)
                                {
                                    case MetalShape.L:
                                        ...
                                        {
                                            viewModel.HrzJointLowerIsH ? angleLength : 0.0,
                                            viewModel.HrzJointLowerIsH ? 0.0 : angleLength,
                                        };
                                        break;
                                    case MetalShape.H:
                                    case MetalShape.C:
                                    ...
                                    if (requireFlagSet.ElementAtOrDefault(i))
                                    {
                                        ...
                                        foreach (var pillarH in pillarHSetSet.ElementAtOrDefault(i))
                                        {
                                            ...
                                            foreach (var sign in signSet)
                                            {
                                                ...
                                                var start = origin
                                                    + (pillarH + sign * pillarHeightFt) * dirH
                                                ...
                                                var id = MaterialSuper.PlaceWithTwoPoints(symbol, danRefPlane.GetReference(), end, start, levelDiff);
                                                ...
                                                var material = new HorizontalJoint
                                                {
                                                    ...
                                                    m_TeiketsuType = (viewModel.HrzJointIsBolt) ?DefineUtil.eJoinType.Bolt 
                                                    :( (viewModel.HrzJointIsMetal) ? DefineUtil.eJoinType.Support 
                    ...
                    {
                        var familyPath = Directory.EnumerateFiles(PathUtil.GetYMSFolder(), Path.GetFileName(viewModel.HrzBraceSize.FamilyPath), SearchOption.AllDirectories).FirstOrDefault();
                        var familyName = Path.GetFileNameWithoutExtension(familyPath);
                        ...
                        if (GantryUtil.GetFamilySymbol(doc, familyPath, familyTypeName, out var symbol, true))
                        {
                            ...
                            if (viewModel.HrzBraceHasTopPlate && (kodaiData.pilePillerData.HasTopPlate || kodaiData.pilePillerData.ExtensionPile))
                            {
                                // トッププレート
                                ...
                                {
                                    -highestOhbikiBottomFt,
                                    highestOhbikiBottomFt - plateThick
                                };
                            }
                            else if (viewModel.HrzBraceIsLowerOhbiki)
                            {
                                ...
                                levelOffsetSet = new double[]
                                {
                                    -highestOhbikiBottomFt - ohbikiFrangeThick,
                                    highestOhbikiBottomFt,
                                };
                            }
                            else
                            {
                                ...
                                {
                                    -highestOhbikiBottomFt - ohbikiFrangeThick,
                                    highestOhbikiBottomFt + ohbikiB - ohbikiFrangeThick,
                                };
                            }
                            ...
                            for (int indexK = 0; indexK < pillarKSet.Length - 1; indexK++)
                            {
                                ...
                                for (int indexH = 0; indexH < pillarHSet.Length - 1; indexH++)
                                {
                                    ...
                                    var braceOrigin = origin + (startH + viewModel.HrzBraceYFt) * axisH + (startK + viewModel.HrzBraceXFt) * axisK;
                                    ...
                                    {
                                        negativePlane.GetReference(),
                                        positivePlane.GetReference(),
                                    };

                                    for (int i = 0; i < 2; i++)
                                    {
                                        var braceStart = braceStartSet.ElementAtOrDefault(i);// braceOrigin + 0.5 * c * sinTheta * axisK;
                                        var braceEnd = braceEndSet.ElementAtOrDefault(i);// braceOrigin + (a + 0.5 * c * sinTheta) * axisK + (b - c * cosTheta) * axisH;
                                        ...
                                        var material = new HorizontalBrace
                                        {
                                            ...
                                            m_TeiketsuType = (viewModel.HrzBraceConnectingType == ConnectingType.Bolt) ? DefineUtil.eJoinType.Bolt
                                                    : ((viewModel.HrzBraceConnectingType == ConnectingType.Metal) ? DefineUtil.eJoinType.Support
                                                    : DefineUtil.eJoinType.Welding)
                                        };
                                        MaterialSuper.WriteToElement(material, doc);
                                    }
                    ...
                    // 垂直ブレス
                    if ((viewModel.VrtBraceHasH || viewModel.VrtBraceHasK) && viewModel.VrtBraceSize is ClsAngleCsv vrtBraceSizeRec)
                    {
                        var familyPath = Directory.EnumerateFiles(PathUtil.GetYMSFolder(), Path.GetFileName(viewModel.VrtBraceSize.FamilyPath), SearchOption.AllDirectories).FirstOrDefault();
                        var familyName = Path.GetFileNameWithoutExtension(familyPath);
                        var familyTypeName = "ﾌﾞﾚｰｽ";
                            ...
                            for (int indexFlag = 0; indexFlag < flags.Length; indexFlag++)
                            {
                                ...
                                for (int indexH = 0; indexH < pillarsH.Length - 1; indexH++)
                                {
                                    ...
                                    for (int indexK = 0; indexK < pillarsK.Length; indexK++)
                                    {
                                        ...
                                        var referenceSet = new Reference[]
                                        {
                                        doc.Create.NewReferencePlane(areaOriginKH, areaOriginKH + dirH, axisZ, doc.ActiveView)?.GetReference(),
                                        doc.Create.NewReferencePlane(areaOriginKH, areaOriginKH - dirH, axisZ, doc.ActiveView)?.GetReference(),
                                        };
                                        ...
                                        for (int indexDan = 0; indexDan < danData.Count(); indexDan++)
                                        {
                                            ...
                                            for (int k = 0; k < 2; k++)
                                            {
                                                ...
                                                if (viewModel.VrtBraceHasRound)
                                                {
                                                    ...
                                                    var roundedLength = (Math.Floor(originalLength / viewModel.VrtBraceRoundFt) + 1.0) * viewModel.VrtBraceRoundFt;
                                                ...
                                                var material = new VerticalBrace
                                                {
                                                    ...
                                                    m_ElementId = id,
                                                    m_Size = viewModel.HrzBraceSize.Size,
                                                    m_TeiketsuType = (viewModel.VrtBraceConnectingType == ConnectingType.Bolt) ? DefineUtil.eJoinType.Bolt
                                                    : ((viewModel.VrtBraceConnectingType == ConnectingType.Metal) ? DefineUtil.eJoinType.Support
                                                    : DefineUtil.eJoinType.Welding)
                                                };
                                                MaterialSuper.WriteToElement(material, doc);
                                            }
                    ...
                    // 構台情報を取得
                    var koudaiData = modelData.ListKoudaiData.FirstOrDefault(x => x.AllKoudaiFlatData.KoudaiName == viewModel.SelectedKodai.KoudaiName);
                    ...
                    if(koudaiData != null)
                    {
                        ...
                        koudaiData.BraceTsunagiData.HorizontalBrace.PositionTopFlaBottomAndBottomFlaTop = viewModel.HrzBraceIsOhbikiInner;
                        koudaiData.BraceTsunagiData.HorizontalBrace.PositionBottomFlaTopAndBottomFlaBottom = viewModel.HrzBraceIsLowerOhbiki;
                        ...
                        koudaiData.BraceTsunagiData.HorizontalTsunagi.PositionFukuinTopAndKyoujikuBottom = viewModel.HrzJointUpperIsH;
                        koudaiData.BraceTsunagiData.HorizontalTsunagi.PositionFukuinBottomAndKyoujikuTop = viewModel.HrzJointLowerIsH;
        ...
        public static void DeleteAll(FrmCreateBraceAndJointViewModel viewModel)
        {
            ...
            using (var transaction = new Transaction(doc, Guid.NewGuid().ToString()))
            {
                ...
                var targetInstances = MaterialSuper.Collect(doc)
                    .Where(x => x is JointAndBrace)
                    .Where(x => x.m_KodaiName == kodaiName);

                var targetReferences = targetInstances
                    .Select(x => x.m_Document.GetElement(x.m_ElementId) as FamilyInstance)
                    .Select(x => x?.Host?.Id)
                    .Where(x => x != null)
                    .Distinct();
                ...
                {
                    var deleteInstances = new FilteredElementCollector(doc)
                        .OfClass(typeof(FamilyInstance))
                        .Select(x => x as FamilyInstance)
                        .Where(x => x != null)
                        .Where(x => x.Host?.Id == reference)
                        .Select(x => x?.Id)
                        .Where(x => x != null)
                        .ToList();
