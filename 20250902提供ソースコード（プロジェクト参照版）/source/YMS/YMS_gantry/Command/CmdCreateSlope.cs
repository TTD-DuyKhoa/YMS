...
JackDwj,H500x500,DWJ-50,50H(構台)";
        ...
        private static void TransformSloping(Document doc, AllKoudaiFlatFrmData kodaiData, KodaiSlopeModel slopeModel, AreaRange[] areaRangeSet, bool inverse)
        {
                ...
                foreach (var neda in splitingNedaSet)
                {
                    ...
                    for (int i = 0; i < nedaPtSet.Length - 1; i++)
                    {
                        ...
                        if (i == nedaPtSet.Length - 2)
                        {
                            ...
                            hasEndExtend = true;
                            if (kodaiData.KyoutyouPillarPitch.Sum()<kodaiData.KyoutyouLength)
                            {
                                var dif = kodaiData.KyoutyouLength - kodaiData.KyoutyouPillarPitch.Sum();
                                end += ClsRevitUtil.CovertToAPI(dif) * startToEndUnit;
                            }
                        }
            ...
            foreach (var areaRange in areaRangeSet)
            {
                ...
                if (areaRange._StartOhbikiK==0&& kodaiData.KyoutyouPillarPitch[0] != 0)
                {
                    minKFeet -= ClsRevitUtil.CovertToAPI(kodaiData.KyoutyouPillarPitch[0]);
                }


                if (!inverse&&　areaRange._EndOhbikiK == kodaiData.KyoutyouPillarPitch.Count()-1 && kodaiData.KyoutyouPillarPitch.Sum() < kodaiData.KyoutyouLength)
                {
                    var dif = kodaiData.KyoutyouLength - kodaiData.KyoutyouPillarPitch.Sum();
                    maxKFeet += ClsRevitUtil.CovertToAPI(dif);
                }
