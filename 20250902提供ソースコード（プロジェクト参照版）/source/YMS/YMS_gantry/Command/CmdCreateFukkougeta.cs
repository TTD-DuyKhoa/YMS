    ...
    class CmdCreateFukkougeta
    {
        ...
        public bool Excute()
        {
            ...
            try
            {
                ...
                {
                    string cRbt= (string)frm.panel1.Controls.OfType<RadioButton>().SingleOrDefault(rb => rb.Checked == true).Tag;
                ...
                else
                {
                    ...
                    using (Transaction tr= new Transaction(_doc))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                newId = GantryUtil.PlaceSymbolOverTheSelectedElm(_uiDoc, sym, frm.CmbKoudaiName.Text, level, offset, functionName: "覆工桁配置",paramList:paramList);
                            }
                            else
                            {
                                XYZ p1 = selection.PickPoint("1点目を指定してください");
