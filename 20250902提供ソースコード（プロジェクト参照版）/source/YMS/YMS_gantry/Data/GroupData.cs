...
namespace YMS_gantry.Data
{
    ...
    public class GroupData: GantryData
    {
        ...
        public List<KoudaiData> GetKoudaiList(ModelData md)
        {
            ...
            if ((from data in md.ListKoudaiData where ListKoudaiName.Contains(data.AllKoudaiFlatData.KoudaiName) select data).Any())
            {
                kd = (from data in md.ListKoudaiData where ListKoudaiName.Contains(data.AllKoudaiFlatData.KoudaiName) select data).ToList();
            }
            return kd;
        }
