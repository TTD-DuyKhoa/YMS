...
namespace YMS_gantry
{
    public class GantryUtil
    {
        ...
        public static void SetParameterValueByParameterName(FamilyInstance familyInstance, string parameterName, string parameterValue)
        {
        ...
        /// <param name="parameterName"></param>
        /// <param name="parameterValue"></param>
        public static void SetSymbolParameterValueByParameterName(FamilySymbol familySymbol, string parameterName, string parameterValue)
        {
            Parameter parm = familySymbol.LookupParameter(parameterName);
            if (parm == null) return;
        ...
        /// <returns></returns>
        public static FamilyInstance CreateInstanceWith1point(Document doc, XYZ point, Reference refer, FamilySymbol symbol, XYZ normal = null, bool noHost = false,bool followWithHostType=true)
        {
            if (symbol == null || !noHost&&refer == null) { return null; }
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        newInstance = doc.Create.NewFamilyInstance(point, symbol, (Level)doc.GetElement(element.LevelId), StructuralType.NonStructural);
                    }
                    else
                    {
        ...
        /// <param name="symbol"></param>
        /// <returns></returns>
        public static List<ElementId> CreateInstancesWith1point(Document doc, List<(XYZ point, ElementId levelId)> dataList, FamilySymbol symbol)
        {
            if (symbol == null)
            {
                return null;
            ...
            try
            {
                ...
                foreach ((XYZ p, ElementId levelId) in dataList)
                {
                    ...
                    FamilyInstanceCreationData data = new FamilyInstanceCreationData(p, symbol, lv, StructuralType.NonStructural);
                    creatinDataList.Add(data);
                }

                if (creatinDataList.Count < 1) { return ids; }
        ...
        /// <param name="parameterName"></param>
        /// <param name="parameterValue"></param>
        public static void SetParameterValueByParameterName(FamilySymbol familySymbol, string parameterName, string parameterValue)
        {
            Family family = familySymbol.Family;
        ...
        public static bool GetFamilySymbol(Document doc, string rfaFilePath, string typeName, out FamilySymbol sym, bool isSubTr = false, string name = "")
        {
            sym = null;
        ...
        public static FamilySymbol GetFamilySymbol(Document doc, string typeName, string familyName, bool isSubTr = false)
        {
            ...
            foreach (FamilySymbol fs in collector.OfClass(typeof(FamilySymbol)).Cast<FamilySymbol>())
            {
                ...
                if (fs.Family.Name.Replace("X", "x") == familyName.Replace("X", "x") && fs.Name.Replace("X", "x") == typeName.Replace("X", "x"))
                {
        ...
        public static List<FamilyInstance> GetFamilyInstanceList(Document doc, FamilySymbol fs)
        {
            ...
            return collector
                ...
                .Where(x => x != null && x.Symbol.Name == fs.Name)
                .ToList();
        }
        ...
        public static FamilyInstance GetFamilyInstance(Document doc, ElementId id)
        {
            ...
            return collector
                .OfClass(typeof(FamilyInstance))
                .Select(x => x as FamilyInstance)
                .Where(x => x != null && x.Id.IntegerValue == id.IntegerValue)
                .FirstOrDefault();
        }
        ...
        public static List<string> GetAllTypeName(Document doc,string targetType)
        {
            ...
            foreach (Element element in elements)
            {
                ...
                if (elementType != null)
                {
                    if (elementType.Name.StartsWith(targetType)) { typeNames.Add(elementType.Name); }
                }
            }
        ...
        public List<ElementId> PlaceFamilyInstance(UIApplication uiapp, FamilySymbol symbol, bool isSketch)
        {
            ...
            try
            {
                opt.FaceBasedPlacementType = (isSketch) ? FaceBasedPlacementType.Default : FaceBasedPlacementType.PlaceOnFace;
        ...
        public static Curve GetCurve(Document doc, ElementId id)
        {
            ...
            else
            {
                ...
                if (length == 0)
                {
                    ...
                    if (typeLength != 0) { length = typeLength; }
                }
        ...
        public static ElementId InsertReferenceFamily(Document doc, XYZ point, Level level, bool isSubTr = false)
        {
            ElementId retId;
            FamilySymbol sym;
            if (!GantryUtil.GetFamilySymbol(doc, Path.Combine(PathUtil.GetGantyrUtilFamilyFolderPath(), $"{KoudaiReference.familyName}.rfa"), KoudaiReference.familyName, out sym, true))
            {
        ...
        public static ElementId InsertPointFamily(Document doc, XYZ point, Level level, bool isSubTr = false)
        {
            ElementId retId;
            FamilySymbol sym;
            if (!GantryUtil.GetFamilySymbol(doc, PathUtil.GetGantyrUtilFamilyFolderPath() + "\\Point.rfa", "Point", out sym, true))
            {
                return ElementId.InvalidElementId;
            }
        ...
        public static ElementId InsertLineFamily(Document doc, XYZ start, XYZ end, Level level, bool isSubTr = false)
        {
            ElementId retId;
            FamilySymbol sym;
            if (!GantryUtil.GetFamilySymbol(doc, PathUtil.GetGantyrUtilFamilyFolderPath() + "\\GaRubberLine.rfa", "GaRubberLine", out sym, true))
            {
                return ElementId.InvalidElementId;
            }
        ...
        /// <returns></returns>
        public static ElementId PlaceSymbolOverTheSelectedElm(UIDocument uiDoc, FamilySymbol sym, string koudaiName, Level lev = null,
            double offset = 0, double exStart = 0, double exEnd = 0, string functionName = "",Dictionary<string,string> paramList=null)
        {
            ElementId id = ElementId.InvalidElementId;
            ...
            insOff += offset;
            id = MaterialSuper.PlaceWithTwoPoints(sym, level.GetPlaneReference(), p1, p2, ClsRevitUtil.ConvertDoubleGeo2Revit(insOff));
            if (paramList != null)
            {
                ...
                {
                    GantryUtil.SetParameterValueByParameterName(uiDoc.Document.GetElement(id) as FamilyInstance, kv.Key, kv.Value);
                }
            }
        ...
        /// <returns></returns>
        public static ElementId PlaceSymbolToSelectedPnt(UIDocument uiDoc, FamilySymbol sym, Level level, double offset = 0, double exStart = 0, double exEnd = 0)
        {
            ElementId id = ElementId.InvalidElementId;
            try
            {
                if (sym.Family.FamilyPlacementType == FamilyPlacementType.CurveBased)
                {
                    ...
                    id = MaterialSuper.PlaceWithTwoPoints(sym, level.GetPlaneReference(), p1, p2, ClsRevitUtil.ConvertDoubleGeo2Revit(offset));
                }
            ...
            try
            {
                if (ins.Host == null)
                {
                    ...
                    else
                    {
                        ...
                        if (el.GetType() == typeof(Level)) { level = el as Level; }
                    }
                }
                else
                {
        ...
        /// <param name="symbol"></param>
        /// <returns></returns>
        public static FamilyInstance CreateFamilyInstanceOnFace(Document doc, Line line, Reference face, FamilySymbol symbol)
        {
            if (symbol == null || face == null) { return null; }
                ...
                foreach (var obj in geom)
                {
                    if (obj is Solid solid)
                    {
                        foreach (Face face in solid.Faces)
                        {
                            ...
                            {
                                if (retFace != null && retFace.Area > face.Area) { continue; }
                                retFace = face;
                            }
                        }
                    }
            ...
            foreach (GeometryInstance geometryObject in geometryElement)
            {
                ...
                foreach (var obj in geom)
                {
                    if (obj is Solid solid)
                    {
                        foreach (Face face in solid.Faces)
                        {
                            if(refer.ConvertToStableRepresentation(familyInstance.Document) == face.Reference.ConvertToStableRepresentation(familyInstance.Document))
                            {
                                retFace = face;
                                break;
                            }
        ...
        public static bool AreComponentsSameSign(XYZ vector1, XYZ vector2)
        {
            ...
            if (vector1.DistanceTo(vector2) < 0.1) { return true; }
        ...
        /// <returns></returns>
        public static bool ChkDoubleValueAsString(string text, bool bMinus = false, bool bZero = false, bool bComma = false, bool bMax = false, double dMax = 0)
        {
            try
            {
                double d = 0;
        ...
        public static List<XYZ> FindIntersectionPnts(Document doc,ElementId id1,ElementId id2)
        {
            ...
            if (typeof(FamilyInstance) != elem.GetType()) {return retList; }
        ...
        public static List<XYZ> OverlapCheck(Element elm1, Element elm2, double distanceBorder = 0.1)
        {
            ...
            foreach (Edge edge in elm1EdgeArrays)
            {
                ...
                try
                {
                    if (ClsGeo.GEO_EQ(curveSP.X, curveEP.X) &&
                        ClsGeo.GEO_EQ(curveSP.Y, curveEP.Y) &&
        ...
        /// <returns></returns>
        public static bool SizeEditMove(Document doc,ElementId targetId,double originalLength,double editedLength, bool isBasePointTop, bool isVertical = false)
        {
            //長さが変わらなければ何もしない
            if (ClsGeo.GEO_EQ(originalLength,editedLength)) { return true; }
            bool resBool=false;
            try
            {
                //部材の終始点を取得
                ...
                {
                    //標準ベクトルを取得
                    XYZ cVec = ((isBasePointTop ? c.GetEndPoint(1) : c.GetEndPoint(0)) - (isBasePointTop ? c.GetEndPoint(0) : c.GetEndPoint(1))).Normalize();
                    //移動距離
                    double dist =Math.Abs(originalLength - editedLength)/2;
                    //移動方向 短くなる場合は 始⇒終 伸びる倍は逆
                    XYZ moveVec = ((originalLength > editedLength) ? cVec : cVec.Negate()) * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(dist);
                    //移動
        ...
        /// <returns></returns>
        public static FamilySymbol DuplicateTypeWithNameRule(Document doc,string koudaiName,FamilySymbol original,string originalTypeName,string koudaiTypeName="")
        {
                ...
                if (!noData)
                {
                    ...
                    foreach (string tName in types)
                    {
                        if (tName.StartsWith(typeName))
                        {
                            ...
                            if (numSt.Length != 3) { numSt = numSt.Substring(0, numSt.Length - 3); }
