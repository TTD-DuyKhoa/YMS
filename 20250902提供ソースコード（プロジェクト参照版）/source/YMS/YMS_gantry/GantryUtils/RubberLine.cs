...
namespace YMS_gantry
{
    public class RubberLine
    {
        ...
        /// <returns></returns>
        public Result Get2Point(UIApplication uiapp,string familyPath, out XYZ pt1, out XYZ pt2,out ElementId idOut, bool removeRubberLine = true)
        {
            pt1 = XYZ.Zero;
            pt2 = XYZ.Zero;
            ...
            if (elementId_added.Count >= 1)
            {
                for (int i = 0; i < elementId_added.Count(); i++)
                {
                    ...
                    if (fi != null)
                    {
                        ...
                        List<GeometryInstance> geoms = fi.get_Geometry(options).Where(o => o is GeometryInstance).Cast<GeometryInstance>().ToList();
        ...
        public static RubberLine Instance { get { return instance; } }
    }
    ...
    public class rubberLineUpdater : IUpdater
    {
        ...
        {
            var set = data.GetAddedElementIds().Select(x => data.GetDocument().GetElement(x) as FamilyInstance).Where(x => x != null && x.Name == familyName).Distinct();
            if (set.Count() >= 1)
            {
                internalESC = true;
