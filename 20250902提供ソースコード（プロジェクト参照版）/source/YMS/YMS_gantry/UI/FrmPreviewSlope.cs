            ...
            else
            {
                ...
                for (int i = 0; i < SlopeData.Data.Count; i++)
                {
                    ...
                    if (cData.type == SlopeType.none)
                    {
                        ...
                        currentFlat = /*(currentFlat > 0 && currentFlat != (float)cData.height) ? */(float)cData.height /*: currentFlat*/;
                        isfEnt = false;
                        angle = 0;
                    }
                    ...
                    sSet.SpanHeight = h;
                    sSet.SpanLength = length;
                    sSet.SpnarealLeng = (float)(Math.Sqrt((length*widthCoef) *(length*widthCoef)/* +(h*highthCoef) * (h*highthCoef)*/));
