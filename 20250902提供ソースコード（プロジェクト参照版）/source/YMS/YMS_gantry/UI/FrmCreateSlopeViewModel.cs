    ...
    public class FrmCreateSlopeViewModel : BindableSuper
    {
            ...
            var result = new KodaiSlopeModel
            {
                ...
                StyleList = Enumerable.Range(0, kyochoNum)
                    .Select(x => new StyleRow
                    {
                        Index = x,
                        Level = 0.0,
                        SlopeType = SlopeStyle.Nil,
                        Percent = 0.0,
                    })
                    .ToArray(),
                SupportList = Enumerable.Range(0, kyochoNum + 1)
                    .Select(x => new SupportRow
                    {
                        Index = x,
        ...
        public static bool EqualValue<T>(T a, T b) where T : class
        {
            ...
            var properties = typeof(T)
                ...
                .ToArray();
            return properties
                .All(x => x.GetValue(a).Equals(x.GetValue(b)));
        }
        public static bool EqualValueArray<T>(IEnumerable<T> a, IEnumerable<T> b) where T : class
        {
            ...
            {
                if (!EqualValue(arrayA.ElementAtOrDefault(i), arrayB.ElementAtOrDefault(i))) { return false; }
            }
            return true;
        }
