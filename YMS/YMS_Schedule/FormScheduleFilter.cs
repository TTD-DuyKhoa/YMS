...
namespace YMS_Schedule
{
  public partial class FormScheduleFilter : System.Windows.Forms.Form
  {
    ...
    private void FormScheduleFilter_Load( object sender, EventArgs e )
    {
      ...
      //item
      Dictionary<string, Dictionary<string, List<string>>> dataItem =
        new Dictionary<string, Dictionary<string, List<string>>>() ;
      results = ( from fields in tbl
        where fields[ indexItem1 ] != ""
        select String.Format( "{0}", fields[ indexItem1 ] ) ).Distinct() ;
      ...
      foreach ( var x in dataItem.Keys.ToList() ) {
        results = ( from fields in tbl
          where fields[ indexItem1 ] == x && fields[ indexItem2 ] != ""
          select String.Format( "{0}", fields[ indexItem2 ] ) ).Distinct() ;
        if ( results != null && results.Count() >= 1 ) {
      ...
      foreach ( var x in dataItem.Keys ) {
        foreach ( var y in dataItem[ x ].Keys.ToList() ) {
          results = ( from fields in tbl
            where fields[ indexItem1 ] == x && fields[ indexItem2 ] == y && fields[ indexItem3 ] != ""
            select String.Format( "{0}", fields[ indexItem3 ] ) ).Distinct() ;
          if ( results != null && results.Count() >= 1 ) {
      ...
      results = ( from fields in tbl where fields[ indexDan ] != "" select String.Format( "{0}", fields[ indexDan ] ) )
        .Distinct() ;
      if ( results != null && results.Count() >= 1 ) {
      ...
      foreach ( var x in filterPhase ) {
        ...
        results = ( from fields in tbl where fields[ index ] != "" select String.Format( "{0}", fields[ index ] ) )
          .Distinct() ;
        if ( results != null && results.Count() >= 1 ) {
    ...
    private void button1_Click( object sender, EventArgs e )
    {
      ...
      if ( checkBox1.Checked ) {
        ...
        foreach ( var x in checkedItem ) {
          ...
          results = ( from fields in tbl
            where fields[ indexItem1 ] == items[ 0 ] && fields[ indexItem2 ] == items[ 1 ] &&
