...
namespace YMS_Schedule
{
  public class YamadomeKoudaiBoltSekisan
  {
    ...
    public bool buzaiAndShuzaiBoltData( Document doc, List<ElementId> idBuzaiList, List<ElementId> idShuzaiList,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID )
    {
      try {
        //マスタ読み込み
        ...
        List<FamilyInstance> instBuzaiList =
          YamadomeKoudaiBoltSekisan.ElementIdList2FamilyinstanceList( doc, idBuzaiList ) ;
        ...
        foreach ( FamilyInstance fi in coverplateInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            SetBoltData( doc, md, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
              outputKigou: kigou ) ;
            if ( ! string.IsNullOrWhiteSpace( md.No13_AddBolt ) ) {
              SetBoltDataAddBolt( doc, md, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist,
                ref yamadomeBoltSheetDatalistWithID ) ;
            }
        ...
        foreach ( FamilyInstance fi in sumibuPieceInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            SetBoltData( doc, md, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
              plusAtsumi: 10 ) ; //YSSのDBと比較して首下が短くなるので10補正を入れておく　裏込めの分？
            break ;
          }
        }
        ...
        foreach ( FamilyInstance fi in hiuchiukePieceInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            SetBoltData( doc, md, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
              outputName: outputName, plusAtsumi: 5 ) ; //YSSのDBと比較して首下が短くなるので5補正を入れておく
            cnt++ ;
            if ( cnt == 1 ) {
              SetBoltDataAddBolt( doc, md, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist,
                ref yamadomeBoltSheetDatalistWithID, outputName: outputName ) ;
            }
        ...
        foreach ( FamilyInstance fi in jackCoverInstanceList ) {
          ...
          SetBoltDataForJackCover( doc, hitData, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist,
            ref yamadomeBoltSheetDatalistWithID ) ;
        }
        ...
        foreach ( FamilyInstance fi in gussetPlateInstanceList ) {
          ...
          SetBoltData( doc, hitData, fi, Ids[ 0 ], ref yamadomeBoltSheetDatalist,
            ref yamadomeBoltSheetDatalistWithID ) ;
        }
        ...
        foreach ( FamilyInstance fi in shohiuchiBlockInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            //接続してる補助ピース/主材のリスト
            List<ElementId> targetHojoPieceInstanceListIds =
              GetSameLevelShuzaiList( doc, fi, hojoPieceIdList, md.No7_Type ) ;
            List<ElementId> targetIds = GetSameLevelShuzaiList( doc, fi, idShuzaiList, md.No7_Type ) ;
        ...
        foreach ( FamilyInstance fi in hojoPieceInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            List<ElementId> targetIds = new List<ElementId>() ;
            if ( string.IsNullOrWhiteSpace( md.No7_Type ) ) {
              SetBoltDataSingle( doc, md, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
                outputName: "補助ピース" ) ;
              break ;
            }
            else {
        ...
        foreach ( FamilyInstance fi in kaitenPieceInstanceList ) {
          ...
          foreach ( stMasterData md in checkList ) {
            ...
            List<ElementId> Ids =
              ClsRevitUtil.GetIntersectFamilysLiteExtensionZDirIs0pnt1( doc, fi.Id, null, targetIds ) ;
            if ( Ids.Count < 1 ) {
              continue ;
            }
            ...
            if ( kigouList.Count == 1 && string.IsNullOrEmpty( kigouList[ 0 ] ) ) {
              ...
              if ( kigouList.Contains( kigou ) ) {
                SetBoltData( doc, md, fi, targetId, ref yamadomeBoltSheetDatalist,
                  ref yamadomeBoltSheetDatalistWithID ) ;
                break ;
              }
            }
        ...
        foreach ( FamilyInstance fi in shabariukePieceInstanceList ) {
          ...
          if ( ! string.IsNullOrWhiteSpace( hitData.No13_AddBolt ) ) {
            SetBoltDataSingleAddBolt( doc, hitData, fi, ref yamadomeBoltSheetDatalist,
              ref yamadomeBoltSheetDatalistWithID ) ;
          }
        }
        ...
        foreach ( FamilyInstance fi in kiribariTsunagizaiBaseInstanceList ) {
          SetBoldDataFromCustumDataForBase( doc, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
            outputName: "切梁繋ぎ材" ) ;
        }
        ...
        foreach ( FamilyInstance fi in atamaTsunagizaiInstanceList ) {
          SetBoldDataFromCustumData( doc, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
            outputName: "頭つなぎ材" ) ;
        }
        ...
        foreach ( FamilyInstance fi in sendanBoltHokyozaiInstanceList ) {
          SetBoldDataFromCustumData( doc, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
            outputName: "せん断ボルト補強材" ) ;
        }

        //㉑腹起スベリ止メ積算
        foreach ( FamilyInstance fi in haraokoshiSuberidomeInstanceList ) {
          SetBoldDataFromCustumDataForHaraokoshiSuberidome( doc, fi, ref yamadomeBoltSheetDatalist,
            ref yamadomeBoltSheetDatalistWithID, outputName: "腹起スベリ止メ" ) ;
        }
    ...
    /// <param name="yamadomeBoltSheetDatalist"></param>
    /// <param name="yamadomeBoltSheetDatalistWithID"></param>
    private void SetBoldDataFromCustumData( Document doc, FamilyInstance fi,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "" )
    {
      ...
      SetBoltDataSingle( doc, boltName, boltNum, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
        outputName, outputKigou: kigou ) ;
    }

    private void SetBoldDataFromCustumDataForBase( Document doc, FamilyInstance fi,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "" )
    {
      var boltNameAndNum = ClsYMSUtil.GetBolt( doc, fi.Id ) ;
      ...
      SetBoltDataSingle( doc, boltName, boltNum, fi, ref yamadomeBoltSheetDatalist, ref yamadomeBoltSheetDatalistWithID,
        outputName: outputName, outputKigou: kigou ) ;
    }

    private void SetBoldDataFromCustumDataForHaraokoshiSuberidome( Document doc, FamilyInstance fi,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "" )
    {
      var boltNameAndNum = ClsYMSUtil.GetBolt( doc, fi.Id ) ;
    ...
    public bool ShuzaiAndShuzaiBoltData( Document doc, ClsSekisanSetting sekisansetting, List<ElementId> shuzaiIdList,
      ref List<stBoltDataForBoltSheet> shuzaiBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> shuzaiBoltSheetDatalistWithID )
    {
      try {
        //マスタ読み込み
        List<stMasterData> masterData = new List<stMasterData>() ;
        ...
        List<FamilyInstance> instShuzaiList =
          YamadomeKoudaiBoltSekisan.ElementIdList2FamilyinstanceList( doc, shuzaiIdList ) ;
        ...
        foreach ( ElementId joudanSumihiutiShuzaiId in JoudanTargetIds ) {
          ...
          List<ElementId> Ids =
            ClsRevitUtil.GetIntersectFamilysLite( doc, joudanSumihiutiShuzaiId, null, GedanTargetIds ) ;
          if ( Ids.Count < 1 ) {
            continue ;
          }

          SetBoltData( doc, hitData, inst, Ids[ 0 ], ref shuzaiBoltSheetDatalist, ref shuzaiBoltSheetDatalistWithID,
            outputName: "隅火打(W)" ) ;
        }


        //腹起横W
        HaraokoshiYokoW_BoltCount( doc, masterData, sekisansetting, targetIds, ref shuzaiBoltSheetDatalist,
          ref shuzaiBoltSheetDatalistWithID ) ;
      }
      catch ( Exception ) {
        return false ;
      }
    ...
    private bool HaraokoshiYokoW_BoltCount( Document doc, List<stMasterData> masterData,
      ClsSekisanSetting sekisansetting, List<ElementId> targetIds,
      ref List<stBoltDataForBoltSheet> shuzaiBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> shuzaiBoltSheetDatalistWithID )
    {
      ...
      try {
        foreach ( ElementId id in targetIds ) {
          ...
          if ( inst.Symbol.Name == "腹起" ) {
            string familyName = inst.Symbol.FamilyName ;
            if ( ! familyName.Contains( "35HA" ) && ! familyName.Contains( "35SMH" ) &&
                 ! familyName.Contains( "40HA" ) && ! familyName.Contains( "40SMH" ) &&
                 ! familyName.Contains( "50HA" ) ) {
              continue ;
            }
        ...
        foreach ( YMS.Parts.ClsHaraokoshiBase baseCls in baseList ) {
          ...
          var nAns = 2.0 * Math.Ceiling( ( half - cpLen * ( haraCount - 2 ) ) / dBoltInterval ) +
                     boltNum * ( haraCount - 2 ) ;
          SetBoltDataSingle( doc, "BN-70", (int) nAns, familyInstance, ref shuzaiBoltSheetDatalist,
            ref shuzaiBoltSheetDatalistWithID, outputName: "腹起　横W" ) ;
        }
        ...
        foreach ( YMS.Parts.ClsHaraokoshiBase baseCls in baseList ) {
          ...
          foreach ( ElementId id in yokoWHaraList ) {
            ...
            decimal length =
              (decimal) ClsRevitUtil.CovertFromAPI( lCurve.Curve.GetEndPoint( 0 )
                .DistanceTo( lCurve.Curve.GetEndPoint( 1 ) ) ) ;
    ...
    private List<ElementId> GetSameLevelShuzaiList( Document doc, FamilyInstance fi, List<ElementId> idShuzaiList,
      string typeName = "" )
    {
    ...
    /// <returns></returns>
    private bool SetBoltData( Document doc, stMasterData masterData, FamilyInstance inst, ElementId hitTargetId,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "", string outputKigou = "",
      double plusAtsumi = 0, int kashoSuu = 1 )
    {
      try {
        ...
        for ( int i = 0 ; i < yamadomeBoltSheetDatalist.Count ; i++ ) {
          ...
          if ( dt.hinshu == stdata.hinshu && dt.DanName == stdata.DanName && dt.buzaiName == stdata.buzaiName &&
               dt.Kigou == stdata.Kigou && dt.boltName == stdata.boltName && dt.num == stdata.num ) {
            dt.Kashosu += 1 ;
            sameHit = true ;
    ...
    /// <returns></returns>
    private bool SetBoltDataForJackCover( Document doc, stMasterData masterData, FamilyInstance inst,
      ElementId hitTargetId, ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "", string outputKigou = "",
      double plusAtsumi = 0, int kashoSuu = 1 )
    {
      try {
    ...
    /// <param name="kashoSuu"></param>
    /// <returns></returns>
    private bool SetBoltDataSingle( Document doc, stMasterData masterData, FamilyInstance inst,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "", string outputKigou = "",
      double plusAtsumi = 0, int kashoSuu = 1 )
    {
    ...
    /// <param name="kashoSuu"></param>
    /// <returns></returns>
    private bool SetBoltDataSingle( Document doc, string boltName, int boltNum, FamilyInstance inst,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "", string outputKigou = "",
      double plusAtsumi = 0, int kashoSuu = 1 )
    {
    ...
    private bool SetBoltDataSingleAddBolt( Document doc, stMasterData masterData, FamilyInstance inst,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "",
      string outputKigou = "" )
    {
    ...
    private bool SetBoltDataAddBolt( Document doc, stMasterData masterData, FamilyInstance inst, ElementId hitTargetId,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID, string outputName = "",
      string outputKigou = "" )
    {
    ...
    public bool KoudaiBoltSekisan( Document doc, ClsSekisanSetting sekisansetting,
      ref List<stBoltDataForBoltSheet> koudaiBoltSheetDatalist,
      ref List<stBoltDataForBoltSheet> koudaiBoltSheetDatalistWithID )
    {
      try {
        //構台ボルト収集
        //[水平ブレス]取付方法
        ...
        if ( suiheiBraceTorituke == "ボルト" ) {
          if ( suiheiBraceBoltType == "普通ボルト" || suiheiBraceBoltType == "HTB" ) {
            List<(FamilyInstance, List<FamilyInstance>)> dataOandH =
              ReadFromGantryProducts.GetOhbikiAndHBraceSet( doc, includeShikigeta: true ) ;

            List<(FamilyInstance, List<FamilyInstance>)> dataPandH =
              ReadFromGantryProducts.GetPilePillerAndHBraceSet( doc ) ;
            ...
            foreach ( (FamilyInstance, List<FamilyInstance>) setData in dataOandH ) {
              ...
              foreach ( FamilyInstance HB in HBList ) {
                ...
                for ( int i = 0 ; i < koudaiBoltSheetDatalist.Count ; i++ ) {
                  ...
                  if ( dt0.hinshu == stdata.hinshu && dt0.Kigou == stdata.Kigou && dt0.boltName == stdata.boltName &&
                       dt0.num == stdata.num ) {
                    dt0.Kashosu += 1 ;
                    sameHit = true ;
        ...
        if ( kiribariukeToritukehouhou == "なし" ) {
          if ( kiribariukeKoteihouhou == "ボルト" ) {
            ...
            var kF = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.Kiribari, ReadFromGantryProducts.eAttachSide.Flange ) ;
            ...
            var kW = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.Kiribari, ReadFromGantryProducts.eAttachSide.Web ) ;
        ...
        if ( VerToritsukeHoho == "なし" ) {
          if ( VerKoteiHoho == "ボルト" ) {
            ...
            var tF = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.VBrace, ReadFromGantryProducts.eAttachSide.Flange ) ;
            ...
            var tW = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.VBrace, ReadFromGantryProducts.eAttachSide.Web ) ;
            ...
            var hF = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.HJoint, ReadFromGantryProducts.eAttachSide.Flange ) ;
            ...
            var hW = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.HJoint, ReadFromGantryProducts.eAttachSide.Web ) ;
            ...
            var ws = ReadFromGantryProducts.GetKoudaivBracehJointBoltCnt( doc,
              ReadFromGantryProducts.eBoltElement.Support, ReadFromGantryProducts.eAttachSide.Web ) ;
        ...
        List<(FamilyInstance, List<FamilyInstance>)> dataON =
          ReadFromGantryProducts.GetOhbikiAndNedaSet( doc, false, includeShikigeta: true ) ;
        List<(FamilyInstance, List<FamilyInstance>)> dataONSlope =
          ReadFromGantryProducts.GetOhbikiAndNedaSet( doc, true, includeShikigeta: true ) ;
            ...
            foreach ( FamilyInstance neda in nedaList ) {
              ...
              foreach ( (FamilyInstance, List<FamilyInstance>) d in dataONSlope ) {
                ...
                foreach ( FamilyInstance isttmp in ceckNedaList ) {
                  //大引きと接していないスロープ根太は除く
                  if ( RevitUtil.ClsRevitUtil.GetIntersectFamilys( doc, isttmp.Id, 0.1, serchIds: tmpList2 ).Count >
                       0 ) {
              ...
              for ( int i = 0 ; i < koudaiBoltSheetDatalist.Count ; i++ ) {
                ...
                if ( dt.hinshu == stdata.hinshu && dt.Kigou == stdata.Kigou && dt.boltName == stdata.boltName &&
                     dt.num == stdata.num ) {
                  dt.Kashosu += 1 ;
                  sameHit = true ;
        ...
        if ( KuiKuiKetauke == "普通ボルト" || KuiKuiKetauke == "HTB" ) {
          List<(FamilyInstance, List<FamilyInstance>)> data =
            ReadFromGantryProducts.GetPilePillerAndKetaUkeSet( doc, includeShikigeta: true ) ;
          ;
    ...
    /// <returns></returns>
    public static int GetYamadomeBoltLength( enBotType type, double sumAtsumi, /*List<double> atsumiList,*/
      double plusAtsumi = 0 )
    {
    ...
    public static int GetKoudaiBoltLength( enBotType type, bool slope = false, double sumAtsumi = 0,
      double plusAtsumi = 0 )
    {
      double res = plusAtsumi + sumAtsumi ;
    ...
    /// <returns></returns>
    public List<stMasterData> GetBoltEstimationList( string target, List<stMasterData> masterData,
    ...
    public int GetIntFromCSVList( List<string> lstStr, int index )
    {
      ...
      //値が空白であれば0を返す
      if ( string.IsNullOrEmpty( lstStr[ index ] ) || string.IsNullOrWhiteSpace( lstStr[ index ] ) ) {
        return 0 ;
      }

      int result ;
      if ( ! int.TryParse( lstStr[ index ], out result ) ) {
        return 0 ;
      }
    ...
    public double GetDoubleFromCSVList( List<string> lstStr, int index )
    {
      ...
      double result ;
      if ( ! double.TryParse( lstStr[ index ], out result ) ) {
        return 0 ;
      }
