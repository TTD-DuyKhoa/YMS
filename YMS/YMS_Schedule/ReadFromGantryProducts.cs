...
namespace YMS_Schedule
{
    public class ReadFromGantryProducts
    {
        ...
        public static List<(FamilyInstance, List<FamilyInstance>)> GetOhbikiAndNedaSet(Document doc, bool targetSlope = false, string koudaiName = "", bool includeShikigeta = false)
        {
            ...
            try
            {
                ...
                var ms = string.IsNullOrEmpty(koudaiName) ? MaterialSuper.Collect(doc) : MaterialSuper.Collect(doc).Where(x => x.m_KodaiName == koudaiName);
                if (ms == null || ms.Count() < 1) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                ...
                if (includeShikigeta)
                {
                    List<OhbikiSuper> lstShikigeta = materilas.Where(x => x.GetType() == typeof(Shikigeta)).Select(x => x as OhbikiSuper)?.ToList() ?? null;
                    if (lstShikigeta != null && lstShikigeta.Count > 0) lstOhbiki.AddRange(lstShikigeta);
                }
                if (lstOhbiki == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }

                //根太のみ抽出
                List<Neda> lstNeda = materilas.Where(x => x.GetType() == typeof(Neda)).Select(x => x as Neda).ToList();
                if (lstNeda == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                ...
                foreach (OhbikiSuper ohbiki in lstOhbiki)
                {
                    ...
                    //接する根太のインスタンスを収集
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, ohbiki.m_ElementId, serchIds: nedaIds);
        ...
        /// <returns></returns>
        public static bool SlopenedaAndNedaConnect(Document doc, ElementId nedaID, ElementId ohbikiID = null, string koudaiName = "", bool includeShikigeta = false)
        {
            bool hit = false;
            ...
            try
            {
                ...
                if (lstOhbiki == null) { return false; }

                //根太のみ抽出
                List<Neda> lstNeda = materilas.Where(x => x.GetType() == typeof(Neda)).Select(x => x as Neda).ToList();
                if (lstNeda == null) { return false; }
                List<ElementId> nedaIds = lstNeda.Where(x => x != null).Select(x => x.m_ElementId).ToList();
        ...
        /// <param name="includeShikigeta">敷桁を含めるかtrue=敷桁と根太の交点も対象とする　false=敷桁を対象としない Default=false</param>
        /// <returns></returns>
        public static List<(FamilyInstance, List<FamilyInstance>)> GetPilePillerAndKetaUkeSet(Document doc, string koudaiName = "", bool includeShikigeta = false)
        {
            List<(FamilyInstance, List<FamilyInstance>)> retList = new List<(FamilyInstance, List<FamilyInstance>)>();
            try
            {
                ...
                //杭関連ファミリのみ抽出
                List<PilePillerSuper> lstPiler = materilas.Where(x => x.GetType() == typeof(PilePiller)).Select(x => x as PilePillerSuper).ToList();
                if (lstPiler == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                List<PilePillerSuper> lstExPiler = materilas.Where(x => x.GetType() == typeof(ExtensionPile)).Select(x => x as PilePillerSuper).ToList();
                if (lstExPiler.Count > 0) { lstPiler.AddRange(lstExPiler); }
                //大引のみ抽出
                List<OhbikiSuper> lstOhbiki = materilas.Where(x => x.GetType() == typeof(Ohbiki)).Select(x => x as OhbikiSuper)?.ToList() ?? null;
                //敷桁を含む場合は敷桁も
                if (includeShikigeta)
                ...
                foreach (PilePillerSuper piller in lstPiler)
                {
                    ...
                    //接するインスタンスを収集
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, piller.m_ElementId, 2, serchIds: ohbikiIds);

                    List<FamilyInstance> ohbikiIns = new List<FamilyInstance>();
                    foreach (ElementId id in intsId)
                    {
                        FamilyInstance nIns = doc.GetElement(id) as FamilyInstance;
                        if (nIns != null) { ohbikiIns.Add(nIns); }
                    }
        ...
        /// <param name="koudaiName">特定の構台のみを対象とするか 空=図面内のすべての構台から部材を収集する 指定:指定した構台からのみ部材を収集する  Default=""</param>
        /// <returns></returns>
        public static List<(FamilyInstance, List<FamilyInstance>)> GetPilePillerAndHBraceSet(Document doc, string koudaiName = "")
        {
            List<(FamilyInstance, List<FamilyInstance>)> retList = new List<(FamilyInstance, List<FamilyInstance>)>();
            try
            {
                ...
                //水平ブレス取得
                List<FamilyInstance> suiheiBraceLst = ReadFromGantryProducts.GetFamilyInstanceListBy(doc, ReadFromGantryProducts.eElementType.HBrace);
                ...
                if (suiheiBraceLst == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                ...
                foreach (PilePillerSuper piller in lstPiler)
                {
                    ...
                    //接する水平ブレスのインスタンスを収集
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, piller.m_ElementId, 1, serchIds: HBraceIds);

                    List<FamilyInstance> HBIns = new List<FamilyInstance>();
                    foreach (ElementId id in intsId)
                    {
                        FamilyInstance nIns = doc.GetElement(id) as FamilyInstance;
                        if (nIns != null) { HBIns.Add(nIns); }
                    }
        ...
        /// <param name="koudaiName">特定の構台のみを対象とするか 空=図面内のすべての構台から部材を収集する 指定:指定した構台からのみ部材を収集する  Default=""</param>
        /// <returns></returns>
        public static List<(FamilyInstance, List<FamilyInstance>)> GetOhbikiAndHBraceSet(Document doc, string koudaiName = "", bool includeShikigeta = false)
        {
            List<(FamilyInstance, List<FamilyInstance>)> retList = new List<(FamilyInstance, List<FamilyInstance>)>();
            try
            {
                ...
                if (suiheiBraceLst == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                List<OhbikiSuper> lstOhbiki = materilas.Where(x => x.GetType() == typeof(Ohbiki) && x.m_Size.StartsWith("[")).Select(x => x as OhbikiSuper)?.ToList() ?? null;
                //敷桁を含む場合は敷桁も
                if (includeShikigeta)
                {
                    List<OhbikiSuper> lstShikigeta = materilas.Where(x => x.GetType() == typeof(Shikigeta)).Select(x => x as OhbikiSuper)?.ToList() ?? null;
                ...
                foreach (OhbikiSuper ohbiki in lstOhbiki)
                {
                    ...
                    //接する水平ブレスのインスタンスを収集
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, ohbiki.m_ElementId, 1, serchIds: HBraceIds);
        ...
        /// <param name="koudaiName">特定の構台のみを対象とするか 空=図面内のすべての構台から部材を収集する 指定:指定した構台からのみ部材を収集する  Default=""</param>
        /// <returns></returns>
        public static List<(FamilyInstance, List<FamilyInstance>)> GetChannelKetaukeSet(Document doc, string koudaiName = "")
        {
            List<(FamilyInstance, List<FamilyInstance>)> retList = new List<(FamilyInstance, List<FamilyInstance>)>();
            try
            {
                ...
                //杭関連ファミリのみ抽出
                List<PilePillerSuper> lstPiler = materilas.Where(x => x.GetType() == typeof(PilePillerSuper)).Select(x => x as PilePillerSuper).ToList();
                if (lstPiler == null) { return new List<(FamilyInstance, List<FamilyInstance>)>(); }
                ...
                foreach (OhbikiSuper ohbiki in lstOhbiki)
                {
                    ...
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, ohbiki.m_ElementId, serchIds: ohbikiintIds);
                    List<FamilyInstance> ohbikiIns = new List<FamilyInstance>();
                    BoundingBoxXYZ bb = oIns.get_BoundingBox(null);
                    foreach (ElementId id in intsId)
                    {
                        List<XYZ> pntsList = GantryUtil.FindIntersectionPnts(doc, ohbiki.m_ElementId, id);
                        if (pntsList.Count <= 2) { continue; }
                        double totalX = 0.0;
                        double totalY = 0.0;
                        double totalZ = 0.0;
                        // すべての点の座標を合計する
                        ...
                        if (pp.DistanceTo(oCu.GetEndPoint(0)) < RevitUtil.ClsRevitUtil.CovertToAPI(1) || pp.DistanceTo(oCu.GetEndPoint(1)) < RevitUtil.ClsRevitUtil.CovertToAPI(1))
                        {
                            continue;
                        }
                        FamilyInstance nIns = doc.GetElement(id) as FamilyInstance;
        ...
        public static int GetKoudaivBracehJointBoltCnt(Document doc, eBoltElement eType, eAttachSide eSide)
        {
            ...
            try
            {
                ...
                if (ms == null || ms.Count() < 1) { return 0; }
                List<MaterialSuper> materilas = ms.ToList();
                //柱,支柱
                List<ElementId> pillers = materilas.Select(x => x as PilePillerSuper)
                    .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId)
        ...
        private static int GetVBracesAttachCount(Document doc, eAttachSide eSide)
        {
            ...
            try
            {
                ...
                List<ElementId> supportId = teiketsuList.Count > 0 ? teiketsuList.Select(x => x.Id).ToList() : new List<ElementId>();
                var ms = MaterialSuper.Collect(doc);
                if (ms == null || ms.Count() < 1) { return 0; }
                List<MaterialSuper> materilas = ms.ToList();
                //柱,支柱
                List<ElementId> pillers = materilas.Select(x => x as PilePillerSuper)
                              .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId).Select(x => x.m_ElementId).ToList();
                ...
                foreach (FamilyInstance ins in vBrace)
                {
                    ...
                    List<ElementId> pillerIds = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, brace.m_ElementId, serchIds: pillers);
                    ...
                    foreach (ElementId id in pillerIds)
                    {
                        ...
                        if (YMS_gantry.GantryUtil.AreComponentsSameSign(pDirection, targetD) || YMS_gantry.GantryUtil.AreComponentsSameSign(pDirection, targetD.Negate()))
                        {
                            isFlange = true;
                        }
        ...
        private  static int GetHJointAttachCount(Document doc, eAttachSide eSide)
        {
            ...
            try
            {
                ...
                foreach (PilePillerSuper pil in pillers)
                {
                    ...
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, pil.m_ElementId, ignorelist: teikesuList(), serchIds: joints);
                    //柱に接するツナギ材だけを収集
                    ...
                    foreach (ElementId jId in jointIds)
                    {
                        ...
                        //ツナギ材に接する別のツナギ材を取得
                        List<ElementId> jintsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, jId, ignorelist: teikesuList(), serchIds: jointIds);
        ...
        private static List<FamilyInstance> GetAttachSupportList(Document doc, eBoltElement eType)
        {
            ...
            IEnumerable<Element> elems = from elem in (new FilteredElementCollector(doc).OfClass(typeof(FamilyInstance)).ToElements())
            ...
            foreach (Element el in elems)
            {
                ...
                if (eType == eBoltElement.VBrace && type.Equals("") || eType == eBoltElement.VBrace && type.Equals("ブレス"))
                {
        ...
        private static int GetWebSupprtItemCnt(Document doc)
        {
            ...
            try
            {
                ...
                foreach (PilePillerSuper p in pillers)
                {
                    ...
                    //柱に接する取付補助材
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, p.m_ElementId,  serchIds: supportId);
                    //取付補助材がツナギ用の物かどうか判定する
                    foreach (ElementId sId in intsId)
                    {
                        //接するツナギがあるかどうか判定
                        List<ElementId> jintsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, sId, ignorelist: teikesuList(), serchIds:joints);
                        if(jintsId.Count<=0)
                        {
                            continue;
                        }
        ...
        private static int GetKiribariUkeAttachCnt(Document doc, eAttachSide eSide)
        {
            ...
            foreach (PilePillerSuper pil in pillers)
            {
                FamilyInstance pillerIns = doc.GetElement(pil.m_ElementId) as FamilyInstance;
                //柱に接するファミリのリスト(締結補助材は除く)
                List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, pil.m_ElementId, ignorelist: teikesuList(), serchIds: kiribari);
                //柱に接する切梁受け材だけを収集
                ...
                foreach (ElementId jId in kiribariIds)
                {
                    ...
                    XYZ jointVec = (jC.GetEndPoint(1) - jC.GetEndPoint(0)).Normalize();
                    //切梁受に接する別の切梁受を取得
                    List<ElementId> jintsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(doc, jId, ignorelist: teikesuList(), serchIds: kiribariIds);
        ...
        public static List<FamilyInstance> GetFamilyInstanceListBy(Document doc, eElementType eType, string koudaiName = "")
        {
            ...
            try
            {
                ...
                switch (eType)
                {
                    ...
                    case eElementType.HBrace:
                        List<HorizontalBrace> lstHBrace = targetMsList.Where(x => x.GetType() == typeof(HorizontalBrace)).Select(x => x as HorizontalBrace)?.ToList() ?? null;
                    ...
                    case eElementType.VBrace:
                        List<VerticalBrace> lstVBrace = targetMsList.Where(x => x.GetType() == typeof(VerticalBrace)).Select(x => x as VerticalBrace)?.ToList() ?? null;
                    ...
                    case eElementType.HJoint:
                        List<HorizontalJoint> lstHJoint = targetMsList.Where(x => x.GetType() == typeof(HorizontalJoint)).Select(x => x as HorizontalJoint)?.ToList() ?? null;
