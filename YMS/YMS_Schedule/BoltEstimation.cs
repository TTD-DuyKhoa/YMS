...
namespace YMS_Schedule
{
  ...
  public class RuleEstimation
  {
    ...
    //山留の部材+主材系のボルトデータ
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> yamadomeBoltSheetDatalist =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;

    //山留の部材+主材系のボルトデータ（ElementID付き）
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> yamadomeBoltSheetDatalistWithID =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;

    //主材+主材系のボルトデータ
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> shuzaiBoltSheetDatalist =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;

    //主材+主材系のボルトデータ（ElementID付き）
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> shuzaiBoltSheetDatalistWithID =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;

    //構台系のボルトデータ
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> koudaiBoltSheetDatalist =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;

    //構台系のボルトデータ（ElemetnID付き）
    private List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet> koudaiBoltSheetDatalistWithID =
      new List<YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet>() ;
    ...
    public bool BoltEstimationMain( UIDocument uidoc, List<ElementId> targetBuzaiIds, ClsSekisanSetting sekisansetting )
    {
      ...
      try {
        ...
        if ( sekisansetting.KoudaiBoltSekisan ) {
          YKBolt.KoudaiBoltSekisan( doc, sekisansetting, ref koudaiBoltSheetDatalist,
            ref koudaiBoltSheetDatalistWithID ) ;
        }

        //主材+主材のボルト積算
        if ( sekisansetting.YamdomeBoltSekisan ) {
          YKBolt.ShuzaiAndShuzaiBoltData( doc, sekisansetting, idShuzaiList, ref shuzaiBoltSheetDatalist,
            ref shuzaiBoltSheetDatalistWithID ) ;
        }

        //部材+主材のボルト積算
        if ( sekisansetting.YamdomeBoltSekisan ) {
          YKBolt.buzaiAndShuzaiBoltData( doc, idBuzaiList, idShuzaiList, ref yamadomeBoltSheetDatalist,
          ...
          if ( shuzaiInst.Symbol.FamilyName.Contains( "仮鋼材" ) ) {
            ...
            if ( shuzaiInst.Symbol.FamilyName.Contains( "高強度腹起_80SMH" ) ) {
              ...
              if ( typeName == "腹起" || typeName == "切梁" || typeName == "隅火打" || typeName == "切梁火打" ) {
                ...
                decimal length =
                  (decimal) ClsRevitUtil.CovertFromAPI( tmpC.Curve.GetEndPoint( 0 )
                    .DistanceTo( tmpC.Curve.GetEndPoint( 1 ) ) ) ;
        ...
        foreach ( FamilyInstance shuzaiInst in instShuzaiList ) {
          if ( IsKoudaiKarikouzai( shuzaiInst ) ) {
            ...
            if ( typeName.Contains( "支柱" ) || typeName.Contains( "方杖" ) ) {
              ...
              decimal length =
                (decimal) ClsRevitUtil.CovertFromAPI( c.GetEndPoint( 0 ).DistanceTo( c.GetEndPoint( 1 ) ) ) ;
    ...
    public string GetYamadomeBoltData()
    {
      ...
      foreach ( YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet st in yamadomeBoltSheetDatalist ) {
        str += st.hinshu + "," + st.buzaiName + "," + st.DanName + "," + st.Kigou + "," + st.Kashosu + "," +
               st.boltName + "," + st.num + "," + ( st.Kashosu * st.num ).ToString() + Environment.NewLine ;
      }
    ...
    public string GetYamadomeBoltDataWithElementID()
    {
      ...
      foreach ( YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet st in yamadomeBoltSheetDatalistWithID ) {
        ...
        str += st.buzaiId.ToString() + "," + st.hinshu + "," + st.buzaiName + "," + st.DanName + "," + st.Kigou + "," +
               st.Kashosu + "," + st.boltName + "," + st.num + "," + ( st.Kashosu * st.num ).ToString() + "," + tmp +
               Environment.NewLine ;
      }
    ...
    public string GetKoudaiBoltData()
    {
      ...
      foreach ( YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet st in koudaiBoltSheetDatalist ) {
        str += st.hinshu + "," + st.Kigou + "," + st.Kashosu + "," + st.boltName + "," + st.num + "," +
               ( st.Kashosu * st.num ).ToString() + Environment.NewLine ;
      }
    ...
    public string GetKoudaiBoltDataWithElementID()
    {
      ...
      foreach ( YamadomeKoudaiBoltSekisan.stBoltDataForBoltSheet st in koudaiBoltSheetDatalistWithID ) {
        ...
        str += st.buzaiId.ToString() + "," + st.hinshu + "," + st.Kigou + "," + st.Kashosu + "," + st.boltName + "," +
               st.num + "," + ( st.Kashosu * st.num ).ToString() + "," + tmp + Environment.NewLine ;
      }
    ...
    public string GetKarikouzaiToShuzaiData()
    {
      ...
      foreach ( stShuzaiData st in shuzaiDataList ) {
        ...
        str += st.id.ToString() + "," + st.DanName + "," + st.buzaiType + "," + st.buzaiSize + "," + tmp +
               Environment.NewLine ;
      }
    ...
    public string GetCoverPlateData()
    {
      ...
      foreach ( stCoverPlate st in coverPlateDataList ) {
        str += st.karikouzaiId.ToString() + "," + st.DanName + "," + st.setugouBuzai + "," + st.buzaiSize + "," +
               st.num.ToString() + Environment.NewLine ;
      }
    ...
    public string GetZaganeData()
    {
      ...
      foreach ( stZaganeData st in zaganeDataList ) {
        str += st.blockId.ToString() + "," + st.DanName + "," + st.target + "," + st.num.ToString() +
               Environment.NewLine ;
      }
    ...
    public string GetBracketData()
    {
      ...
      foreach ( stBracketData st in bracketDataList ) {
        str += st.karikouzaiId.ToString() + "," + st.DanName + "," + st.youto + "," + st.name + "," +
               st.num.ToString() + Environment.NewLine ;
      }
    ...
    public string GetKaouhinYamadomeData()
    {
      ...
      foreach ( stkakouhinSheetYamadomeData st in DupLst ) {
        str += st.oyadata.name + "," + st.oyadata.type + "," + st.oyadata.size + "," + st.oyadata.length.ToString() +
               "," + st.oyadata.num.ToString() + "," + st.oyadata.koukei + "," + st.oyadata.kouakeKosuu + "," +
               st.oyadata.unitWeight + "," + st.oyadata.weight + "," + "" + Environment.NewLine ;

        foreach ( stKakouhinData koSt in st.koDataList ) {
          str += koSt.name + "," + koSt.type + "," + koSt.size + "," + koSt.length.ToString() + "," +
                 koSt.num.ToString() + "," + koSt.koukei + "," + koSt.kouakeKosuu + "," + koSt.unitWeight + "," +
                 koSt.weight + "," + "" + Environment.NewLine ;
        }
    ...
    public string GetKaouhinKoudaiData()
    {
      ...
      foreach ( stkakouhinSheetKoudaiData st in DupList ) {
        str += st.oyadata.name + "," + st.oyadata.type + "," + st.oyadata.size + "," + st.oyadata.length.ToString() +
               "," + st.oyadata.num.ToString() + "," + st.oyadata.koukei + "," + st.oyadata.kouakeKosuu + "," +
               st.oyadata.unitWeight.ToString() + "," + st.oyadata.weight + "," + "" + Environment.NewLine ;

        foreach ( stKakouhinData koSt in st.koDataList ) {
          str += koSt.name + "," + koSt.type + "," + koSt.size + "," + koSt.length.ToString() + "," +
                 koSt.num.ToString() + "," + koSt.koukei + "," + koSt.kouakeKosuu + "," + koSt.unitWeight.ToString() +
                 "," + koSt.weight + "," + "" + Environment.NewLine ;
        }
    ...
    public string GetKabekuiSheetData( Document doc )
    {
      ...
      string str =
        "分類,用途,鋼材タイプ,サイズ,CASE名,長さ,ピッチ,埋め殺し長さ,ジョイント数,数量,単位質量,ソイル径,壁長,壁延長,継手仕様,プレート1(F),枚数１,プレート2(F),枚数２,ボルトタイプ(F),ボルト本数(F),プレート(W),枚数,ボルトタイプ(W),ボルト本数(W),ボルト孔径,備考欄" +
        Environment.NewLine ;
        ...
        foreach ( var kabeKuiId in kuiIds ) {
          ...
          foreach ( var haraokoshiBaseId in haraokoshiBaseIds ) {
            ...
            SetComparisonResult result =
              cvHaraokoshiBase.Intersect( cvOyakui, out IntersectionResultArray intersectionResults ) ;
    ...
    public static bool GetCsvLineKabe( Document doc, out List<string> kabeList )
    {
      ...
      try {
        ...
        foreach ( ElementId id in allSoilList ) {
          ...
          string caseEdaban = ClsRevitUtil.GetTypeParameterString( sym, "CASE" ) + "-" +
                              ClsRevitUtil.GetTypeParameterString( sym, "枝番" ) ;
        ...
        foreach ( ElementId id in allKabeList ) {
          ...
          if ( kabeData.Usage != ClsKouyaita.KOUYAITA ) {
            ...
            kabeData.Size =
              YMS.Master.ClsHBeamCsv
                .GetSizePileFamilyPathInName( sym.FamilyName ) ; //ClsRevitUtil.GetTypeParameterString(sym, "サイズ");
          }
          else {
            kabeData.SteelType = ClsRevitUtil.GetTypeParameterString( sym, "小分類" ) ;
            kabeData.Size =
              YMS.Master.ClsKouyaitaCsv
                .GetSizePileFamilyPathInName( sym.FamilyName ) ; //ClsRevitUtil.GetTypeParameterString(sym, "サイズ");
          }

          kabeData.CaseName = ClsRevitUtil.GetTypeParameterString( sym, "CASE" ) + "-" +
                              ClsRevitUtil.GetTypeParameterString( sym, "枝番" ) ;


          // すでに同じ CaseName と Usage の組み合わせが存在する場合、Quantity を加算する
          var existingData = kabeDataList.FirstOrDefault( data =>
            data.CaseName == kabeData.CaseName && data.Usage == kabeData.Usage &&
          ...
          if ( kabeData.Usage != ClsKouyaita.KOUYAITA ) {
            YMS.Master.ClsPileTsugiteCsv pileCsv =
              YMS.Master.ClsPileTsugiteCsv.GetCls( kabeData.JointSpecification, kabeData.Size ) ;
            ...
            YMS.Master.ClsKouyaitaTsugiteCsv pileCsv =
              YMS.Master.ClsKouyaitaTsugiteCsv.GetCls( kabeData.JointSpecification, kabeData.Size ) ;
        ...
        foreach ( ElementId id in allDesumibuList ) {
          ...
          var existingData = kabeDataList.FirstOrDefault( data =>
            data.Length == kabeData.Length && data.Usage == kabeData.Usage && data.SteelType == kabeData.SteelType &&
            data.Size == kabeData.Size ) ;
    ...
    public static bool GetCsvLineKui( Document doc, out List<string> kuiList )
    {
      ...
      try {
        ...
        foreach ( var tanakuiId in tanakuiIds ) {
          ...
          tanakuiData.Classification =
            System.Text.RegularExpressions.Regex.Replace( instTanakui.Symbol.Name, @"_\d+$", "" ) ;
          ...
          tanakuiData.Size =
            YMS.Master.ClsHBeamCsv.GetSizePileFamilyPathInName( instTanakui.Symbol
              .FamilyName ) ; //ClsRevitUtil.GetTypeParameterString(instTanakui.Symbol, "サイズ");
          ...
          tanakuiData.WallLength =
            ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetTypeParameter( instTanakui.Symbol, "長さ" ) ) ;
          ;
          ...
          if ( tanakuiData.JointSpecification == null ) {
            string kotei =
              ClsRevitUtil.GetTypeParameterAsValueString( instTanakui.Symbol, DefineUtil.PARAM_JOINT_TYPE ) ;
            tanakuiData.JointSpecification = kotei != "" ? kotei.Contains( "ﾎﾞﾙﾄ" ) ? "ボルト" : "溶接" : "" ;
          }

          YMS.Master.ClsPileTsugiteCsv pileCsv =
            YMS.Master.ClsPileTsugiteCsv.GetCls( tanakuiData.JointSpecification, tanakuiData.Size ) ;
          ...
          var existingData = tanakuiDataList.FirstOrDefault( data =>
            data.CaseName == tanakuiData.CaseName && data.Size == tanakuiData.Size &&
            data.SteelType == tanakuiData.SteelType ) ;
          if ( existingData.CaseName != null ) {
    ...
    public static bool GetCsvLineKoudaiKui( Document doc, out List<string> kuiList )
    {
      ...
      try {
        ...
        foreach ( var kuiId in koudaiKuiList ) {
          ...
          // すでに同じタイプ名とサイズ、長さの組み合わせが存在する場合、Quantity を加算する
          var existingData = koudaikuiDataList.FirstOrDefault( data =>
            data.Classification == tanakuiData.Classification && data.Size == tanakuiData.Size &&
            data.Length == tanakuiData.Length && data.JointCount == tanakuiData.JointCount ) ;
          if ( existingData.CaseName != null ) {
            int num = koudaikuiDataList.IndexOf( existingData ) ;
            existingData.Quantity += 1 ;
            koudaikuiDataList[ num ] = existingData ;
    ...
    private bool IsKoudaiKarikouzai( FamilyInstance ins )
    {
      ...
      try {
        //桁材の仮鋼材
        if ( ins.Symbol.FamilyName.Contains( "桁材" ) &&
             ( ins.Symbol.FamilyName.Contains( "HA" ) || ins.Symbol.FamilyName.Contains( "SMH" ) ) ) {
          retB = true ;
        }
        //柱の仮鋼材
        else if ( ins.Symbol.FamilyName.Contains( "柱" ) &&
                  ( ins.Symbol.FamilyName.Contains( "HA" ) || ins.Symbol.FamilyName.Contains( "SMH" ) ) ) {
    ...
    /// <returns></returns>
    public static List<stkakouhinSheetYamadomeData> DupStkakouhinSheetYamadomeData(
      List<stkakouhinSheetYamadomeData> lstSt )
    {
      ...
      foreach ( stkakouhinSheetYamadomeData st in lstSt ) {
        ...
        for ( int i = 0 ; i < result.Count ; i++ ) {
          ...
          if ( EqualStruct( st.oyadata, existing.oyadata ) && EqualStructList( st.koDataList, existing.koDataList ) ) {
            existing.num++ ;
            result[ i ] = existing ;
            foundDuplicate = true ;
            break ;
