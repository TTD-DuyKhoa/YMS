...
namespace YMS
{
  public class ClsYMSUtil
  {
    ...
    /// <returns></returns>
    public static double GetKouzaiHabaFromBase( Document doc, ElementId idPartner, bool bDoubleUPorKiri = false,
      bool bDoubleUNorHara = false )
    {
    ...
    /// <param name="idPartner">腹起 切梁 切梁火打 隅火打 に対応</param>
    /// <returns></returns>
    public static string GetKouzaiSizeFromBase( Document doc, ElementId idPartner, bool bDoubleUPorKiri = false,
      bool bDoubleUNorHara = false )
    {
      string strKouzaiType = "" ;
    ...
    {
      string strKotei = ( kotei == Master.Kotei.Bolt
      ...
      foreach ( var id in haraokoshiBaseIds ) {
        ...
        if ( ClsRevitUtil.IsPointOnCurve( cvHaraokosiBase, baseSp ) ) {
          try {
            ...
            if ( syuzaiSize != "" ) {
              kiribariSize = syuzaiSize ;
            }

            ;
            if ( kiribariSize == "60SMH" ) {
              kiribariSize = "40HA" ;
            }
    ...
    public static bool GetDifferenceWithBase( Document doc, ElementId baseId, out double diff, out double diff2 )
    {
      ...
      if ( inst.Name != ClsKiribariBase.baseName && inst.Name != ClsCornerHiuchiBase.baseName &&
           inst.Name != ClsKiribariHiuchiBase.baseName )
        return false ;
    ...
    public static bool GetDifferenceWithBaseDouble( Document doc, ElementId baseId, out double diff1, out double diff2 )
    {
      ...
      if ( baseName != ClsKiribariHiuchiBase.baseName && baseName != ClsCornerHiuchiBase.baseName &&
           baseName != ClsKiribariTsugiBase.baseName )
        return false ;
    ...
    /// <returns></returns>
    public static List<List<XYZ>> GetNonObjectBaseLinePoints( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint,
      bool Nop = true )
    {
      List<List<XYZ>> res = new List<List<XYZ>>() ;
      try {
    ...
    /// <param name="Nop">特定のファミリを検索対象から外す　（杭　ブルマン　リキマン）</param>
    /// <returns></returns>
    public static bool GetNonObjectFlagBaseLinePoints( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, double size,
      bool reverse = false, bool Nop = true )
    {
    ...
    /// <returns></returns>
    public static List<List<XYZ>> GetObjectOnBaseLine( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint,
      ref List<ElementId> hitIdList )
    {
      List<List<XYZ>> res = new List<List<XYZ>>() ;
      try {
        View3D view3D = Get3DView( doc ) ;
        ...
        //図面上のジャッキカバーを全て取得
        List<ElementId> jackCoverList =
          ClsRevitUtil.GetSelectCreatedFamilyInstanceFamilySymbolList( doc, "ジャッキカバー", true ) ;
        ...
        foreach ( ElementId el in lstElementId ) {
          ...
          if ( kensakuIds.Count > 0 ) {
            //ジャッキカバーの仕様が変わったため
            List<ElementId> Ids =
              ClsRevitUtil.GetIntersectFamilysToDependentElements( doc, inst.Id, 0.1, null, kensakuIds ) ;
            if ( Ids.Count > 0 ) {
              foreach ( ElementId getId in Ids ) {
                ...
                if ( instSetuzoku != null ) //ファミリ名でフィルター
                {
                  if ( Master.ClsJackCoverCSV.GetFamilyNameList().ToList().Contains( instSetuzoku.Symbol.FamilyName ) ||
                       Master.ClsCoverPlateCSV.GetFamilyNameList().ToList()
                         .Contains( instSetuzoku.Symbol.FamilyName ) ) {
    ...
    /// <param name="tmpEdPoint">終点(Z軸を上下段でずらす)</param>
    /// <returns></returns>
    public static List<List<XYZ>> GetNonObjectBaseLinePointsFilterObject( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint,
    ...
    /// <returns></returns>
    public static List<ElementId> DeleteObjectBaseLinePoints( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint,
      List<string> filterList )
    {
      List<List<XYZ>> res = new List<List<XYZ>>() ;
      List<ElementId> ids = new List<ElementId>() ;
    ...
    public static List<List<XYZ>> GetNonObjectBaseLinePoints2D( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint,
      bool Nop = true )
    {
      List<List<XYZ>> res = new List<List<XYZ>>() ;
      try {
        ...
        List<KeyValuePair<ElementId, BoundingBoxXYZ>> idBoundingBoxPairList =
          new List<KeyValuePair<ElementId, BoundingBoxXYZ>>() ;
            ...
            if ( boundingBox != null ) {
              ...
              if ( outLine.Intersects( baseOutLine, 0.001 ) ) {
                ...
                if ( element is FamilyInstance ) {
                  ...
                  KeyValuePair<ElementId, BoundingBoxXYZ> idBoundingBoxPair =
                    new KeyValuePair<ElementId, BoundingBoxXYZ>( element.Id, boundingBox ) ;
        ...
        foreach ( KeyValuePair<ElementId, BoundingBoxXYZ> idBoundingBoxPair in idBoundingBoxPairList ) {
          ...
          List<XYZ>
            lstXYZ = GetCurveOnPointList( tmpStPoint, tmpEdPoint,
              checkList ) ; //(from data in refs where data.ElementId == el select data.GlobalPoint).ToList();
          ...
          XYZ max = lstXYZ.OrderByDescending( item => item.DistanceTo( tmpStPoint ) ).First() ;
          XYZ min = lstXYZ.OrderByDescending( item => item.DistanceTo( tmpStPoint ) )
            .Last() ; //lstXYZ.OrderByDescending(item => item.DistanceTo(tmpEdPoint)).First(); //
            ...
            if ( instanceObj is Solid ) {
              ...
              foreach ( Face face in solid.Faces ) {
                if ( face is PlanarFace ) {
                  ...
                  if ( planarFace != null ) {
                    referencesPoint.Add( planarFace
                      .Origin ) ; //.Evaluate(new UV(0.5, 0.5)));planarFace.Originとnew UV(0, 0)は取得位置が一緒
    ...
    public static List<XYZ> GetCurveOnPointList( XYZ tmpStPoint, XYZ tmpEdPoint, List<XYZ> checkList )
    {
      ...
      //Z軸の判定は後ほど行うため0に合わせる
      Curve
        cv = Line.CreateBound( tmpStPoint,
          tmpEdPoint ) ; //new XYZ(tmpStPoint.X, tmpStPoint.Y, 0), new XYZ(tmpEdPoint.X, tmpEdPoint.Y, 0));
    ...
    public static bool ChangeView( UIDocument uidoc, string name, ViewType viewType = ViewType.Elevation )
    {
      ...
      var collector = new FilteredElementCollector( doc ).OfClass( typeof( View ) ).Cast<View>()
    ...
    {
      //既存の場合は作成しない
      if ( new FilteredElementCollector( doc ).OfClass( typeof( ViewSection ) ).Cast<ViewSection>()
            .FirstOrDefault( x => x.Name == viewName ) != null )
        return ;

      //図面上の平面図を取得※どれでもよい
      var viewPlan = new FilteredElementCollector( doc ).OfClass( typeof( View ) ).Cast<View>()
      ...
      //図面上の立面図を取得※どれでもよい
      var viewSection = new FilteredElementCollector( doc ).OfClass( typeof( ViewSection ) ).Cast<ViewSection>()
      ...
      using ( Transaction t = new Transaction( doc, "Create New Elevation View" ) ) {
        ...
        ViewSection
          newElevationView = marker.CreateElevation( doc, viewPlan.Id, 0 ) ; //作った立面図の初期データを図面に既存である立面図に合わせる処理が必要
        newElevationView.Name = viewName ;
        //図面上にある立面図と同じ表示範囲に設定する
        newElevationView.CropBox = viewSection.CropBox ;
        string parm = "前方クリップ オフセット" ;
        ClsRevitUtil.SetParameter( doc, newElevationView.Id, parm,
          ClsRevitUtil.GetParameterDouble( doc, viewSection.Id, parm ) ) ;
    ...
    /// <returns></returns>
    public static double GetTotalLength( double dTotalLength, string steelSize, string tanbuParts1, string tanbuParts2,
      string tanbuSize1, string tanbusize2, double angle, double dHaraAngle, ref double dOffsetA, ref double dOffsetB,
      ref double dOffsetC )
    {
      double d = 0.0 ;
      double d2 = 0.0 ;
    ...
    /// <param name="dOffsetC">鋼材終点から終点端部部品まで</param>
    /// <returns></returns>
    public static double GetTotalLength( double dTotalLength, ClsTanbuParts clsTanbuParts, double angle,
      out double dOffsetA, out double dOffsetB )
    {
      double dAngle = angle ;
    ...
    /// <returns>補助線</returns>
    public static ModelLine CreateHojyoLine( Document doc, XYZ pointS, XYZ pointE, double dH1, double dH2, double dEx1,
      double dEx2, bool binout = false )
    {
      XYZ Direction = Line.CreateBound( pointS, pointE ).Direction ;
      //補助線の作成
      XYZ hojyoS = new XYZ( pointS.X + ( dH1 * Direction.Y ) - ( dEx1 * Direction.X ),
        pointS.Y - ( dH1 * Direction.X ) - ( dEx1 * Direction.Y ), pointS.Z ) ;
      XYZ hojyoE = new XYZ( pointE.X + ( dH2 * Direction.Y ) + ( dEx2 * Direction.X ),
        pointE.Y - ( dH2 * Direction.X ) + ( dEx2 * Direction.Y ), pointE.Z ) ;
      Line hojyo = Line.CreateBound( hojyoS, hojyoE ) ;
    ...
    public static Line CreateLine( Document doc, XYZ pointS, XYZ pointE, double dH1, double dH2, double dEx1,
      double dEx2, bool binout = false )
    {
      XYZ Direction = Line.CreateBound( pointS, pointE ).Direction ;
      //補助線の作成
    ...
    public static bool FindConnectedPoints( XYZ startPoint, XYZ endPoint, List<List<XYZ>> linePoints,
      ref List<List<XYZ>> resultList )
    {
    ...
    /// <param name="resultList">見つかった線分の始終点をこの中に入れて返す</param>
    /// <returns>複数ある場合は終点に近い方を採用</returns>
    public static bool FindConnectedPoints( XYZ startPoint, XYZ endPoint, bool bStartSide,
      ref List<List<XYZ>> linePoints, ref List<List<XYZ>> resultList )
    {
      try {
        Curve cvTarget = Line.CreateBound( startPoint, endPoint ) ;
    ...
    public static List<ElementId> GetIntersectionBase( Document doc, ElementId id )
    {
      ...
      List<ElementId>
        targetFamilies =
          new List<ElementId>() ; // ClsRevitUtil.GetSelectLevelCreatedFamilyInstanceList(doc, "ベース", levelId, true);
      //腹起と切梁のみレベルを指定して全て取得
    ...
    public static List<ElementId> GetIntersectionKiribariBase( Document doc, ElementId id )
    {
      ...
      List<ElementId> targetFamilies =
        ClsRevitUtil.GetSelectLevelCreatedFamilyInstanceList( doc, ClsKiribariBase.baseName, levelId ) ;
    ...
    public static List<ElementId> GetIntersectionHiuchiBase( Document doc, ElementId id )
    {
      ...
      List<ElementId> targetFamilies =
        ClsRevitUtil.GetSelectLevelCreatedFamilyInstanceList( doc, "火打ベース", levelId, true ) ;
    ...
    {
      FilteredElementCollector collector = new FilteredElementCollector( doc ) ;
      var existingFamilies = collector.OfClass( typeof( Family ) ).Cast<Family>()
    ...
    public static void ActivateSymbolInFamily( Document doc, string familyName, string symbolName )
    {
      // ファミリを取得
      FilteredElementCollector collector = new FilteredElementCollector( doc ) ;
      var families = collector.OfClass( typeof( Family ) ).Cast<Family>().Where( family => family.Name == familyName ) ;

      foreach ( var family in families ) {
        // ファミリ内のシンボルを取得
        var familySymbols = new FilteredElementCollector( doc ).OfCategory( BuiltInCategory.OST_GenericModel )
    ...
    public static XYZ[] CalcShabariSection( Line shaBaseLine, Plane shaBasePlane, double shaWidthMm, double shaHeightMm,
      Plane sectionPlane )
    {
      // 20250217時点 動作未確認
      ...
      return Enumerable.Empty<XYZ>().Append( dirH + dirW ).Append( dirH - dirW ).Append( -dirH - dirW )
        .Append( -dirH + dirW ).Select( x => shaBaseLine.Origin + x )
        .Select( x => Line.CreateUnbound( x, shaBaseLine.Direction ) )
        .Select( x => SLibRevitReo.IntersectPlaneAndLine( sectionPlane, x ) ).Where( x => x != null ).ToArray() ;
    }
    ...
    /// <returns></returns>
    public static XYZ CalcShabariSectionTop( Line shaBaseLine, Plane shaBasePlane, double shaWidthMm,
      double shaHeightMm, Plane sectionPlane, bool bFirst = false )
    {
    ...
    /// <returns></returns>
    public static XYZ CalcShabariSectionBottom( Line shaBaseLine, Plane shaBasePlane, double shaWidthMm,
      double shaHeightMm, Plane sectionPlane, double ukeWidthMn )
    {
      var ukeWhalf = ClsRevitUtil.CovertToAPI( 0.5 * ukeWidthMn ) ;
      var sectionPlanes = Enumerable.Empty<double>().Append( ukeWhalf ).Append( -ukeWhalf ).Select( x =>
        Plane.CreateByNormalAndOrigin( sectionPlane.Normal,
          sectionPlane.Origin + x * sectionPlane.Normal.Normalize() ) ) ;
      var sectionPointSet = sectionPlanes
        .Select( x => CalcShabariSection( shaBaseLine, shaBasePlane, shaWidthMm, shaHeightMm, x ) ).ToArray() ;
      //var sectionPoints = CalcShabariSection(shaBaseLine, shaBasePlane, shaWidthMm, shaHeightMm, sectionPlane);
