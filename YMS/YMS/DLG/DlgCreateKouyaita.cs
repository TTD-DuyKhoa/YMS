...
namespace YMS.DLG
{
    public partial class DlgCreateKouyaita : Form
    {
        ...
        public void initOtherControl()
        {
            ...
            Master.ClsKouyaitaTsugiteCsv cornerPileCsv = Master.ClsKouyaitaTsugiteCsv.GetCls(strKotei, cmbCornerYaitaSize.Text);
            ...
            Master.ClsKouyaitaTsugiteCsv cornerPileCsv2 = Master.ClsKouyaitaTsugiteCsv.GetCls(strKotei2, cmbCornerYaitaSize.Text);
        ...
        public bool CheckInput()
        {
            ...
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountFlangeSide, "ボルト本数(1)（フランジ側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountFlangeSideB, "ボルト本数B(1)（フランジ側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountFlangeSide2, "ボルト本数(2)（フランジ側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountFlangeSideB2, "ボルト本数B(2)（フランジ側）", bMinus: true, bComma: true))
                return false;

            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountWebSide, "ボルト本数(1)（Web側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountWebSideB, "ボルト本数B(1)（Web側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountWebSide2, "ボルト本数(2)（Web側）", bMinus: true, bComma: true))
                return false;
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountWebSideB2, "ボルト本数B(2)（Web側）", bMinus: true, bComma: true))
                return false;
        ...
        private void txtEdaNumber_TextChanged(object sender, EventArgs e)
        {
            ...
            enCASE_EdabanHit enCASE_EdabanHit = clsCASE.ExistCaseEdaban(enKabeshu.koyauita, txtCase.Text, txtEdaNumber.Text, ref nextCASE, ref nextEdaban);
        ...
        private void SetNextEdaNum()
        {
            if (chkKougoHaichi.Checked && !string.IsNullOrWhiteSpace(txtEdaNumber.Text))
            {
                ...
                string nextCASE = string.Empty;
                string nextEdaban = string.Empty;
                enCASE_EdabanHit enCASE_EdabanHit = clsCASE.ExistCaseEdaban(enKabeshu.koyauita, txtCase.Text, edaban, ref nextCASE, ref nextEdaban);
                bool use = true;
                if (enCASE_EdabanHit == enCASE_EdabanHit.sameKabeshu)
                {
                    foreach (stCASEKoyaita koyauita in clsCASE.m_koyaitaDataList)
                        ...
                        if (koyauita.CASE == txtCase.Text && koyauita.Edaban == edaban)
                        {
                            if (txtKashoNum.Text == koyauita.JointNum.ToString())
                            {
                                if (dgvPileLength.Columns.Count == koyauita.KuiLengthList.Count && 0 < dgvPileLength.Rows.Count)
                                {
                                    for (int i = 0; i < dgvPileLength.Columns.Count; i++)
                                    {
                                        if ((int)dgvPileLength[i, 0].Value != koyauita.KuiLengthList[koyauita.KuiLengthList.Count - 1 - i])
                                        {
                                            use = false;
                                            break;
                                        }
