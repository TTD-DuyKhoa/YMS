    ...
    public partial class DlgCreateTanakui : System.Windows.Forms.Form
    {
        ...
        private void GetIniData()
        {
            try
            {
                ...
                rbnCreatePossitionLeftTop.Checked = bool.Parse(ClsIni.GetIniFile(sec, rbnCreatePossitionLeftTop.Name, path));
                rbnCreatePossitionLeftBottom.Checked = bool.Parse(ClsIni.GetIniFile(sec, rbnCreatePossitionLeftBottom.Name, path));
                rbnCreatePossitionRightTop.Checked = bool.Parse(ClsIni.GetIniFile(sec, rbnCreatePossitionRightTop.Name, path));
                rbnCreatePossitionRightBottom.Checked = bool.Parse(ClsIni.GetIniFile(sec, rbnCreatePossitionRightBottom.Name, path));
        ...
        private void SetIniData()
        {
            ...
            ClsIni.WritePrivateProfileString(sec, rbnCreatePossitionLeftTop.Name, rbnCreatePossitionLeftTop.Checked.ToString(), path);
            ClsIni.WritePrivateProfileString(sec, rbnCreatePossitionLeftBottom.Name, rbnCreatePossitionLeftBottom.Checked.ToString(), path);
            ClsIni.WritePrivateProfileString(sec, rbnCreatePossitionRightTop.Name, rbnCreatePossitionRightTop.Checked.ToString(), path);
            ClsIni.WritePrivateProfileString(sec, rbnCreatePossitionRightBottom.Name, rbnCreatePossitionRightBottom.Checked.ToString(), path);
        ...
        private bool CheckInput()
        {
            ...
            if (!ClsCommonUtils.ChkContorolTextDouble(txtBoltCountFlangeSide, "ボルト本数(フランジ側)", bMinus: true, bComma: true))
            {
                return false;
            }
        ...
        private bool SetData()
        {
            ...
            if (rbnFromToP.Checked == true)
            {
                if (txtFromToP.Text == null) { return false; }
                clsTanaKui.m_HightFromGL = double.Parse(txtFromToP.Text);
            }

            // 指定したグループ内でチェックされているラジオボタンを取得する
            var RadioButtonChecked_InGroup = grpCreatePosition.Controls.OfType<RadioButton>().SingleOrDefault(rb => rb.Checked == true);
            ...
            if (cmbShoriType.SelectedItem == null) { return false; }
            ...
            if (cmbKiribariOsaeBracketSize.SelectedItem == null) { return false; }
            ...
            if (txtJointCount.Text == null) { return false; }
            clsTanaKui.m_TsugiteCount = ClsCommonUtils.ChangeStrToInt(txtJointCount.Text);
            clsTanaKui.m_FixingType = (ClsTanakui.FixingType)(rbnKoteiTypeBolt.Checked ? FixingType.Bolt : FixingType.Welding);

            List<int> lstN1 = new List<int>();
            for (int i = 0; i < dgvPileLength.Columns.Count; i++)
            {
                if (dgvPileLength[i, 0].Value == null) { return false; }
                lstN1.Add(ClsCommonUtils.ChangeStrToInt(dgvPileLength[i, 0].Value.ToString()));
            }
