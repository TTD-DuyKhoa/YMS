...
namespace YMS.DLG
{
    public partial class DlgCreateCornerHiuchiBase : System.Windows.Forms.Form
    {
        ...
        {
            // 鋼材タイプの処理
            InitComboBoxHelper(cmbSteelType, () => Master.ClsYamadomeCsv.GetTypeList(bHigh: true, bMega: false, bTwin: false));
        ...
        private void SetIniData()
        {
            ...
            if (cornerHiuchi.m_ShoriType != ClsCornerHiuchiBase.ShoriType.Change)
            {
                ...
                ClsIni.WritePrivateProfileString(sec, rbnShoriHohoManual.Name, rbnShoriHohoManual.Checked.ToString(), iniPath);
                ClsIni.WritePrivateProfileString(sec, rbnShoriHohoReplace.Name, rbnShoriHohoReplace.Checked.ToString(), iniPath);
            }
            ...
            ClsIni.WritePrivateProfileString(sec, rbnHiuchiAngleToubun.Name, rbnHiuchiAngleToubun.Checked.ToString(), iniPath);
