...
namespace YMS
{
    ...
    public class RequestHandler : IExternalEventHandler
    {
        ...
        public bool CreateKussakuBoidKabe(UIApplication uiapp)
        {
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                ...
                try
                {
                    ...
                    XYZ p = new XYZ((wallOrigin.X + wallEndPoint.X) / 2.0, (wallOrigin.Y + wallEndPoint.Y) / 2.0, (wallOrigin.Z + wallEndPoint.Z) / 2.0);
        ...
        public bool CreateKussakuBoidSoil(UIApplication uiapp)
        {
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                ...
                try
                {
                    ...
                    XYZ p = new XYZ((soilOrigin.X + soilEndPoint.X) / 2.0,
                                    (soilOrigin.Y + soilEndPoint.Y) / 2.0,
        ...
        /// <returns></returns>
        public static List<ElementId> GetFamilyInstanceList(Document doc, string familyName, string typeName, string categoryName = "", string levelName = "")
        {
        ...
        public static IEnumerable<Element> GetFamilyInstance(Document doc, string familyName, string typeName, string categoryName = "", string levelName = "")
        {
            if (doc == null)
            {
                return null;
        ...
        /// <param name="levelName">レベル名</param>
        /// <returns></returns>
        private static IEnumerable<Element> FilterFamilyInstanceLevelName(Document doc, IEnumerable<Element> elems, string levelName)
        {
            if (doc == null)
            {
                return null;
