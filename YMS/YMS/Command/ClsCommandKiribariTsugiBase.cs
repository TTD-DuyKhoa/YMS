      ...
      for ( ; ; ) {
        ...
        try {
          if ( clsKT.m_ShoriType == ClsKiribariTsugiBase.ShoriType.Point ) {
            ...
            if ( ! ClsRevitUtil.PickObjectFilters( uidoc, "腹起ベースor切梁ベースを指定してください", ClsGlobal.m_baseKiriOrHaraList,
                  ref idPartner ) ) {
              return ;
            }
            ...
            if ( d < 50.0 ) {
              picPoint = new XYZ( picPoint.X - ( ClsRevitUtil.CovertToAPI( d ) * dirPitch.X ),
                picPoint.Y - ( ClsRevitUtil.CovertToAPI( d ) * dirPitch.Y ), picPoint.Z ) ;
              ...
              picPoint = new XYZ( picPoint.X + ( ClsRevitUtil.CovertToAPI( 100.0 - d ) * dirPitch.X ),
                picPoint.Y + ( ClsRevitUtil.CovertToAPI( 100.0 - d ) * dirPitch.Y ), picPoint.Z ) ;
            ...
            picPoint = new XYZ( picPoint.X + ( ClsRevitUtil.CovertToAPI( 1500 ) * dirSanjiku.X ),
              picPoint.Y + ( ClsRevitUtil.CovertToAPI( 1500 ) * dirSanjiku.Y ), picPoint.Z ) ;
            ...
            XYZ newPoint = new XYZ( picPoint.X + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * dirSanjiku.X ),
              picPoint.Y + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * dirSanjiku.Y ), picPoint.Z ) ;
        ...
        if ( i == 0 ) {
          ...
          templateBase.m_Kousei = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "構成" ) == "シングル"
              ? ClsKiribariTsugiBase.Kousei.Single
              : ClsKiribariTsugiBase.Kousei.Double, templateBase.m_Kousei ) ;
          templateBase.m_SteelSizeSingle =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材サイズ(シングル)" ),
              templateBase.m_SteelSizeSingle ) ;
          templateBase.m_KiriSideSteelSizeDouble = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "切梁側/鋼材サイズ(ダブル)" ), templateBase.m_KiriSideSteelSizeDouble ) ;
          templateBase.m_KiriSideTsunagiLength = ClsRevitUtil.CompareValues(
            (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "切梁側/切梁繋ぎ長さ" ) ),
            templateBase.m_KiriSideTsunagiLength ) ;
          templateBase.m_KiriSideParts = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "切梁側/部品" ),
            templateBase.m_KiriSideParts ) ;
          templateBase.m_HaraSideSteelSizeDouble = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "腹起側/鋼材サイズ(ダブル)" ), templateBase.m_HaraSideSteelSizeDouble ) ;
          templateBase.m_HaraSideTsunagiLength = ClsRevitUtil.CompareValues(
            (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "腹起側/切梁繋ぎ長さ" ) ),
            templateBase.m_HaraSideTsunagiLength ) ;
          templateBase.m_HaraSideParts = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "腹起側/部品" ),
            templateBase.m_HaraSideParts ) ;
        }
      }
