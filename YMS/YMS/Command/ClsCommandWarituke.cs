    ...
    public class ClsCommandWarituke
    {
                        ...
                        if (ClsGeo.GEO_LE(dist, jougenMM))
                        {
                            ...
                            for (int i = 1; i <= nkensakuNum; i++)
                            {
                                ...
                                if (ClsGeo.GEO_EQ(checkSize, dist))
                                {
                                    ...
                                    string path = Master.ClsYamadomeCsv.GetYamadomeFamilyFromKarikouzai(familyName, ClsGeo.mm2m(checkSize));
                                    ...
                                    using (Transaction t = new Transaction(doc, "鋼材作成"))
                                    {
                                        ...
                                        // シンボルを回転
                                        Line rotationAxis = Line.CreateBound(cv.GetEndPoint(0), cv.GetEndPoint(0) + XYZ.BasisZ);    // Z軸周りに回転
        ...
        public static void CommandDeleteWarituke(UIApplication uiapp)
        {
            ...
            try
            {
                ...
                else if (result == TaskDialogResult.No)
                {
                    ...
                    //仮鋼材を削除
                    if (!ClsRevitUtil.PickObjectsSymbolFilters(uidoc, "削除する割付部材を選択してください。", kariKouzaiList, ref selectedIds))
                    {
                        return;
                    }
        ...
        private static void WaritukeMain(UIApplication uiapp, List<ElementId> deleteBuuzaiIdList)
        {
            ...
            if (typeName == "腹起")
            {
                        ...
                        if (lCurveHara != null)
                        {
                            ...
                            if (insec != null)
                            {
                                if (!ClsRevitUtil.CheckNearGetEndPoint(cvHaraBase, insec))
                                {
                                    tmpEdPoint = insec;
                                    eInsec = true;
                                    if (ClsRevitUtil.GetInstMojiParameter(doc, haraId, "横本数") == "ダブル" && chb.m_katimake != ClsHaraokoshiBase.WinLose.Lose)
                                    {
                                        tmpEdPoint = new XYZ(tmpEdPoint.X + (kouzaiSize * DirectionWin.X), tmpEdPoint.Y + (kouzaiSize * DirectionWin.Y), tmpEdPoint.Z);
                                    }
                                }
                                else
                                {
                                    ...
                                    if (ClsRevitUtil.GetInstMojiParameter(doc, haraId, "横本数") == "ダブル" && chb.m_katimake != ClsHaraokoshiBase.WinLose.Lose)
                                    {
                                        tmpStPoint = new XYZ(tmpStPoint.X - (kouzaiSize * DirectionWin.X), tmpStPoint.Y - (kouzaiSize * DirectionWin.Y), tmpStPoint.Z);
                                    }
                                }
                            }
                        }
                ...
                tmpStPoint = new XYZ(tmpStPoint.X + (kouzaiSize / 2 * Direction.Y), tmpStPoint.Y - (kouzaiSize / 2 * Direction.X), tmpStPoint.Z);
                tmpEdPoint = new XYZ(tmpEdPoint.X + (kouzaiSize / 2 * Direction.Y), tmpEdPoint.Y - (kouzaiSize / 2 * Direction.X), tmpEdPoint.Z);

                if ((ClsRevitUtil.GetInstMojiParameter(doc, m_baseId, "横本数") == "ダブル" && !m_isKussaku) || (steelSizeOrign == ("80SMH") && !m_isKussaku))
                {
                    tmpStPoint = new XYZ(tmpStPoint.X + (kouzaiSize * Direction.Y), tmpStPoint.Y - (kouzaiSize * Direction.X), tmpStPoint.Z);
                    tmpEdPoint = new XYZ(tmpEdPoint.X + (kouzaiSize * Direction.Y), tmpEdPoint.Y - (kouzaiSize * Direction.X), tmpEdPoint.Z);
                }
            ...
            else if (typeName == "斜梁繋ぎ材")
            {
                ...
                if (1 < syabariInsecList.Count)
                    (tmpStPoint, tmpEdPoint) = ClsSyabariTsunagizaiBase.GetSyabariTsunagizaiPoint(doc, syabariInsecList[0], syabariInsecList[1], m_baseId);
                m_BaseOffset = (kouzaiSize / 2);
                offset = m_BaseOffset;
                typeName = "斜梁";
            }
            else if(typeName == "斜梁受け材")
            {
                ...
                if (idSyabari != null)
                    (tmpStPoint, tmpEdPoint) = ClsSyabariUkeBase.GetSyabariUkePoint(doc, idSyabari, m_baseId, Master.ClsYamadomeCsv.GetWidth(steelSizeOrign));
            ...
            foreach (List<XYZ> notPojntList in notPojntLists)
            {
                ...
                while (true)
                {
                    ...
                    if (doubleMaxLength != 0.0 && dlgWaritsukeMode.DialogResult == DialogResult.No && doubleMaxLength < Line.CreateBound(m_InsertPoint, m_EndPoint).Length)
                    {
                    ...
                    scaleIds = ClsWarituke.CreateScale(uidoc, new XYZ(m_InsertPoint.X, m_InsertPoint.Y, m_InsertPoint.Z - offset), new XYZ(m_EndPoint.X, m_EndPoint.Y, m_EndPoint.Z - offset), WaritukeDist.warituke100mm, maxLength, minLength, cross);
                    ...
                    try
                    {
                        switch (dlgWaritsuke.m_Command)
                        {
                            case WaritukeCommand.CreateSyuzai:
                                ...
                                if (searchFlag)
                                {
                                    AddCoverPlateWithHiuchiBlockSP(doc, instBase, typeName, kouzaiSize, vecCcoverPlateoffset, steelSize, tmpSp, tmpEp);
                                }
                            ...
                            case WaritukeCommand.CreateJack1:
                                WaritsukeJack(doc, typeName, instBase.Host.Id, baseOriginLength, out buzaiLength, true, dlgWaritsuke.m_UseJackCover);
                                break;
                            case WaritukeCommand.CreateJack2:
                                WaritsukeJack(doc, typeName, instBase.Host.Id, baseOriginLength, out buzaiLength, false, dlgWaritsuke.m_UseJackCover);
                                break;
        ...
        private static void WaritsukeHojoPiece(Document doc, string baseName, string size, ElementId levelId, out double buzaiLength)
        {
            ClsHojoPiece clsHojoPiece = new ClsHojoPiece();
            clsHojoPiece.CreateHojoPiece(doc, m_baseId, m_InsertPoint, m_EndPoint, levelId, size, baseName, out buzaiLength, m_BaseOffset);
            ...
            if (m_isHaraokoshiTateW)
            {
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    t.Start();
                    // 縦ダブルの下段に移動する
                    ClsRevitUtil.SetParameter(doc, clsHojoPiece.m_CreatedId, "基準レベルからの高さ", -(dSize / 2) - (m_VerticalGap / 2));
                    t.Commit();
                }
            }
        ...
        private static void WaritsukeKirikaePiece(Document doc, string baseName, ElementId levelId, bool isReverse, out double buzaiLength)
        {
        ...
        private static void WaritsukeJack(Document doc, string baseName, ElementId levelId, double remainingLength, out double buzaiLength, bool isType1, bool useJackCover)
        {
            ...
            // ジャッキの配置処理
            jack.CreateJack(doc, m_baseId, m_InsertPoint, m_EndPoint, m_InsertPoint, remainingLength: remainingLength, flagName: "割付");
        ...
        private static void WarituskeSyuzai(Document doc, FamilyInstance instBase, FamilySymbol kouzaiSym)
        {
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                ...
                if (instBase.Host is ReferencePlane referencePlane)
                {
                    ...
                    ClsRevitUtil.SetParameter(doc, createdId, "集計レベル", ClsRevitUtil.GetParameterElementId(doc, instBase.Id, "集計レベル"));
                ...
                if (m_isHaraokoshiTateW)
                {
                    ...
                    // 縦ダブルの下段に移動する
                    ClsRevitUtil.SetParameter(doc, createdId, "基準レベルからの高さ",  - (dSize/2) - (m_VerticalGap / 2));//m_BaseOffset縦ダブルのときは上下段オフセットは不要
                }
        ...
        private static void WaritsukeCoverPlate(Document doc, FamilyInstance instBase, XYZ vecCcoverPlateoffset, string steelSize, double kouzaiSize, string typeName)
        {
            if (m_createdIds.Count > 0 && m_createdXYZs.Count > 0)
            {
                ...
                if (inst.Symbol.FamilyName.Contains("主材"))
                {
                    ...
                    // 掘削側のカバープレートを作成
                    clsCoverPlate.CreateCoverPlate(doc, m_InsertPoint, m_EndPoint, instBase, steelSize, typeName, true, vecCcoverPlateoffset, m_BaseOffset);
                    cpKussaku = clsCoverPlate.m_CreatedId;

                    // 壁側のカバープレートを作成
                    clsCoverPlate.CreateCoverPlate(doc, m_InsertPoint, m_EndPoint, instBase, steelSize, typeName, false, vecCcoverPlateoffset, m_BaseOffset);
                    ...
                    if (instBase.Symbol.FamilyName.Contains("腹起"))
                    {
                        ...
                        {
                            DialogResult result = MessageBox.Show("カバープレートが接触していますが、作成しますか？", "確認", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2);
                    ...
                    if (m_isHaraokoshiTateW)
                    {
                        if (cpKussaku != null)// || !deletedFamilyIds.Contains(cpKussaku))
                        {
                            ...
                            {
                                t.Start();
                                ClsRevitUtil.SetParameter(doc, cpKussaku, "基準レベルからの高さ",  - (kouzaiSize / 2) - (m_VerticalGap / 2));//m_BaseOffset
                                t.Commit();
                            }
                            // 配置したファミリをコピーして上段にする
                        ...
                        if (cpKabe != null)// || !deletedFamilyIds.Contains(cpKabe))
                        {
                            ...
                            {
                                t.Start();
                                ClsRevitUtil.SetParameter(doc, cpKabe, "基準レベルからの高さ",  - (kouzaiSize / 2) - (m_VerticalGap / 2));//m_BaseOffset
                                t.Commit();
                            }
                            // 配置したファミリをコピーして上段にする
        ...
        private static void AddCoverPlateWithHiuchiBlockSP(Document doc, FamilyInstance instBase, string typeName, double kouzaiSize, XYZ vecCcoverPlateoffset, string steelSize, XYZ tmpSp, XYZ tmpEp)
        {
        ...
        private static void AddCoverPlateWithHiuchiBlockEP(Document doc, FamilyInstance instBase, string typeName, double kouzaiSize, XYZ vecCcoverPlateoffset, string steelSize)
        {
            if (!HitWithHiuchiBlock(doc, m_createdIds.Last()))
            {
                return;
