      ...
      if ( clsKHB.m_ShoriType == ClsKiribariHiuchiBase.ShoriType.KiriKiri ) {
        for ( ; ; ) {
          try {
            ...
            clsKHB.CreateKiribariHiuchiBase( uidoc, idKiri1, idKiri2, levelID, dan, rfKiri1.GlobalPoint,
              rfKiri2.GlobalPoint ) ;
          }
          catch {
            break ;
      ...
      else //三軸
      {
        for ( ; ; ) {
          try {
            ...
            if ( ! ClsRevitUtil.PickObjectFilters( uidoc, "三軸ピースを選択してください", ClsGlobal.m_sanjikuPeaceList,
                  ref idSanjiku ) ) {
              return ;
            }
            ...
            picPoint = new XYZ( picPoint.X + ( ClsRevitUtil.CovertToAPI( distSanjiku ) * dirSanjiku.X ),
              picPoint.Y + ( ClsRevitUtil.CovertToAPI( distSanjiku ) * dirSanjiku.Y ), picPoint.Z ) ;
            ElementId idKiri = null ;
            ...
            if ( clsKHB.m_CreateType == ClsKiribariHiuchiBase.CreateType.Both ) {
              ...
              XYZ newPoint =
                ClsRevitUtil.GetDistanceNewPoint( insec, dirHara, ClsRevitUtil.CovertFromAPI( shitenkankyori ) ) ;

              clsKHB.CreateKiribariHiuchiBase( doc, picPoint, newPoint, levelID, dan ) ;

              shitenkankyori = crossDist / Math.Sin( Math.PI - Math.PI / 6 - ( Math.PI - angle ) ) *
                               Math.Sin( Math.PI / 6 ) ; //左
              newPoint = ClsRevitUtil.GetDistanceNewPoint( insec, -dirHara,
                ClsRevitUtil.CovertFromAPI( shitenkankyori ) ) ;
            ...
            else //自動不明
            {
              ...
              XYZ newEndPoint = ClsRevitUtil.GetDistanceNewPoint( insec, -dirKiriHiuch,
                ClsRevitUtil.CovertFromAPI( kiriBLine.Length ) ) ;
    ...
    public static bool CommandChangeKiribariHiuchiBase( UIDocument uidoc )
    {
      ...
      for ( int i = 0 ; i < idList.Count() ; i++ ) {
        ...
        if ( i == 0 ) {
          ...
          //元データを取得
          templateBase.m_CreateHoho = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "構成" ) == "シングル"
              ? ClsKiribariHiuchiBase.CreateHoho.Single
              : ClsKiribariHiuchiBase.CreateHoho.Double, templateBase.m_CreateHoho ) ;

          templateBase.m_SteelType = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材タイプ" ),
            templateBase.m_SteelType ) ;
          templateBase.m_SteelSizeSingle =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材サイズ(シングル)" ),
              templateBase.m_SteelSizeSingle ) ;
          templateBase.m_SteelSizeDoubleUpper =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材サイズ(ダブル上)" ),
              templateBase.m_SteelSizeDoubleUpper ) ;
          templateBase.m_SteelSizeDoubleUnder =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材サイズ(ダブル下)" ),
              templateBase.m_SteelSizeDoubleUnder ) ;
          templateBase.m_HiuchiZureRyo = ClsRevitUtil.CompareValues(
            (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "上下火打ずれ量a" ) ),
            templateBase.m_HiuchiZureRyo ) ;

          templateBase.m_PartsTypeKiriSide =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "部品タイプ(切梁側)" ),
              templateBase.m_PartsTypeKiriSide ) ;
          templateBase.m_PartsSizeKiriSide =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "部品サイズ(切梁側)" ),
              templateBase.m_PartsSizeKiriSide ) ;
          templateBase.m_PartsTypeHaraSide =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "部品タイプ(腹起側)" ),
              templateBase.m_PartsTypeHaraSide ) ;
          templateBase.m_PartsSizeHaraSide =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "部品サイズ(腹起側)" ),
              templateBase.m_PartsSizeHaraSide ) ;
        }
      }
