...
namespace YMS.Command
{
  class ClsCommandCopyKiribari
  {
    public static void CommandCopyKiribari( UIDocument uidoc )
    {
      ...
      double kouzaiSize =
        ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( steelSizeOrign.Replace( "HA", "" ) ) * 10 / 2 ) ;
      if ( steelSizeOrign.Contains( "SMH" ) ) {
        if ( steelSizeOrign.Contains( "80SMH" ) ) {
          ...
          kouzaiSize =
            ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( steelSizeOrign.Replace( "SMH", "" ) ) * 10 / 2 ) ;
        }
      }
      ...
      List<List<XYZ>> notPojntLists =
        ClsYMSUtil.GetObjectOnBaseLine( doc, tmpStPoint, tmpEdPoint, ref copyTargetBuzaiIds ) ;
      ...
      for ( int i = 0 ; i < idList.Count ; i++ ) {
        ...
        // コピー前の Level 一覧
        var beforeLevelIds = new FilteredElementCollector( doc ).OfClass( typeof( Level ) ).ToElementIds().ToHashSet() ;
        ...
        var afterLevelIds = new FilteredElementCollector( doc ).OfClass( typeof( Level ) ).ToElementIds().ToHashSet() ;
