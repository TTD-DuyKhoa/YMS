    ...
    public static bool CommandChangeKouzaiLength( UIDocument uidoc )
    {
      ...
      for ( int i = 0 ; i < targetParamsList.Count() ; i++ ) {
        for ( int j = i + 1 ; j < targetParamsList.Count() ; j++ ) {
          if ( ClsRevitUtil.IsConnectStraight( targetParamsList[ i ].startPoint, targetParamsList[ i ].vec,
                targetParamsList[ i ].length, targetParamsList[ j ].startPoint, targetParamsList[ j ].vec,
                targetParamsList[ j ].length ) ) {
      ...
      if ( mode == DLG.DlgChangeLength.EMode.Edit ) {
        ...
        newEndPoint = ClsRevitUtil.MoveCoordinates( newStartPoint, newEndPoint,
          ClsRevitUtil.ConvertDoubleGeo2Revit( newLength ) ) ;
        ...
        newEndPoint2 = ClsRevitUtil.MoveCoordinates( newStartPoint, newEndPoint,
          ClsRevitUtil.ConvertDoubleGeo2Revit( ClsGeo.m2mm( div1 ) ) ) ;
        newEndPoint = ClsRevitUtil.MoveCoordinates( newEndPoint2, newEndPoint,
          ClsRevitUtil.ConvertDoubleGeo2Revit( ClsGeo.m2mm( div2 ) ) ) ;
        ...
        if ( kigouSizeStr.Contains( "80SMH" ) ) {
          ...
          kigouSize = ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( kigouSizeStr.Replace( "SMH", "" ) ) * 10 ) ;
          sizeStr = kigouSizeStr.Replace( "SMH", "" ) ;
        }
        else if ( kigouSizeStr.Contains( "HA" ) ) {
          kigouSize = ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( kigouSizeStr.Replace( "HA", "" ) ) *
                                                10 ) ;
