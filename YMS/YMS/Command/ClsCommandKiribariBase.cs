  ...
  class ClsCommandKiribariBase
  {
    public static bool CreateKiribariBase( UIDocument uidoc )
    {
      ...
      if ( clskiriBase.m_ShoriType == ClsKiribariBase.ShoriType.BaseLine ) {
        //基準線
        if ( ! ClsHaraokoshiBase.PickBaseObject( uidoc,
              ref id ) ) //ClsRevitUtil.PickObject(uidoc, "切梁ベースを配置する段の腹起ベース", "腹起ベース", ref id))
        {
          return false ;
        }
    ...
    public static bool CommandChangeKiribariBase( UIDocument uidoc )
    {
      ...
      for ( int i = 0 ; i < idList.Count() ; i++ ) {
        ...
        if ( i == 0 ) {
          ...
          templateBase.m_tanbuStart = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "端部部品(始点側)" ),
            templateBase.m_tanbuStart ) ;
          templateBase.m_tanbuEnd = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "端部部品(終点側)" ),
            templateBase.m_tanbuEnd ) ;
          templateBase.m_jack1 = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "ジャッキタイプ(1)" ),
            templateBase.m_jack1 ) ;
          templateBase.m_jack2 = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "ジャッキタイプ(2)" ),
            templateBase.m_jack2 ) ;
        }
      }
