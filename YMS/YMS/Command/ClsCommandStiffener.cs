            ...
            foreach (ElementId haraId in haraTargetLevelList)
            {
                ...
                foreach (ElementId id in allList)
                {
                    ...
                    if (hiuchiName != "なし" && hiuchiName.Contains("火打ブロック"))
                    {
                        ...
                        XYZ moveHalfSizePoint = new XYZ(point.X - dist / 2 * haraDir.X,
                                                        point.Y - dist / 2 * haraDir.Y,
                                                        point.Z);
                        ...
                        {
                            double block = ClsRevitUtil.CovertToAPI(3000.0 / 2 - tanbuHaba / 2);// Master.ClsHiuchiCsv.GetThickness(hiuchiSize));
                            XYZ pointR = new XYZ(point.X + block * haraDir.X,
                                    point.Y + block * haraDir.Y,
                                    point.Z);
                            //kiriDir = ClsRevitUtil.RotationVector(kiriDir);
                            csf.CreateStiffener(doc, haraSize, dan, pointR, haraDir, targetLevelId, tanbuHaba);

                            XYZ pointL = new XYZ(point.X - block * haraDir.X,
                                    point.Y - block * haraDir.Y,
                                    point.Z);
