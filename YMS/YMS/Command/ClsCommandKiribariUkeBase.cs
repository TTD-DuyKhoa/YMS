        ...
        try {
          ...
          if ( clsKUB.m_ShoriType == ClsKiribariUkeBase.ShoriType.Replace ) {
            ...
            using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
              ...
              ModelLine lineLeft =
                ClsYMSUtil.CreateHojyoLine( doc, tmpStPoint, tmpEdPoint, -wid1, -wid2, 0, 0 ) ; // len1, len2);
              ModelLine lineRight =
                ClsYMSUtil.CreateHojyoLine( doc, tmpEdPoint, tmpStPoint, -wid1, -wid2, 0, 0 ) ; //len1, len2);
              t.Commit() ;
    ...
    public static bool CommandChangeKiribariUkeBase( UIDocument uidoc )
    {
      ...
      for ( int i = 0 ; i < idList.Count() ; i++ ) {
        ...
        if ( i == 0 ) {
          ...
          templateBase.m_TsukidashiRyoS = ClsRevitUtil.CompareValues(
            (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "突き出し量始点" ) ),
            templateBase.m_TsukidashiRyoS ) ;
          templateBase.m_TsukidashiRyoE = ClsRevitUtil.CompareValues(
            (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "突き出し量終点" ) ),
            templateBase.m_TsukidashiRyoE ) ;
        }
      }
