        ...
        try {
          if ( clsSTB.m_ShoriType == ClsSyabariTsunagizaiBase.ShoriType.Replace ) {
            ...
            foreach ( var lineId in ids ) {
              ...
              if ( ! ClsGeo.IsLeft( ( tmpEdPoint - tmpStPoint ).Normalize(),
                    ( tmpThirdPoint - tmpStPoint ).Normalize() ) ) {
            ...
            if ( clsSTB.m_SplitFlg && false ) {
              ...
              foreach ( ElementId Id in List ) {
                ...
                if ( p2 != null ) {
                  if ( ! clsSTB.CreateSyabariTsunagizaiBase( doc, p1, p2, ( inst2.Location as LocationPoint ).Point,
                        levelID ) )
                    continue ;
                  p1 = p2 ;
                }
