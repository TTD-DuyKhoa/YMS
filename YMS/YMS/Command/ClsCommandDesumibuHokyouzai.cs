...
namespace YMS.Command
{
  public class ClsCommandDesumibuHokyouzai
  {
    public static void CommandCreateDesumibuHokyouzai( UIApplication uiapp )
    {
      ...
      XYZ pointS = new XYZ( point.X - ( dir.X * dFx ) + ( dir.Y * dEx ), point.Y - ( dir.Y * dFx ) - ( dir.X * dEx ),
        point.Z ) ;
      XYZ pointE = new XYZ( point.X + ( dir.X * dFx ) + ( dir.Y * dEx ), point.Y + ( dir.Y * dFx ) - ( dir.X * dEx ),
        point.Z ) ;
      ...
      XYZ pointS1 = new XYZ( point1.X - ( dir1.X * dFx1 ) + ( dir1.Y * dEx1 ),
        point1.Y - ( dir1.Y * dFx1 ) - ( dir1.X * dEx1 ), point1.Z ) ;
      ...
      XYZ pointS2 = new XYZ( point2.X - ( dir2.X * dFx2 ) + ( dir2.Y * dEx2 ),
        point2.Y - ( dir2.Y * dFx2 ) - ( dir2.X * dEx2 ), point2.Z ) ;
      ...
      XYZ perpendicPoint1 = new XYZ( pointS1.X + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * dir1.X ),
        pointS1.Y + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * dir1.Y ), pointS1.Z ) ;
      ...
      XYZ perpendicPoint2 = new XYZ( pointE2.X + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * -dir2.X ),
        pointE2.Y + ( ClsRevitUtil.CovertToAPI( int.MaxValue ) * -dir2.Y ), pointE2.Z ) ;
      ...
      clsDesumibuHokyouzai.CreateDesumibuHokyouzai( doc, levelID, insecPoint, sumikiriLength, length1, length2, voidDep,
        angle, rotation ) ;
    }
