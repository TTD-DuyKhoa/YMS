    ...
    public static bool CommandChangeHaraokoshiBase( UIDocument uidoc, List<ElementId> idList = null )
    {
      ...
      for ( int i = 0 ; i < idList.Count() ; i++ ) {
        ...
        if ( i == 0 ) {
          ...
          templateBase.m_dan =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "段" ), templateBase.m_dan ) ;
          templateBase.m_yoko = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "横本数" ) == "シングル"
              ? ClsHaraokoshiBase.SideNum.Single
              : ClsHaraokoshiBase.SideNum.Double, templateBase.m_yoko ) ;
          templateBase.m_tate = ClsRevitUtil.CompareValues(
            ClsRevitUtil.GetParameter( doc, id, "縦本数" ) == "シングル"
              ? ClsHaraokoshiBase.VerticalNum.Single
              : ClsHaraokoshiBase.VerticalNum.Double, templateBase.m_tate ) ;
          templateBase.m_kouzaiType = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材タイプ" ),
            templateBase.m_kouzaiType ) ;
          templateBase.m_kouzaiSize = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "鋼材サイズ" ),
            templateBase.m_kouzaiSize ) ;
          if ( templateBase.IsMega )
            templateBase.m_mega = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameterInteger( doc, id, "メガビーム本数" ),
              templateBase.m_mega ) ;
          templateBase.m_offset = ClsRevitUtil.CompareValues(
            ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "オフセット量" ) ),
            templateBase.m_offset ) ;
          templateBase.m_tateGap = ClsRevitUtil.CompareValues(
            ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "縦方向の隙間" ) ),
            templateBase.m_tateGap ) ;
          templateBase.m_gapTyouseiType =
            ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "隙間調整材鋼材ﾀｲﾌﾟ" ),
              templateBase.m_gapTyouseiType ) ;
          templateBase.m_gapTyousei = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, id, "隙間調整材" ),
            templateBase.m_gapTyousei ) ;
          templateBase.m_gapTyouseiLenght = ClsRevitUtil.CompareValues(
            ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "隙間調整材長さ" ) ),
            templateBase.m_gapTyouseiLenght ) ;
        }
      }
