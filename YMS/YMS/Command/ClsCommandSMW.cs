...
namespace YMS.CommandCls
{
    class ClsCommandSMW
    {
        public static void CommandSMW(UIApplication uiapp)
        {
            ...
            //ダイアログを表示
            YMS.DLG.DlgCreateSMW SMW = new DLG.DlgCreateSMW(doc);
            ClsGlobal.m_SMW = SMW;
            for (; ; )
            {
                ...
                {
                    ElementId id = null;
                    if (!ClsSMW.PickObject(uidoc, ref id))
                        return;
                    clsSMW.SetParameter(doc, id);
                    SMW = new DLG.DlgCreateSMW(doc,clsSMW);
                }
                else
                    break;
            }
            ...
            for (; ; )
            {
                ...
                try
                {
                    if (clsSMW.m_way == 0)
                    {
                        ...
                        for (int i = 0; i < resList.Count; i++)
                        {
                            ...
                            if (!bIrizumi)//出隅のときのみ別でSMWを作成する
                            {
                                ...
                                clsSMW.CreateSingleSMW(doc, desumipoint, dir, levelId, bIrizumi, kabeShinId : selShinId, apexAngle: apexAngle);
                            }
                            ...
                            //SMWを作図
                            if (!clsSMW.CreateSMW(doc, picPointS, picPointE, levelId, bIrizumi, true, selShinId))
                            {
                                //作図に失敗した時のコメント
                            }
                        }
                    ...
                    if (clsSMW.m_way == 2)
                    {
                        ...
                        picPointS = selection.PickPoint(clsSMW.FamilyNameKui + "を配置する開始位置を選択してください");
                        if (picPointS == null)
                        {
                            return;
                        }
                        picPointE = selection.PickPoint(clsSMW.FamilyNameKui + "を配置する終了位置を選択してください");
                        if (picPointE == null)
                        {
                            return;
                        }
                    ...
                    if (clsSMW.m_way == 3)
                    {
                        ...
                        //出隅横のサイズが違う場合があるため
                        ElementId oya1 = null;
                        if (!ClsSMW.PickObject(uidoc, ref oya1))
                            return;
                        ElementId oya2 = null;
                        if (!ClsSMW.PickObject(uidoc, ref oya2))
                            return;
                        ...
                        //Oyakuiを作図
                        if (!clsSMW.CreateSingleSMW(doc, desumipoint, dir, levelId, bIrizumi, true, true, id1, apexAngle))
                        {
                            //作図に失敗した時のコメント
                        }
                        continue;
                    ...
                    //SMWを作図
                    if (!clsSMW.CreateSMW(doc, picPointS, picPointE, levelId, true, defVoid, selShinId))
                    {
                        //作図に失敗した時のコメント
                    }
                }
