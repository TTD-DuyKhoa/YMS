    ...
    public static bool CreateJack( UIApplication uiapp )
    {
      ...
      try {
        for ( ; ; ) {
          ...
          if ( ! ClsRevitUtil.PickObjectFilters( uidoc, "切梁ベース or 切梁火打ベース or 隅火打ベースを指定してください。（Escキーで機能終了）",
                ClsGlobal.m_baseKiriOrKHiuchiOrCHiuchiOrSyabariList, ref baseId ) ) {
            break ;
          }
          ...
          string jackType = ClsRevitUtil.CompareValues( ClsRevitUtil.GetParameter( doc, baseId, "ジャッキタイプ(1)" ),
