...
namespace YMS.Master
{
  class ClsStiffenerCSV
  {
    ...
    public static string GetFamilyPath( string haraSize, string type2 )
    {
      ...
      if ( ( from data in m_Data where data.HaraSize == haraSize && data.Type2 == type2 select data.FamilyPath )
          .Any() ) {
        str = ( from data in m_Data where data.HaraSize == haraSize && data.Type2 == type2 select data.FamilyPath )
          .ToList().First() ;
      }
    ...
    public static string GetMark( string haraSize, string type2 )
    {
      ...
      if ( ( from data in m_Data where data.HaraSize == haraSize && data.Type2 == type2 select data.Mark ).Any() ) {
        str = ( from data in m_Data where data.HaraSize == haraSize && data.Type2 == type2 select data.Mark ).ToList()
          .First() ;
      }
