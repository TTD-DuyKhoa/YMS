  ...
  public class ClsKouyaitaCsv
  {
    ...
    public static List<string> GetCornerList( string type, string size )
    {
      ...
      if ( ( from data in m_Data
            where data.Type == type && data.CornerFlg == true && data.CornerType1 == str1
            select data.Size ).Any() ) {
        lst.Add( ( from data in m_Data
          where data.Type == type && data.CornerFlg == true && data.CornerType1 == str1
          select data.Size ).ToList().First() ) ;
      }

      if ( ( from data in m_Data
            where data.Type == type && data.CornerFlg == true && data.CornerType1 == str2
            select data.Size ).Any() ) {
        lst.Add( ( from data in m_Data
          where data.Type == type && data.CornerFlg == true && data.CornerType1 == str2
          select data.Size ).ToList().First() ) ;
