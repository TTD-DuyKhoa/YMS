  ...
  public class ClsSyabariBase
  {
    ...
    public bool CreateSyabariBase( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, ElementId levelId )
    {
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        slopePlane =
          ClsRevitUtil.CreateReferencePlaneW( doc, tmpStPoint, tmpEdPoint, ClsRevitUtil.CovertToAPI( m_endOffset1 ) ) ;
        t.Commit() ;
      }
    ...
    public bool CreateSyabariBase( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, ReferencePlane referencePlane,
      ElementId levelId )
    {
      //参照面の作成
    ...
    public static PieceVertices CalcPieceVertices( PieceData data, double theta3, double theta2, double theta1,
      double theta0, double? mortarLength = null )
    {
    ...
    public static double CalcMortarLength( Plane sectionPlane, Line shaBaseLine, /*XYZ kozaiPt, XYZ kozaiOppositePt,*/
      Plane shaBasePlane, double shaW, double shaH )
    {
      ...
      var length = ptsOnHaraWall.Select( x => transform * x ).Select( x => Math.Abs( x.Z ) )
        .Max() ; //.FirstOrDefault();
    ...
    {
      var shinfamily = path ;
      var symbolName0 = ! string.IsNullOrEmpty( symbolName )
        ? symbolName
        : System.IO.Path.GetFileNameWithoutExtension( shinfamily ) ;
    ...
    public (ElementId baseId, ElementId piece1, ElementId piece2) ReplaceShabariBase( Document doc,
      ElementId originalBaseId )
    {
      //斜梁ベースを選択
      ...
      var num = 2 ;
      var buhinType = new string[] { shabariClass.m_buhinTypeStart, shabariClass.m_buhinTypeEnd, } ;
      var buhinSize = new string[] { shabariClass.m_buhinSizeStart, shabariClass.m_buhinSizeEnd, } ;
      ...
      for ( int i = 0 ; i < num ; i++ ) {
        var rec = Master.ClsSyabariPieceCSV.Shared.FirstOrDefault( x =>
          x.TypeName == buhinType[ i ] && x.SizeName == buhinSize[ i ] ) ;
      ...
      var haraPtSet =
        ClsHaraokoshiBase.GetIntersectionBase2ModelLine( doc,
          shaBase.Id ) ; //ClsHaraokoshiBase.GetIntersectionBase2(doc, shaBase.Id);
      ...
      var baseId = CreateShabariBase( doc, pieceIds.ElementAtOrDefault( 0 ), pieceIds.ElementAtOrDefault( 1 ),
        shaBaseInstance ) ;
    ...
    /// <param name="message">指示</param>
    /// <returns></returns>
    public static bool PickBaseOrKariKouzaiObject( UIDocument uidoc, ref ElementId id,
      string message = baseName + "仮鋼材" )
    {
