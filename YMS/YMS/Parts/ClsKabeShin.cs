...
namespace YMS
{
  public class ClsKabeShin
  {
    ...
    /// <returns></returns>
    private static bool CreateKabeShin( UIDocument uidoc, string levelName, ElementId selShinid,
      ref ElementId createId )
    {
      Document doc = uidoc.Document ;
    ...
    /// <param name="createId">作成した壁芯</param>
    /// <returns></returns>
    public static bool CreateKabeShin( Document doc, string levelName, XYZ tmpStPoint, XYZ tmpEdPoint,
      ref ElementId createId )
    {
      string symbolFolpath = ClsZumenInfo.GetYMSFolder() ;
      string familyPath = System.IO.Path.Combine( symbolFolpath, "ベース関係\\" + familyName + ".rfa" ) ;
    ...
    public static bool GetCsvLineKabe( Document doc, out List<string> kabeList )
    {
      ...
      try {
        foreach ( ElementId id in allKabeList ) {
          ...
          var existingData =
            kabeDataList.FirstOrDefault( data => data.CaseName == kabeData.CaseName && data.Usage == kabeData.Usage ) ;
          ...
          if ( kabeData.Usage != ClsKouyaita.KOUYAITA ) {
            ...
            Master.ClsKouyaitaTsugiteCsv pileCsv =
              Master.ClsKouyaitaTsugiteCsv.GetCls( kabeData.JointSpecification, kabeData.Size ) ;
    ...
    public static bool GetCsvLineKui( Document doc, out List<string> kuiList )
    {
      ...
      try {
        ...
        foreach ( var tanakuiId in tanakuiIds ) {
          ...
          tanakuiData.Size =
            Master.ClsHBeamCsv.GetSizePileFamilyPathInName( instTanakui.Symbol
