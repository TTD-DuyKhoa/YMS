...
namespace YMS
{
  public class ClsKariKouzai
  {
    ...
    public static bool CreateKariKouzai( Document doc, ref string contents, ref List<ElementId> selectionIds )
    {
      ...
      //INIからの読み込み
      double syabariukePieceLowerLimitθ1 = 0,
        syabariukePieceUpperLimitθ1 = 0,
        syabariukePieceLowerLimitθ2 = 0,
        syabariukePieceUpperLimitθ2 = 0,
        kaitenPieceLowerLimitθ2 = 0,
        kaitenPieceUpperLimitθ2 = 0 ;
      ...
      if ( File.Exists( iniPath ) ) {
        ...
        needCheckingRotationAngle =
          double.TryParse( syabariukePieceLowerLimitθ1Str, out syabariukePieceLowerLimitθ1 ) &&
      ...
      foreach ( ElementId baseID in targetFamilies ) {
        ...
        if ( buzaiName == "切梁火打" ) //端部部品なしの場合は計算を抜く
        {
          ...
          double hLengthUP =
            ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, baseID, HIUCHILENGTHDOUBLEUP ) ) ;
        ...
        if ( buzaiName == "斜梁" ) //仮鋼材オン→斜梁火打作成→仮鋼材オフ→仮鋼材オンの時の斜梁火打ちの再作成はここでやる　#34047
        {
          ...
          if ( needCheckingRotationAngle ) {
            for ( int i = 0 ; i < 2 ; i++ ) {
              ...
              if ( ! symbolName.Contains( "斜梁受" ) && ! symbolName.Contains( "回転" ) ) {
                continue ;
              }
              ...
              if ( symbolName.Contains( "斜梁受" ) ) {
                ...
                if ( double.TryParse( theta1Str.Replace( "°", "" ), out double theta1 ) ) {
                  ...
                  if ( ! ( ClsGeo.GEO_GE( theta1, syabariukePieceLowerLimitθ1 ) &&
                           ClsGeo.GEO_LE( theta1, syabariukePieceUpperLimitθ1 ) ) ) {
        ...
        if ( kousei != DOUBLE ) {
          if ( GetSingleFamilySymbol( ref doc, baseID, kouzaiType, buzaiName, kouzaiSize, out FamilySymbol sym,
                ref dKouzaiSize ) )
            CreateSingleKarikouzai( ref doc, baseID, sym, dKouzaiSize, pS, pE ) ;
        }
        else {
          if ( GetDoubleFamilySymbol( doc, kouzaiType, buzaiName, kouzaiSizeUP, kouzaiSizeDN,
                out List<FamilySymbol> symList, out List<double> kouzaiSizeList ) )
    ...
    public static (XYZ, XYZ) GetHaraokoshiLine( Document doc, ElementId id, double dKouzaiSize,
      ClsHaraokoshiBase.WinLose WinLose, ClsHaraokoshiBase.SideNum yoko )
    {
      ...
      foreach ( ElementId haraId in haraIdList ) {
        ...
        if ( lCurveHara != null ) {
          ...
          if ( insec != null ) {
            insecList.Add( insec ) ;
            insecKouzaiSizeList.Add( ClsRevitUtil.CovertToAPI(
              Master.ClsYamadomeCsv.GetWidth( ClsRevitUtil.GetParameter( doc, haraId, KOUZAISIZE ) ) ) ) ;
            ...
            insecSideNumList.Add( ClsRevitUtil.GetParameter( doc, haraId, "横本数" ) == "シングル"
        ...
        for ( int i = 0 ; i < insecList.Count ; i++ ) {
          ...
          if ( distanceToStart < distanceToEnd ) {
            ...
            tmpStPoint = new XYZ( insecList[ i ].X - ( kouzaisize * Direction.X ),
              insecList[ i ].Y - ( kouzaisize * Direction.Y ), insecList[ i ].Z ) ;
          }
          else {
            // 判定したい点は終点に近いです
      ...
      //壁側に位置をずらす
      tmpStPoint = new XYZ( tmpStPoint.X + ( dKouzaiSize * Direction.Y ), tmpStPoint.Y - ( dKouzaiSize * Direction.X ),
        tmpStPoint.Z ) ;
      tmpEdPoint = new XYZ( tmpEdPoint.X + ( dKouzaiSize * Direction.Y ), tmpEdPoint.Y - ( dKouzaiSize * Direction.X ),
        tmpEdPoint.Z ) ;
    ...
    private static XYZ AdjustIrizumiIntersection( Curve cv, XYZ insec, double dKouzaiSize )
    {
      ...
      if ( distanceToStart < distanceToEnd ) {
        // 判定したい点は始点に近いです
        adjustPoint = new XYZ( insec.X + ( dKouzaiSize * 2 * Direction.X ), insec.Y + ( dKouzaiSize * 2 * Direction.Y ),
          insec.Z ) ;
      }
      else {
        // 判定したい点は終点に近いです
    ...
    /// <returns></returns>
    private static bool CreateSingleKarikouzai( ref Document doc, ElementId id, FamilySymbol kouzaiSym,
      double dKouzaiSize, XYZ pointS = null, XYZ pointE = null )
    {
      string dan = ClsRevitUtil.GetParameter( doc, id, "段" ) ;
      FamilyInstance inst = doc.GetElement( id ) as FamilyInstance ;
      ElementId levelID = inst.Host.Id ;
      ...
      if ( buzaiShinName == ClsHaraokoshiBase.baseName ) //腹起の場合は壁側に仮鋼材を配置
      {
        ...
        if ( yoko == ClsHaraokoshiBase.SideNum.Double ) {
          ...
          hojyoS = new XYZ( hojyoS.X + ( dKouzaiSize * 2 * dir.Y ), hojyoS.Y - ( dKouzaiSize * 2 * dir.X ), hojyoS.Z ) ;
          hojyoE = new XYZ( hojyoE.X + ( dKouzaiSize * 2 * dir.Y ), hojyoE.Y - ( dKouzaiSize * 2 * dir.X ), hojyoE.Z ) ;
      ...
      if ( buzaiShinName == ClsCornerHiuchiBase.baseName ) {
        ...
        if ( ClsYMSUtil.GetNonObjectFlagBaseLinePoints( doc, tmpStPoint, tmpEdPoint, size ) )
          if ( ClsYMSUtil.GetNonObjectFlagBaseLinePoints( doc, tmpStPoint, tmpEdPoint, size, true ) ) {
            ClsCornerHiuchiBase chb = new ClsCornerHiuchiBase() ;
            chb.SetParameter( doc, id ) ;
            chb.CreateHojyoPeace( doc, notPojntList[ 0 ], notPojntList[ 1 ], chb.m_HiuchiLengthSingleL,
              dan ) ; //パラメータを増やすシングルも上下につく
            notPojntLists = ClsYMSUtil.GetNonObjectBaseLinePoints( doc, tmpStPoint, tmpEdPoint ) ;
          }
      }
      ...
      if ( buzaiShinName == ClsKiribariHiuchiBase.baseName ) {
        ...
        if ( ClsYMSUtil.GetNonObjectFlagBaseLinePoints( doc, tmpStPoint, tmpEdPoint, size ) )
          if ( ClsYMSUtil.GetNonObjectFlagBaseLinePoints( doc, tmpStPoint, tmpEdPoint, size, true ) ) {
            ...
            ckb.SetParameter( doc, id ) ;
            ckb.CreateHojyoPeace( doc, notPojntList[ 0 ], notPojntList[ 1 ], ckb.m_SteelSizeSingle,
              ckb.m_HiuchiLengthSingleL, dan ) ; //パラメータを増やす
            notPojntLists = ClsYMSUtil.GetNonObjectBaseLinePoints( doc, tmpStPoint, tmpEdPoint ) ;
          }
      }
        ...
        foreach ( List<XYZ> notPojntList in notPojntLists ) {
          ...
          if ( kouzaiSym.FamilyName.Contains( "80SMH" ) ) {
            ...
            if ( yoko == ClsHaraokoshiBase.SideNum.Double ) {
              ...
              XYZ dir = Line.CreateBound( hojyoS, hojyoE ).Direction ;
              hojyoS = new XYZ( hojyoS.X + ( dKouzaiSize * 2 * dir.Y ), hojyoS.Y - ( dKouzaiSize * 2 * dir.X ),
                hojyoS.Z ) ;
              hojyoE = new XYZ( hojyoE.X + ( dKouzaiSize * 2 * dir.Y ), hojyoE.Y - ( dKouzaiSize * 2 * dir.X ),
                hojyoE.Z ) ;
    ...
    private static bool CreateSingleKarikouzaiMegabeam( ref Document doc, ElementId id, Curve cv, string dan,
      FamilySymbol kouzaiSym, FamilySymbol symbol )
    {
      FamilyInstance inst = doc.GetElement( id ) as FamilyInstance ;
      ElementId levelID = inst.Host.Id ;
      string buzaiShinName = inst.Name ;
      ...
      for ( int i = 0 ; i < megaBeamCount ; i++ ) {
        ...
        ElementTransformUtils.MoveElement( doc, CreatedID,
          leftVector * ClsRevitUtil.ConvertDoubleGeo2Revit( ClsYMSUtil.GetKouzaiHabaFromBase( doc, id ) ) ) ;
    ...
    public static bool CreateSingleKarikouzaiSyabari( Document doc, ElementId id, string buzaiName, string kouzaiSize,
      XYZ pointS = null )
    {
    ...
    private static bool GetSingleFamilySymbol( ref Document doc, ElementId id, string kouzaiType, string buzaiName,
      string kouzaiSize, out FamilySymbol sym, ref double dKouzaiSize )
    {
      sym = null ;
    ...
    /// <returns></returns>
    private static bool CreateDoubleKariKouzai( Document doc, ElementId id, List<FamilySymbol> symList,
      List<double> kouzaiSizeList, XYZ pointS = null, XYZ pointE = null )
    {
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        FamilyInstance inst = doc.GetElement( id ) as FamilyInstance ;
        ElementId levelID = inst.Host.Id ;
        ...
        double hLengthUP =
          ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, id, HIUCHILENGTHDOUBLEUP ) ) ) ;
        double hLengthDN =
          ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, id, HIUCHILENGTHDOUBLEDN ) ) ) ;
        double hLengthZR =
          ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, id, HIUCHIZURERYOU ) ) ) ;
            ...
            if ( ClsRevitUtil.IsPointOnCurve( tgCv, pCheckOnCurve ) ) {
              ...
              if ( buzaiShinName == ClsKiribariTsugiBase.baseName ) {
                if ( tgInst.Name == ClsHaraokoshiBase.baseName ) {
                  dLength = ClsRevitUtil.CovertToAPI(
                    ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, id, KOUZAILENGTHHARAOKOSHI ) ) ) ;
                }
                else if ( tgInst.Name == ClsKiribariBase.baseName ) {
                  dLength = ClsRevitUtil.CovertToAPI(
                    ClsCommonUtils.ChangeStrToDbl( ClsRevitUtil.GetParameter( doc, id, KOUZAILENGTHKIRIBARI ) ) ) ;
              ...
              if ( buzaiShinName == ClsKiribariHiuchiBase.baseName ) {
                ...
                ClsKiribariHiuchiBase ckb = new ClsKiribariHiuchiBase() ;
                ckb.SetParameter( doc, id ) ;
                ckb.CreateHojyoPeace( doc, notPojntList[ 0 ], notPojntList[ 1 ], kouzaiSizeKiribariHiuchi,
                  hojyoJudgeLength, dan ) ; //パラメータを増やす
                notPojntLists = ClsYMSUtil.GetNonObjectBaseLinePoints( doc, sPnt, ePnt ) ;
    ...
    /// <param name="doc"></param>
    /// <returns></returns>
    private static bool GetDoubleFamilySymbol( Document doc, string kouzaiType, string buzaiName, string kouzaiSizeUP,
      string kouzaiSizeDN, out List<FamilySymbol> symList, out List<double> kouzaiSizeList )
    {
      //FamilyInstance inst = doc.GetElement(id) as FamilyInstance;
