...
namespace YMS.Parts
{
  public class ClsBracket
  {
    ...
    public bool CreateKiribariBracket( Document doc, ElementId kuiId, bool sizeIsAuto = false )
    {
      ...
      if ( ! GetTargetKiribariBaseId_Web( doc, kuiId, out List<ElementId> kiribariIds, out List<bool> isFront,
            out List<XYZ> crossPoints ) ) {
        return false ;
      }
      ...
      for ( int i = 0 ; i < kiribariIds.Count ; i++ ) {
        ...
        if ( sizeIsAuto ) {
          m_BracketSize =
            GetKiribariBracketSizeWithAuto( ClsRevitUtil.GetInstMojiParameter( doc, kiribariIds[ i ], "鋼材サイズ" ) ) ;
        }
        ...
        double kuiSizeHalf = ClsRevitUtil.CovertToAPI(
          ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou1( instKui.Symbol.FamilyName, 1 ) ) / 2 ) ;

        XYZ insertPointSt = XYZ.Zero ;
        if ( isFront[ i ] ) {
          insertPointSt = new XYZ( pkui.X + ( ( kuiSizeHalf ) * instKui.FacingOrientation.X ),
            pkui.Y + ( ( kuiSizeHalf ) * instKui.FacingOrientation.Y ), pkui.Z ) ;
        }
        else {
          insertPointSt = new XYZ( pkui.X - ( ( kuiSizeHalf ) * instKui.FacingOrientation.X ),
            pkui.Y - ( ( kuiSizeHalf ) * instKui.FacingOrientation.Y ), pkui.Z ) ;
        ...
        string kiribariSteelSize =
          ClsRevitUtil.GetInstMojiParameter( doc, kiribariIds[ i ], "鋼材サイズ" ) ; //.Replace("HA", "");
        //kiribariSteelSize = kiribariSteelSize.Replace("SMH", "");
        kiribariSize =
          ClsRevitUtil.CovertToAPI(
            Master.ClsYamadomeCsv
              .GetWidth( kiribariSteelSize ) ) ; // ClsCommonUtils.ChangeStrToDbl(kiribariSteelSize) * 10);
        if ( kiribariSize == 0 ) {
        ...
        using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
          ...
          ElementTransformUtils.RotateElement( doc, createdId,
            Line.CreateBound( insertPointSt, insertPointSt + XYZ.BasisZ ), kuiAngle ) ;
          double angle = GetRotateAngle( pkui, insertPointSt, instBracket ) ;
          ElementTransformUtils.RotateElement( doc, createdId,
            Line.CreateBound( insertPointSt, insertPointSt + XYZ.BasisZ ), angle ) ;
    ...
    public bool CreateKiribariOsaeBracket( Document doc, ElementId kuiId, bool sizeIsAuto = false )
    {
      ...
      // 対象となる切梁を取得
      if ( ! GetTargetKiribariBaseId_Web( doc, kuiId, out List<ElementId> kiribariIds_Web, out List<bool> isFront_Web,
            out List<XYZ> crossPoints_Web ) ) {
        return false ;
      }
      ...
      if ( ! GetTargetKiribariBaseId_Flange( doc, kuiId, out List<ElementId> kiribariIds_Flange,
            out List<bool> isFront_Flange, out List<XYZ> crossPoints_Flange ) ) {
        return false ;
      }
      ...
      for ( int i = 0 ; i < kiribariIds_Flange.Count() ; i++ ) {
        ...
        for ( int k = 0 ; k < kiribariIds_Web.Count() ; k++ ) {
          ...
          if ( sizeIsAuto ) {
            m_BracketSize =
              GetKiribariBracketSizeWithAuto(
                ClsRevitUtil.GetInstMojiParameter( doc, kiribariIds_Flange[ i ], "鋼材サイズ" ) ) ;
          }
          ...
          switch ( m_TargetKuiCreatePosition ) {
            case CreatePosition.LeftTop :
              if ( kuiAngle >= Math.PI / 2 ) {
                insertPoint = new XYZ( ptkui.X + ( ( kuiSizeHalf + bracketWidthHalf ) * vecWeb.X ),
                  ptkui.Y + ( ( kuiSizeHalf + bracketWidthHalf ) * vecWeb.Y ), ptkui.Z ) ;
          ...
          string kiribariSteelSize =
            ClsRevitUtil.GetInstMojiParameter( doc, kiribariIds_Flange[ i ], "鋼材サイズ" ) ; //.Replace("HA", "");
          double kiribariSize =
            ClsRevitUtil.CovertToAPI(
              Master.ClsYamadomeCsv
                .GetWidth( kiribariSteelSize ) ) ; // ClsCommonUtils.ChangeStrToDbl(kiribariSteelSize) * 10);
    ...
    public bool CreateHaraokoshiBracket( Document doc, List<ElementId> oyakuiIds, List<ElementId> haraokoshiIds,
      string bracketFamilyTypeName )
    {
      ...
      foreach ( var haraokoshiId in haraokoshiIds ) {
        ...
        double syuzaiLength =
          ClsRevitUtil.CovertToAPI( ClsYMSUtil.GetSyuzaiLength( instHaraokoshi.Symbol.Family.Name ) ) ;
        if ( syuzaiLength == 0 ) {
          continue ;
        }
        ...
        foreach ( var id in oyakuiIds ) {
          ...
          SetComparisonResult result =
            cvHaraokoshi.Intersect( cvOyakui, out IntersectionResultArray intersectionResults ) ;
          ...
          // 線分同士の交差を判定//ツインビーム用
          Line extendedLine = Line.CreateUnbound( cvHaraokoshi.GetEndPoint( 0 ),
            cvHaraokoshi.GetEndPoint( 1 ) - cvHaraokoshi.GetEndPoint( 0 ) ) ;
          ...
          if ( result == SetComparisonResult.Overlap ) {
            foreach ( IntersectionResult intersectionResult in intersectionResultsTwin ) {
              intersectionPointsTwin.Add(
                new Tuple<XYZ, FamilyInstance>( intersectionResult.XYZPoint, instTrgOyakui ) ) ;
            }
          }
        }
        ...
        foreach ( var kiribaribaseId in kiribariBaseIds ) {
          ...
          SetComparisonResult result =
            cvHaraokoshi.Intersect( cvBase, out IntersectionResultArray intersectionResults ) ;
          if ( result == SetComparisonResult.Overlap ) {
            foreach ( IntersectionResult intersectionResult in intersectionResults ) {
              intersectionPointsTwinBeam.Add(
                new Tuple<XYZ, FamilyInstance>( intersectionResult.XYZPoint, instBase ) ) ;
            }
          }
        }
        ...
        if ( ! isOsae || haraokoshiSteelSize == "80SMH" ) {
          if ( ! usedKuiIdsLevel.Contains(
                new KeyValuePair<ElementId, string>( instKuiS.Id,
                  levelName ) ) ) //!usedKuiIds.Contains(instKuiS.Id) || 
          {
            CreateHaraokoshiBracket( doc, bracketFamilyTypeName, haraokoshiId, searchRange, haraokoshiBaseId, sym,
              haraokoshiSize, elevation, instKuiS ) ;
            var pair = new KeyValuePair<ElementId, string>( instKuiS.Id, levelName ) ;
            usedKuiIdsLevel.Add( pair ) ;
          }

          if ( ! usedKuiIdsLevel.Contains(
                new KeyValuePair<ElementId, string>( instKuiE.Id,
                  levelName ) ) ) //!usedKuiIds.Contains(instKuiE.Id) || 
          {
            CreateHaraokoshiBracket( doc, bracketFamilyTypeName, haraokoshiId, searchRange, haraokoshiBaseId, sym,
              haraokoshiSize, elevation, instKuiE ) ;
        ...
        if ( haraokoshiSteelSize == "80SMH" ) {
          if ( ! usedKuiIdsLevel.Contains(
                new KeyValuePair<ElementId, string>( instKuiMid.Id,
                  levelName ) ) ) //!usedKuiIds.Contains(instKuiMid.Id) || 
          {
            CreateHaraokoshiBracket( doc, bracketFamilyTypeName, haraokoshiId, searchRange, haraokoshiBaseId, sym,
              haraokoshiSize, elevation, instKuiMid ) ;
        ...
        if ( bracketFamilyTypeName == "腹起押えBL" && intersectionPointsTwinBeam.Count() != 0 ) {
          // ファミリを取得
          bracketFamilyPath =
            Master.ClsBracketCsv.GetFamilyPath( ClsBracketSizeCsv.GetBracketSize( "80SMH", 1, 2, true ) ) ;
          ...
          if ( ! usedKuiIdsLevel.Contains(
                new KeyValuePair<ElementId, string>( instKui1_Twin.Id,
                  levelName ) ) ) //!usedKuiIds.Contains(instKui1_Twin.Id) || 
          {
            CreateHaraokoshiBracket( doc, bracketFamilyTypeName, haraokoshiId, searchRange, haraokoshiBaseId, sym,
              haraokoshiSize, elevation, instKui1_Twin ) ;
            var pair = new KeyValuePair<ElementId, string>( instKui1_Twin.Id, levelName ) ;
            usedKuiIdsLevel.Add( pair ) ;
          }

          if ( ! usedKuiIdsLevel.Contains(
                new KeyValuePair<ElementId, string>( instKui2_Twin.Id,
                  levelName ) ) ) //!usedKuiIds.Contains(instKui2_Twin.Id) || 
          {
            CreateHaraokoshiBracket( doc, bracketFamilyTypeName, haraokoshiId, searchRange, haraokoshiBaseId, sym,
              haraokoshiSize, elevation, instKui2_Twin ) ;
    ...
    private static void CreateHaraokoshiBracket( Document doc, string bracketFamilyTypeName, ElementId haraokoshiId,
      double searchRange, ElementId haraokoshiBaseId, FamilySymbol sym, double haraokoshiSize, double elevation,
      FamilyInstance instTrgOyakui )
    {
      if ( instTrgOyakui == null ) return ;

      double trgKuiSize =
        ClsRevitUtil.CovertToAPI(
          ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou1( instTrgOyakui.Symbol.FamilyName, 1 ) ) / 2 ) ;
      XYZ insertPointSt = XYZ.Zero ;
      ...
      if ( instTrgOyakui.Symbol.FamilyName.Contains( "杭" ) ) {
        ...
        insertPointSt =
          GetInsertPoint_Kouyaita( doc, instTrgOyakui, instTrgOyakui.Symbol.Family.Name, ref centerPoint ) ;
      }
    ...
    public bool CreateHaraokoshiOsaeBracket_Auto( Document doc, ElementId kuiId, ElementId haraokoshiId )
    {
      ...
      foreach ( var id in bracketIds ) {
        ...
        // 線分同士の交差を判定
        SetComparisonResult result =
          cvHaraokoshi.Intersect( cvBracket, out IntersectionResultArray intersectionResults ) ;
        if ( result == SetComparisonResult.Overlap ) {
          foreach ( IntersectionResult intersectionResult in intersectionResults ) {
      ...
      string bracketFamilyPath =
        Master.ClsBracketCsv.GetFamilyPath( ClsBracketSizeCsv.GetBracketSize( haraokoshiSteelSize, numV, numH ) ) ;
      string bracketFamilyName = ClsRevitUtil.GetFamilyName( bracketFamilyPath ) ;
      if ( ! ClsRevitUtil.LoadFamilyData( doc, bracketFamilyPath, out Family family ) ) {
        return false ;
      }
    ...
    private bool GetTargetHaraokoshiId( Document doc, FamilyInstance instKui, out ElementId HaraokoshiId,
      out bool isFront )
    {
      HaraokoshiId = null ;
      isFront = true ;
      ...
      if ( instKui != null ) {
        ...
        foreach ( var id in haraokoshiFamilyInstancesIds ) {
          ...
          if ( locationCurve != null ) {
            ...
            // CurveのZ座標を0に設定
            cvHaraokoshi =
              Line.CreateBound( new XYZ( cvHaraokoshi.GetEndPoint( 0 ).X, cvHaraokoshi.GetEndPoint( 0 ).Y, XYZ.Zero.Z ),
    ...
    private bool GetTargetKiribariBaseId_Web( Document doc, ElementId kuiId, out List<ElementId> beamIds,
      out List<bool> isFront, out List<XYZ> crossPoints )
    {
    ...
    private bool GetTargetKiribariBaseId_Flange( Document doc, ElementId kuiId, out List<ElementId> beamIds,
      out List<bool> isFront, out List<XYZ> crossPoints )
    {
      beamIds = new List<ElementId>() ;
      isFront = new List<bool>() ;
    ...
    public bool CreatePreloadGuideZai( Document doc, ElementId bracketId, ElementId kiribariId )
    {
      ...
      if ( instTarget.Symbol.Name == "切梁BL" ) {
        ...
        XYZ defaultFacing = new XYZ( instTarget.FacingOrientation.X, instTarget.FacingOrientation.Y,
          instTarget.FacingOrientation.Z ) ;
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        ElementTransformUtils.RotateElement( doc, createdId1, Line.CreateBound( ptInsert1, ptInsert1 + XYZ.BasisZ ),
          angle1 ) ;
        FamilyInstance instGuideZai2 = doc.GetElement( createdId2 ) as FamilyInstance ;
        ElementTransformUtils.RotateElement( doc, createdId2, Line.CreateBound( ptInsert2, ptInsert2 + XYZ.BasisZ ),
          angle2 ) ;
    ...
    private void GetPreloadGuideZaiAngle( XYZ ptBracket, XYZ ptCross, FamilyInstance instTarget, ref double angle1,
      ref double angle2 )
    {
    ...
    private static XYZ GetInsertPoint_Oyakui( Document doc, FamilyInstance instOyakui, string oyakuiSize,
      ref XYZ centerPoint, bool isFront = true )
    {
      ...
      double oyaHeight =
        ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou1( oyakuiSize, 1 ) ) ) ;
    ...
    public static XYZ GetInsertPoint_Kouyaita( Document doc, FamilyInstance instKouyaita, string kouyaitaSize,
      ref XYZ centerPoint )
    {
      // 鋼矢板の挿入点は中央ではなく端部
      ...
      if ( voidVec == 1 ) {
        if ( Math.Abs( vecKouyaita.X - 1 ) < tolerance && Math.Abs( vecKouyaita.Y ) < tolerance &&
             Math.Abs( vecKouyaita.Z ) < tolerance ) {
          vecOffset = new XYZ( 0, 1, 0 ) ;
        }
        else if ( Math.Abs( vecKouyaita.X ) < tolerance && Math.Abs( vecKouyaita.Y - 1 ) < tolerance &&
                  Math.Abs( vecKouyaita.Z ) < tolerance ) {
      ...
      double oyaWidth =
        ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( Master.ClsKouyaitaCsv.GetWidth( kouyaitaSize ) ) ) ;
      XYZ ptKouyaitaCenter = ptKouyaita + ( vecOffset * ( oyaWidth / 2 ) ) ;
      centerPoint = ptKouyaitaCenter ;

      double oyaHeight =
        ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( Master.ClsKouyaitaCsv.GetHeight( kouyaitaSize ) ) ) ;
    ...
    public Curve GetOyakuiCurve( Document doc, ElementId oyakuiId, XYZ haraokoshiStartPoint, XYZ haraokoshiEndPoint,
      double searchRange )
    {
      // FamilyInstanceを取得
