...
namespace YMS.Parts
{
    public  class ClsStiffener
    {
        ...
        public bool CreateStiffener(Document doc, string haraSize, string dan, XYZ point, XYZ dir, ElementId levelId, double tanbuHaba)
        {
            string path;
            ...
            if (m_StiffenerType == PLATE)
            {
                ...
                point = new XYZ(point.X + dist * dir.Y,
                                point.Y - dist * dir.X,
                                point.Z);
                ...
                using (Transaction transaction = new Transaction(doc, "スチフナー作成"))
                {
                        ...
                        case 2:
                            {
                                ...
                                XYZ point1 = new XYZ(point.X + dist * dir.X,
                                                point.Y + dist * dir.Y,
                                                point.Z);
                                pointList.Add(point1);

                                XYZ point2 = new XYZ(point.X - dist * dir.X,
                                                point.Y - dist * dir.Y,
                                                point.Z);
                    ...
                    foreach (XYZ insertPoint in pointList)
                    {
                        ...
                        if (m_StiffenerType == PLATE)
                        {
                            ...
                            ClsRevitUtil.RotateElement(doc, CreatedID, axis, angle);
                            ClsRevitUtil.SetParameter(doc, CreatedID, "基準レベルからの高さ", offset - ClsRevitUtil.CovertToAPI(dHaraSize / 2));
                        }
                        else
                        {
