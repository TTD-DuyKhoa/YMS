    ...
    public class ClsShimetukePiece
    {
        ...
        public bool CreateShimetukePiece(Document doc, ElementId bracketId, ElementId kiribariId)
        {
            ...
            if (location != null)
            {
                ...
                double kiribariLength = ClsRevitUtil.CovertToAPI(ClsYMSUtil.GetSyuzaiLength(instKiribari.Symbol.Family.Name));
                if (kiribariLength == 0)
                {
                    return false;
                }
            ...
            using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
            {
                ...
                ElementTransformUtils.RotateElement(doc, createdId, Line.CreateBound(ptInsert, ptInsert + XYZ.BasisZ), angle);
