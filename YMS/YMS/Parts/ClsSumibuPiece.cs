...
namespace YMS.Parts
{
  public class ClsSumibuPiece
  {
    ...
    /// <returns></returns>
    public bool CreateSumibuPiece( Document doc, ElementId id, XYZ insertPoint, string kouzaiSize, double angle,
      bool bIsHorizontal, bool bIsUpper = false )
    {
      string familyPath = "" ;
      ...
      if ( bIsHorizontal ) {
        ...
        if ( bIsUpper ) {
          ...
          if ( ! SymbolExistsInFamily( doc, familyName, symbolType ) ) {
            // 存在しない場合は複製する
            FamilySymbol sourceSymbol = new FilteredElementCollector( doc ).OfClass( typeof( FamilySymbol ) )
              .Cast<FamilySymbol>().FirstOrDefault( s => s.Family.Name == familyName && s.Name == "V" ) ;
        ...
        foreach ( var symbolId in familySymbols ) {
          ...
          if ( symbol != null && symbol.Name == symbolType ) {
            using ( Transaction transaction = new Transaction( doc, "シンボルの挿入" ) ) {
              ...
              // インスタンスを作成
              placedInstance = doc.Create.NewFamilyInstance( (XYZ) insertPoint, (FamilySymbol) symbol,
                StructuralType.NonStructural ) ;

              // 回転軸を作成
              Line rotationAxis =
                Line.CreateBound( (XYZ) insertPoint, (XYZ) insertPoint + new XYZ( 0, 0, 1 ) ) ; // Z軸周りに回転
              // 配置された FamilyInstance を回転
              ElementTransformUtils.RotateElement( doc, placedInstance.Id, rotationAxis, angle ) ;

              if ( bIsHorizontal &&
                   ClsRevitUtil.CheckNearGetEndPoint( ( haraInst.Location as LocationCurve ).Curve, insertPoint ) &&
                   m_ToritsukeDan == "同段" ) {
              ...
              // インスタンスに拡張データを付与
              ClsRevitUtil.CustomDataSet<bool>( doc, placedInstance.Id, "作成方法",
                ( m_CreateType == CreateType.Auto ) ? true : false ) ;
    ...
    /// <param name="angle"></param>
    /// <returns></returns>
    public bool GetCrossPointAndAngle( LocationCurve lCurve1, LocationCurve lCurve2, out XYZ crossPoint,
      out double angle )
    {
      crossPoint = null ;
      angle = 0.0 ;
    ...
    /// <returns></returns>
    public bool GetCenterPlaceOffset( LocationCurve lCurve1, LocationCurve lCurve2, XYZ crossPoint, double kouzaiHaba,
      out XYZ selVec1, out XYZ selVec2, out XYZ moveVector )
    {
