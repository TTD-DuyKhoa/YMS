...
namespace YMS.Parts
{
  public class ClsKiribariHiuchiBase
  {
    ...
    public bool CreateKiribariHiuchiBase( UIDocument uidoc, ElementId id1, ElementId id2, ElementId levelID, string dan,
      XYZ picPoint1 = null, XYZ picPoint2 = null )
    {
      //ドキュメントを取得
        ...
        if ( GetBaseStartEndPoint( doc, id1, id2, ref pntS, ref pntE, ref insec ) ) {
          ...
          if ( picPoint1 != null ) {
            if ( ! ClsRevitUtil.JudgeOnLine( insec, pntS, picPoint1 ) ) {
              ...
              newPoint = ClsRevitUtil.GetDistanceNewPoint( insec, -dirKiriLine,
                ClsRevitUtil.CovertFromAPI( kiriLine.Length ) ) ;
            }
          }
      ...
      else //自動不明
      {
        ...
        if ( GetBaseStartEndPoint( doc, id1, id2, ref pntS, ref pntE, ref insec ) ) {
          ...
          XYZ dirKiriLine = kiriLine.Direction ;
          XYZ newEndPoint =
            ClsRevitUtil.GetDistanceNewPoint( insec, -dirKiriLine, ClsRevitUtil.CovertFromAPI( kiriLine.Length ) ) ;
    ...
    public bool CreateKiribariHiuchiBase( Document doc, XYZ pntS, XYZ pntE, ElementId levelID, string dan )
    {
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        if ( m_CreateHoho == CreateHoho.Double ) {
          ...
          ClsRevitUtil.SetParameter( doc, CreatedID, "火打長さ(ダブル上)L1",
            ClsRevitUtil.CovertToAPI( m_HiuchiLengthDoubleUpperL1 ) ) ;
          ClsRevitUtil.SetParameter( doc, CreatedID, "火打長さ(ダブル下)L2",
            ClsRevitUtil.CovertToAPI( m_HiuchiLengthDoubleUnderL2 ) ) ;
          ...
          ClsRevitUtil.SetParameter( doc, CreatedID, "火打長さ(シングル)L",
            ClsRevitUtil.CovertToAPI( m_HiuchiLengthSingleL ) ) ;
    ...
    /// <returns></returns>
    public bool GetBaseStartEndPoint( Document doc, ElementId id1, ElementId id2, ref XYZ startPnt, ref XYZ endPnt,
      ref XYZ intersectPoint, bool inner = false )
    {
      ...
      //ベース同士の交点取得
      XYZ intersectPnt =
        ClsRevitUtil.GetIntersection( cv,
          cv2 ) ; //.CalculateIntersectionPoint(tmpStPoint, tmpEdPoint, tmpStPoint2, tmpEdPoint2);
      ...
      double angleB =
        180 - ( angleA < 0 ? -angleA : angleA ) - angleC ; // (angleC < 0 ? -angleC : angleC);//Cの判定 - angleC
    ...
    /// <param name="endPnt"></param>
    /// <returns></returns>
    public bool GetBaseStartEndPoint( Document doc, XYZ base1startPnt, XYZ base1endPnt, XYZ base2startPnt,
      XYZ base2endPnt, ref XYZ startPnt, ref XYZ endPnt )

    {
      ...
      //ベース同士の交点取得
      XYZ intersectPnt =
        ClsRevitUtil.GetIntersection( cvInst,
          cvInst2 ) ; //.CalculateIntersectionPoint(tmpStPoint, tmpEdPoint, tmpStPoint2, tmpEdPoint2);
    ...
    public bool CreateTanbuParts( Document doc )
    {
      ...
      if ( m_PartsTypeKiriSide == Master.ClsHiuchiCsv.TypeNameFVP ||
           m_PartsTypeKiriSide == Master.ClsHiuchiCsv.TypeNameKaitenVP ) {
      ...
      if ( m_PartsTypeHaraSide == Master.ClsHiuchiCsv.TypeNameFVP ||
           m_PartsTypeHaraSide == Master.ClsHiuchiCsv.TypeNameKaitenVP ) {
        //自在火打ちの処理は保留
    ...
    public bool CreateTanbuParts1( Document doc, ref XYZ pS, ref XYZ pE ) //自在火打ち対応
    {
      ...
      double dB = 0 ;
      double dMain = 0 ;
      GetTotalLength( dHaraAngle, kouzaiSizeA, kouzaiSizeB, ref dMain, ref dA, ref dB,
        m_ShoriType == ShoriType.KiriKiri ) ;
      ...
      using ( Transaction tx = new Transaction( doc, "Load Family" ) ) {
        ...
        if ( sym2 != null ) {
          ...
          if ( m_PartsTypeHaraSide == Master.ClsHiuchiCsv.TypeNameFVP ) {
            ...
            XYZ dirMove =
              new XYZ( -dW * ( 1 - insecBaseAngleCos ), 0, 0 ) ; //移動長さ// * -Math.Cos(ClsGeo.Deg2Rad(dAngleB * 2))
            ElementTransformUtils.MoveElement( doc, CreatedID, dirMove ) ;
    ...
    /// <returns></returns>
    public double GetTotalLength( double dHaraAngle, string kouzaiSizeA, string kouzaiSizeB, ref double dMain,
      ref double dOffsetA, ref double dOffsetB, bool bKiri = false )
    {
    ...
    public bool CreateHojyoPeace( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, string steelSize, double dTotalLength,
      string dan )
    {
    ...
    public void SetParameter( Document doc, ElementId id )
    {
      ...
      m_CreateHoho = ClsRevitUtil.GetParameter( doc, id, "構成" ) == "シングル"
      ...
      if ( m_CreateHoho == ClsKiribariHiuchiBase.CreateHoho.Double ) {
        ...
        m_HiuchiLengthDoubleUpperL1 =
          (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "火打長さ(ダブル上)L1" ) ) ;
        m_HiuchiLengthDoubleUnderL2 =
          (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "火打長さ(ダブル下)L2" ) ) ;
        ...
        m_HiuchiLengthSingleL =
          (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetParameterDouble( doc, id, "火打長さ(シングル)L" ) ) ;
      }
    ...
    public static Curve CreateInfiniteCurve( Curve cvBase )
    {
      ...
      Curve cv = Line.CreateBound(
        new XYZ( tmpStPoint.X - ( dist * dir.X ), tmpStPoint.Y - ( dist * dir.Y ), tmpStPoint.Z ),
