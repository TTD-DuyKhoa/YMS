  ...
  public class ClsBaseList
  {
        ...
        if ( clsHaraBase.m_FlgEdit ) {
          ...
          if ( levelID != shinInstLevel.Host.Id ) {
            ...
            if ( dlgBaseList.m_originalSettingsId == id ) {
              var yoko = ClsRevitUtil.GetParameter( doc, id, "横本数" ) == "シングル"
                ? ClsHaraokoshiBase.SideNum.Single
                : ClsHaraokoshiBase.SideNum.Double ;
              var tate = ClsRevitUtil.GetParameter( doc, id, "縦本数" ) == "シングル"
        ...
        if ( clsKHB.m_FlgEdit ) {
          ...
          if ( clsKHB.GetBaseStartEndPoint( doc, id1, id2, ref pntS, ref pntE, ref insec ) ) {
            ...
            if ( ! ClsRevitUtil.JudgeOnLine( insec, pntE, tmpEdPoint ) ) {
              ...
              XYZ dirKiriLine = kiriLine.Direction ;
              newEndPoint = ClsRevitUtil.GetDistanceNewPoint( insec, -dirKiriLine,
                ClsRevitUtil.CovertFromAPI( kiriLine.Length ) ) ;
            }
    ...
    public static XYZ ShiftNumber( Document doc, XYZ dir, List<ElementId> numberList, ElementId targetNumberId )
    {
      ...
      foreach ( ElementId numberId in numberList ) {
        ...
        XYZ moveHalfPointNumber = new XYZ( pointNumber.X - dist / 2 * dir.X, pointNumber.Y - dist / 2 * dir.Y,
          pointNumber.Z ) ;
        ...
        if ( ClsRevitUtil.IsInArea( vertexList, pointTargetNumber, XYZ.BasisZ ) ) {
          ...
          //}
          //break;
          pointTargetNumber = new XYZ( pointTargetNumber.X + dist * dir.X, pointTargetNumber.Y + dist * dir.Y,
            pointTargetNumber.Z ) ;
        }
      }
