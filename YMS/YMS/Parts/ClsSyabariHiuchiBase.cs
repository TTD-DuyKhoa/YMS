...
namespace YMS.Parts
{
  public class ClsSyabariHiuchiBase
  {
    ...
    {
      var rec = Master.ClsSyabariPieceCSV.Shared.FirstOrDefault( x =>
        x.TypeName == m_PartsTypeHaraSide && x.SizeName == m_PartsSizeHaraSide ) ;
      ...
      var shaPieceSymbol =
        ClsSyabariBase.LoadSymbol( doc, Master.ClsHiuchiCsv.GetFamilyPath( m_PartsSizeSyabariSide ) ) ;
      // 斜梁火打ベースファミリの読込
      ...
      var haraSectionPlane =
        Plane.CreateByNormalAndOrigin( haraLine.Direction.CrossProduct( XYZ.BasisZ ), haraLine.Origin ) ;
      ...
      if ( m_CreateType == CreateType.AnyLengthManual ) {
        targetDirections = new XYZ[] { haraBaseRef.GlobalPoint - p0 } ;
      }
      else {
        targetDirections = new XYZ[] { haraBaseLine.Direction, -haraBaseLine.Direction } ;
      }
      ...
      using ( var transaction = new Transaction( doc, Guid.NewGuid().ToString() ) ) {
        ...
        foreach ( var targetDirection in targetDirections ) {
          ...
          var mortarLength = ClsSyabariBase.CalcMortarLength( haraSectionPlane, hiuchiBaseLine, shaBasePlane,
            shaWidthMM, shaWidthMM ) ;
          var haraPiecedata = isL ? haraPiecedataL : haraPiecedataR ;
          var pieceVertices =
            ClsSyabariBase.CalcPieceVertices( haraPiecedata, 0.0, theta2, theta1, 0.0, mortarLength ) ;
          ...
          var e3toP2 = p2 - e3 ;
          var pieceRotate = SMatrix3d.Rotation( XYZ.BasisZ, p2,
            pieceVertices.BasisY.AngleOnPlaneTo( shaSystem.YVec, XYZ.BasisZ ) ) ;
          ...
          haraPieceInstance.GetParameter( ParameterTypeId.InstanceScheduleOnlyLevelParam )
            ?.Set( shaBaseLevelId ) ; // 集計レベルの設定
          ClsRevitUtil.SetParameter( doc, haraPieceInstance.Id, ClsSyabariBase.KeyTheta2,
            ClsYMSUtil.CheckSyabariAngle( theta2 ) ) ;
          ClsRevitUtil.SetParameter( doc, haraPieceInstance.Id, ClsSyabariBase.KeyTheta1,
            ClsYMSUtil.CheckSyabariAngle( theta1 ) ) ;
          ClsRevitUtil.SetParameter( doc, haraPieceInstance.Id, ClsSyabariBase.KeyTheta0,
            ClsYMSUtil.CheckSyabariAngle( theta0 ) ) ;
          ...
          if ( ! shaPieceSymbol.IsActive ) {
            shaPieceSymbol.Activate() ;
          }

          var shaPieceInstance = doc.Create.NewFamilyInstance( shaPiecePlaneRef, p3, p1toP0, shaPieceSymbol ) ;
          shaPieceInstance.GetParameter( ParameterTypeId.InstanceScheduleOnlyLevelParam )
            ?.Set( shaBaseLevelId ) ; // 集計レベルの設定
          ...
          if ( ! hiuchiBaseSym.IsActive ) {
            hiuchiBaseSym.Activate() ;
          }

          var hiuchiBaseInstance = doc.Create.NewFamilyInstance( hiuchiBasePlane.GetReference(), e0,
            ( p3 - e0 ).Normalize(), hiuchiBaseSym ) ;
          ...
          hiuchiBaseInstance.GetParameter( ParameterTypeId.InstanceScheduleOnlyLevelParam )
            ?.Set( shaBaseLevelId ) ; // 集計レベルの設定
    ...
    /// <returns></returns>
    public bool CreateSyabariHiuchiBaseForKarikouzai( Document doc, ElementId haraokoshiId, XYZ haraPnt,
      ElementId shaBaseId )
    {
      var rec = Master.ClsSyabariPieceCSV.Shared.FirstOrDefault( x =>
        x.TypeName == m_PartsTypeHaraSide && x.SizeName == m_PartsSizeHaraSide ) ;
      ...
      //    };
      //}
      targetDirections = new XYZ[] { haraPnt - p0 } ;
    ...
    public static bool SetHaraokoshiBaseShabariBaseIdString( Document doc, ElementId hiuchiId, string haraokoshiId,
      string shabariId )
    {
      try {
