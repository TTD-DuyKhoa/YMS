...
namespace YMS.Parts
{
    ...
    public class ClsKouyaita
    {
        ...
        /// <returns></returns>
        public bool CreateKouyaita(Document doc, XYZ picPointS, XYZ picPointE, ElementId levelId, VoidVec voidVec, out VoidVec lastVoidVec, bool bIrizumi = true, bool oppMessage = true)
        {
            lastVoidVec = voidVec;
            try
            {
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    for (int i = 0; i < dCount; i++)
                    {
                        ...
                        picPoint = new XYZ(picPointS.X + (ClsRevitUtil.CovertToAPI(pitch * (i + kougoPut - cornerFlag) + cornerPitch) * soilDirection.X),
                                           picPointS.Y + (ClsRevitUtil.CovertToAPI(pitch * (i + kougoPut - cornerFlag) + cornerPitch) * soilDirection.Y),
                                           picPointS.Z);
                        ...
                        sym = ClsRevitUtil.ChangeTypeID(doc, sym, CreatedOyaID, newtypeName + "_" + m_case + "_" + m_edaNum + "_" + nKougo.ToString());
                    ...
                    if (oppMessage)
                    {
                        ...
                        if (dResult == DialogResult.Retry)
                        {
                            ...
                            for (int i = 0; i < dCount; i++)
                            {
                                ...
                                picPoint = new XYZ(picPointS.X + (ClsRevitUtil.CovertToAPI(pitch * (i + kougoPut)) * soilDirection.X),
                                ...
                                ////交互対応
                                int nKougo = (m_KougoFlg && (i % 2) == 1 ? 2 : 1);
                                kouya = ClsRevitUtil.ChangeTypeID(doc, kouya, CreatedOyaID, KOUYAITA + "_" + m_case + "_" + m_edaNum + "_" + nKougo.ToString());
                                ...
                                for (int j = 0; j < PileJointMax; j++)
                                {
                                    ...
                                    ClsRevitUtil.SetTypeParameter(kouya, "杭" + (j + 1).ToString(), ClsRevitUtil.CovertToAPI(nPileLength));
                                }
        ...
        /// <returns></returns>
        public List<ElementId> CreateKouyaita1(Document doc, XYZ picPointS, XYZ picPointE, ElementId levelId, VoidVec voidVec, ref VoidVec lastVoidVec, bool createCorner = false, bool bIrizumi = true, ElementId kabeShinId = null)
        {
            ...
            try
            {
                ...
                if(m_bIzyou && (m_size == "SP-3" || m_size == "SP-4") && m_way == 0)
                {
                    ...
                    string familyPathIkei = System.IO.Path.Combine(symbolFolpath, "山留壁関係\\02_鋼矢板\\ﾌｧﾐﾘ\\ﾊﾞﾁ矢板_" + m_size + ".rfa");
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    {
                        picPoint = new XYZ(picPointS.X + (ClsRevitUtil.CovertToAPI(pitch * (i + kougoPut - cornerFlag - ikeiFlag) + cornerPitch + ikeiPitch) * dir.X),
                                           picPointS.Y + (ClsRevitUtil.CovertToAPI(pitch * (i + kougoPut - cornerFlag - ikeiFlag) + cornerPitch + ikeiPitch) * dir.Y),
                                           picPointS.Z);
                        ...
                        if (createCorner && corner != null)//自動の時の処理
                        {
                            ...
                            {
                                cornerFlag += 1;
                                cornerPitch = ClsCommonUtils.ChangeStrToInt(Master.ClsKouyaitaCsv.GetLength(m_kouyaitaSize)) * cornerFlag;
                                sym = corner;
                                newtypeName = CORNERYAITA;
                                //コーナー継手左
                                cornerTsugite = "右";
                            ...
                            else if (nCount <= i + 1 && (createType == 0 && lastCornerFlag || (createType == 1 && lastVoidVec == VoidVec.Kussaku)) && bIrizumi)//最後
                            {
                                sym = corner;
                                newtypeName = CORNERYAITA;
                                //コーナー継手右
                            ...
                            else if (nCount <= i + 2 && (createType == 0 && cornerFlag == 0 || (createType == 1 && lastVoidVec == VoidVec.Kussaku)) && bIrizumi)//最後の一個手前
                            {
                                lastCornerFlag = true;
                                cornerFlag += 1;
                                cornerPitch = ClsCommonUtils.ChangeStrToInt(Master.ClsKouyaitaCsv.GetLength(m_kouyaitaSize)) * cornerFlag;
                        ...
                        sym = ClsRevitUtil.ChangeTypeID(doc, sym, CreatedD, newtypeName + cornerTsugite + "_" + m_case + "_" + edaban);
        ...
        /// <returns></returns>
        public ElementId CreateSingleKouyaita(Document doc, XYZ picPoint, XYZ dir, ElementId levelId, VoidVec lastVoidVec, ElementId kabeShinId = null)
        {
        ...
        public void FlipKouyaita(Document doc, ElementId id)
        {
            ...
            double dist = ClsRevitUtil.CovertToAPI(ClsCommonUtils.ChangeStrToInt(Master.ClsKouyaitaCsv.GetLength(m_size)));
        ...
        public void SetParameter(Document doc, ElementId id)
        {
            ...
            {
                int kuiLength = (int)ClsRevitUtil.CovertFromAPI(ClsRevitUtil.GetTypeParameter(sym, "杭" + (i + 1).ToString()));
                if (kuiLength <= 0)
                    break;
        ...
        /// <param name="dEx">補助線の延長</param>
        /// <returns>補助線</returns>
        public static ModelLine CreatePointHojyoLine(Document doc, XYZ pointS, XYZ dirH, XYZ Direction, double dH, double dEx)
        {
            //補助線の作成
            XYZ hojyoS = new XYZ(pointS.X + (dH * dirH.X) - (dEx * Direction.X),
                                 pointS.Y + (dH * dirH.Y) - (dEx * Direction.Y),
                                 pointS.Z);
        ...
        {
            //補助線の作成
            XYZ hojyoS = new XYZ(pointMid.X - (dEx * Direction.X),
                                 pointMid.Y - (dEx * Direction.Y),
                                 pointMid.Z);
            XYZ hojyoE = new XYZ(pointMid.X + (dEx * Direction.X),
                                 pointMid.Y + (dEx * Direction.Y),
                                 pointMid.Z);
