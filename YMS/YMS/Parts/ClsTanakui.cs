...
namespace YMS.Parts
{
  public class ClsTanakui
  {
    ...
    public bool CreateTanaKui( Document doc, XYZ insertPoint, ElementId levelId, bool renzokushori,
      ref string uniqueName )
    {
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        if ( m_Zanti == "一部埋め殺し" ) {
          ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "残置/引抜",
            m_Zanti + "(" + ( m_ZantiLength.ToString() ) + ")" ) ;
        }
        else {
        ...
        for ( int i = 0 ; i < m_TsugiteCount + 1 ; i++ ) {
          ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "杭" + ( i + 1 ),
            ClsRevitUtil.CovertToAPI( m_PileLengthList[ i ] ) ) ;
        }
    ...
    private void SetCustomData( Document doc, ElementId id )
    {
      ...
      ClsYMSUtil.SetBoltFlange( doc, id, m_TsugiteBoltSize_Flange,
        ClsCommonUtils.ChangeStrToInt( m_TsugiteBoltQuantity_Flange ) ) ;
      ClsYMSUtil.SetBoltWeb( doc, id, m_TsugiteBoltSize_Web,
        ClsCommonUtils.ChangeStrToInt( m_TsugiteBoltQuantity_Web ) ) ;
    }
    ...
    public void SetParameter( Document doc, ElementId id )
    {
      ...
      for ( int i = 0 ; i < m_TsugiteCount + 1 ; i++ ) {
        m_PileLengthList.Add(
          (int) ClsRevitUtil.CovertFromAPI( ClsRevitUtil.GetTypeParameter( inst.Symbol, "杭" + ( i + 1 ) ) ) ) ;
      }
    }
    ...
    public static Curve[] CreateBoundaryLinesFlange( FamilyInstance pileInstance, double offset, double angle = 0.0 )
    {
      ...
      // H型の1画目と3画目の座標を計算
      XYZ startPoint1 = new XYZ( centerPoint.X - ClsRevitUtil.CovertToAPI( sizeData.W ) / 2,
        centerPoint.Y + ClsRevitUtil.CovertToAPI( sizeData.H ) / 2, 0 ) ;
      XYZ endPoint1 = new XYZ( centerPoint.X - ClsRevitUtil.CovertToAPI( sizeData.W ) / 2,
        centerPoint.Y - ClsRevitUtil.CovertToAPI( sizeData.H ) / 2, 0 ) ;
      XYZ startPoint3 = new XYZ( centerPoint.X + ClsRevitUtil.CovertToAPI( sizeData.W ) / 2,
        centerPoint.Y + ClsRevitUtil.CovertToAPI( sizeData.H ) / 2, 0 ) ;
      XYZ endPoint3 = new XYZ( centerPoint.X + ClsRevitUtil.CovertToAPI( sizeData.W ) / 2,
        centerPoint.Y - ClsRevitUtil.CovertToAPI( sizeData.H ) / 2, 0 ) ;
    ...
    public static Curve[] CreateBoundaryLinesWeb( FamilyInstance pileInstance, double offset )
    {
      ...
      // H型の2画目の座標を計算
      XYZ ptEnd_Front = new XYZ( centerPoint.X - ( facingDirection.X * ClsRevitUtil.CovertToAPI( sizeData.W ) / 2 ),
        centerPoint.Y - ( facingDirection.Y * ClsRevitUtil.CovertToAPI( sizeData.W ) / 2 ), XYZ.Zero.Z ) ;
      XYZ ptEnd_Back = new XYZ( centerPoint.X + ( facingDirection.X * ClsRevitUtil.CovertToAPI( sizeData.W ) / 2 ),
        centerPoint.Y + ( facingDirection.Y * ClsRevitUtil.CovertToAPI( sizeData.W ) / 2 ), XYZ.Zero.Z ) ;
