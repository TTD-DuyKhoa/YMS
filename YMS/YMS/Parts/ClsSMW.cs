...
namespace YMS.Parts
{
    public class ClsSMW
    {
        ...
        /// <returns></returns>
        public bool CreateSMW(Document doc, XYZ picPointS, XYZ picPointE, ElementId levelId, bool bIrizumi = true, bool defVoid = true, ElementId kabeShinId = null)
        {
            try
            {
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    {
                        if ((!bIrizumi && i == 0) || (!m_bTanbuS && i == 0 && kuiNoSoilCountS == 0))// || (!m_bTanbuE && dCount - 1 <= i)) 
                        {
                            continue;
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        picPoint = new XYZ(picPointE.X + (ClsRevitUtil.CovertToAPI(soil * i) * oopDir.X),
                                           picPointE.Y + (ClsRevitUtil.CovertToAPI(soil * i) * oopDir.Y),
                                           picPointE.Z);
                        ...
                        //配置パターン
                        if ((x == 0 && m_putPtnFlag == 3 && i == 2) || i == 1 || (m_way == 0 && bIrizumi && m_bCorner && i == 0))
                        {
                            if(x == 0 && m_putPtnFlag == 3 && i == 1 && !ClsGeo.GEO_EQ(new XYZ(picPointE.X + (ClsRevitUtil.CovertToAPI(soil * 2) * oopDir.X),
                                           picPointE.Y + (ClsRevitUtil.CovertToAPI(soil * 2) * oopDir.Y),
                                           picPointE.Z), lastPoint))
                            {
                                if (putPtn == 0)
                                    putPtn = 1;
                                else
                    ...
                    try
                    {
                        outLine = CreateKabeHojyoLine(doc, originalPointS, picPointE, dH, ClsRevitUtil.CovertToAPI(m_dia));// / 2));//外側
                        inLine = CreateKabeHojyoLine(doc, originalPointS, picPointE, -dH, ClsRevitUtil.CovertToAPI(m_dia));// / 2));//内側
        ...
        /// <param name="length"></param>
        /// <returns></returns>
        public bool CreateSingleSMW(Document doc, XYZ picPointS, XYZ soilDirection, ElementId levelId, bool bIrizumi = true, bool bSoil = true, bool bH = true, ElementId kabeShinId = null, double apexAngle = 0.0)
        {
            try
            {
                string symbolFolpath = ClsZumenInfo.GetYMSFolder();
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    if (bH)
                    {
                        ...
                        if (!bIrizumi)//出隅杭
                        {
                            double oneSide = ClsRevitUtil.CalculateIsoscelesTriangleOneSide(dFlangeWidth * 2, apexAngle, out double baseAngle);
                        ...
                        ////補助線の作成
                        double dH = ClsRevitUtil.CovertToAPI(dEx);
                        outLine = CreatePointHojyoLine(doc, picPointS, -inst.FacingOrientation, soilDirection, dH, ClsRevitUtil.CovertToAPI(m_dia / 2));//外側
                        inLine = CreatePointHojyoLine(doc, picPointS, -inst.FacingOrientation, soilDirection, -dH, ClsRevitUtil.CovertToAPI(m_dia / 2));//内側
                    }
                    //変更が加わるメッセージ原因不明
                    t.Commit();
                }
                if (outLine != null && inLine != null)
                    ////壁くり抜き処理
                    if (m_bVoid) ClsVoid.DoVoid(doc, selId, inLine, m_void, m_dia / 2, levelId);
        ...
        public void SetParameter(Document doc, ElementId id)
        {
            ...
            foreach (ElementId soilId in soilList)
            {
                ...
                if (m_case == CASE && m_edaNum == edaNum)
                {
                    ...
                    //m_soil = 0;
                    m_soilTop = (int)ClsRevitUtil.CovertFromAPI(ClsRevitUtil.GetParameterDouble(doc, soilId, ClsGlobal.m_refLvTop));
