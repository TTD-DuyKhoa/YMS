...
namespace YMS.Parts
{
  public class ClsHaraokoshiBase
  {
    ...
    public bool IsMega
    {
      get { return m_kouzaiType == Master.ClsYamadomeCsv.TypeMegaBeam ; }
    }
    ...
    public bool CreateHaraokoshiBase( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, double shift )
    {
      ...
      tmpStPoint = new XYZ( tmpStPoint.X - ( dEx * Direction.Y ), tmpStPoint.Y + ( dEx * Direction.X ), tmpStPoint.Z ) ;
      tmpEdPoint = new XYZ( tmpEdPoint.X - ( dEx * Direction.Y ), tmpEdPoint.Y + ( dEx * Direction.X ), tmpEdPoint.Z ) ;
    ...
    public void SetParameter( Document doc, ElementId id )
    {
      ...
      m_yoko = ClsRevitUtil.GetParameter( doc, id, "横本数" ) == "シングル"
        ? ClsHaraokoshiBase.SideNum.Single
        : ClsHaraokoshiBase.SideNum.Double ;
      m_tate = ClsRevitUtil.GetParameter( doc, id, "縦本数" ) == "シングル"
    ...
    public bool CreateHojyoPeace( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, double haraokoshiTateOffset,
      double oop = 1 )
    {
        ...
        if ( kouzaiSym != null ) {
          ...
          for ( int i = 0 ; i < megaBeamCount ; i++ ) {
            ...
            ElementTransformUtils.MoveElement( doc, CreatedID,
              leftVector * ClsRevitUtil.ConvertDoubleGeo2Revit( 200.0 ) ) ;
            ClsKariKouzai.SetBaseId( doc, CreatedID, baseID ) ;
            ClsKariKouzai.SetKariKouzaiFlag( doc, CreatedID ) ;
    ...
    public bool CreateGapTyousei( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, double dKouzaiSize )
    {
      ...
      XYZ dir = Line.CreateBound( tmpStPoint, tmpEdPoint ).Direction ;
      tmpStPoint = new XYZ( tmpStPoint.X + ( dKouzaiSize * dir.Y ), tmpStPoint.Y - ( dKouzaiSize * dir.X ),
        tmpStPoint.Z ) ;
      tmpEdPoint = new XYZ( tmpEdPoint.X + ( dKouzaiSize * dir.Y ), tmpEdPoint.Y - ( dKouzaiSize * dir.X ),
        tmpEdPoint.Z ) ;
      ...
      if ( m_gapTyouseiType == SukimaH ) {
        ...
        familyName = RevitUtil.ClsRevitUtil.GetFamilyName( path ) ;
        size = ClsRevitUtil.CovertToAPI(
          ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou( familyName, 2 ) ) / 2 ) ;
      }
      else {
        return false ;
      }
    ...
    public static List<GetIntersectionBase2Result> GetIntersectionBase2( Document doc, ElementId id,
      ElementId levelId = null )
    {
      ...
      foreach ( ElementId tgId in targetFamilies ) {
        ...
        if ( ClsGeo.GEO_EQ0( p0.DistanceTo( p1 ) ) ) {
          insecIdList.Add( new GetIntersectionBase2Result { Id = tgId, Point = p1 } ) ;
        }
        //XYZ insec = ClsRevitUtil.GetIntersection(cvBase, tgCv);
        //if (insec != null)
        //{
    ...
    /// <param name="levelId"></param>
    /// <returns></returns>
    public static List<GetIntersectionBase2Result> GetIntersectionBase2ModelLine( Document doc, ElementId id,
      ElementId levelId = null )
    {
      var insecIdList = new List<GetIntersectionBase2Result>() ;
