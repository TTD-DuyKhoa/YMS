...
namespace YMS.Parts
{
  public class ClsSyabariTsunagizaiBase
  {
    ...
    public bool CreateSyabariTsunagizaiBase( Document doc, XYZ tmpStPoint, XYZ tmpEdPoint, XYZ tmpThirdPoint,
      ElementId levelId )
    {
      //参照面の作成
    ...
    public bool CreateSyabariTsunagizai( Document doc, ElementId id1, ElementId id2, ElementId id3 )
    {
      ...
      foreach ( var pickResult in pickResultSet ) {
        ...
        var sectionTop =
          ClsYMSUtil.CalcShabariSectionTop( shabariBaseLine, shabariBasePlane, shabariW, shabariH, sectionPlane ) ;
        if ( sectionTop != null ) {
      ...
      using ( var transaction = new Transaction( doc, Guid.NewGuid().ToString() ) ) {
        ...
        if ( topPoints.Count == 2 ) {
          ...
          var top13Line =
            Line.CreateBound( tmpStPoint,
              tmpStPoint - ClsRevitUtil.CovertToAPI( 10000 ) * XYZ.BasisZ ) ; //鋼材サイズが500より大きいものが無く交点が見つかればよいため500
          ...
          ElementId createId = ClsRevitUtil.Create( doc, tmpStPoint, ( tmpEdPoint - tmpStPoint ).Normalize(),
            referencePlane, sym ) ;
    ...
    public static void EnsureReferencePlaneRightHanded( ref XYZ bubbleEnd, ref XYZ freeEnd, ref XYZ thirdPoint )
    {
    ...
    public static (XYZ, XYZ) GetSyabariTsunagizaiPoint( Document doc, ElementId id1, ElementId id2, ElementId id3 )
    {
      ...
      if ( topPoints.Count == 2 ) {
        ...
        var top13Line =
          Line.CreateBound( tmpStPoint,
            tmpStPoint - ClsRevitUtil.CovertToAPI( 500.0 ) * XYZ.BasisZ ) ; //鋼材サイズが500より大きいものが無く交点が見つかればよいため500
        var insec13 = ClsRevitUtil.GetIntersection( line3, top13Line ) ;
    ...
    public bool CreateBruman( Document doc, ElementId id1, ElementId id2, ElementId id3 )
    {
      ...
      // 斜梁つなぎベースを鋼材分ずらした位置から鉛直方向の断面平面を作成
      {
        ...
        tmpStPoint = new XYZ( tmpStPoint.X + ( sizeB * dir.Y ), tmpStPoint.Y - ( sizeB * dir.X ), tmpStPoint.Z ) ;
        tmpEdPoint = new XYZ( tmpEdPoint.X + ( sizeB * dir.Y ), tmpEdPoint.Y - ( sizeB * dir.X ), tmpEdPoint.Z ) ;
      ...
      foreach ( var pickResult in pickResultSet ) {
        ...
        // 斜梁断面の端点の中で Z 成分が最大の点を取得
        var sectionTop = ClsYMSUtil.CalcShabariSectionTop( shabariBaseLine, shabariBasePlane, shabariW, shabariH,
          sectionPlane, true ) ;
        if ( sectionTop != null ) {
          topPoints.Add( sectionTop ) ;
        }
      ...
      using ( var t = new Transaction( doc, Guid.NewGuid().ToString() ) ) {
        ...
        if ( topPoints.Count == 2 ) {
          ...
          var vec1 = new XYZ() ;
          if ( ClsGeo.IsLeft( topPoints[ 0 ] - shabariBaseLine1.GetEndPoint( 0 ),
                shabariBaseLine1.GetEndPoint( 1 ) - shabariBaseLine1.GetEndPoint( 0 ) ) ) {
            vec1 = ( ( topPoints[ 0 ] - topPoints[ 1 ] ).Normalize() +
                     ( shabariBaseLine1.GetEndPoint( 0 ) - shabariBaseLine1.GetEndPoint( 1 ) ).Normalize() )
              .Normalize() ;
          }
          else {
            vec1 = ( ( topPoints[ 1 ] - topPoints[ 0 ] ).Normalize() +
                     ( shabariBaseLine1.GetEndPoint( 0 ) - shabariBaseLine1.GetEndPoint( 1 ) ).Normalize() )
              .Normalize() ;
          }
          ...
          var vec2 = new XYZ() ;
          if ( ClsGeo.IsLeft( topPoints[ 1 ] - shabariBaseLine2.GetEndPoint( 0 ),
                shabariBaseLine2.GetEndPoint( 1 ) - shabariBaseLine2.GetEndPoint( 0 ) ) ) {
            vec2 = ( ( topPoints[ 0 ] - topPoints[ 1 ] ).Normalize() +
                     ( shabariBaseLine2.GetEndPoint( 0 ) - shabariBaseLine2.GetEndPoint( 1 ) ).Normalize() )
              .Normalize() ;
          }
          else {
            vec2 = ( ( topPoints[ 1 ] - topPoints[ 0 ] ).Normalize() +
                     ( shabariBaseLine2.GetEndPoint( 0 ) - shabariBaseLine2.GetEndPoint( 1 ) ).Normalize() )
              .Normalize() ;
          }
