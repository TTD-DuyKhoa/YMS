...
namespace YMS.Parts
{
  ...
  public class ClsCornerHiuchiBase
  {
    ...
    public bool GetBaseStartEndPoint( Document doc, ref XYZ startPnt, ref XYZ endPnt )
    {
      ...
      //腹起同士の交点取得
      XYZ intersectPnt =
        ClsRevitUtil.GetIntersection( lCurve.Curve,
          lCurve2.Curve ) ; //.CalculateIntersectionPoint(tmpStPoint, tmpEdPoint, tmpStPoint2, tmpEdPoint2);
      ...
      if ( intersectPnt == null ) {
        ( double dist1, double dist2 ) =
          ClsRevitUtil.GetDistanceToIntersection( tmpStPoint, tmpEdPoint, tmpStPoint2, tmpEdPoint2 ) ;
        Curve InfiniteCurve =
          ClsRevitUtil.CreateInfiniteCurve( tmpStPoint, tmpEdPoint, dist1 ) ; //延長したCurveをこの後の処理にも使用した方がよいかも
    ...
    public bool CreateTanbuParts( Document doc, ref XYZ pS, ref XYZ pE ) //自在火打ち対応
    {
      ...
      //角度
      double dHaraAngle =
        ClsRevitUtil.CalculateAngleBetweenLines( tmpStPointA, tmpEdPointA, tmpStPointB,
          tmpEdPointB ) ; //, tmpEdPointB, tmpStPointB);
    ...
    /// <param name="endPnt"></param>
    /// <returns></returns>
    public bool GetBaseStartEndPoint( Document doc, XYZ haraBase1startPnt, XYZ haraBase1endPnt, XYZ haraBase2startPnt,
      XYZ haraBase2endPnt, ref XYZ startPnt, ref XYZ endPnt )
    ...
    public bool CreateCornerHiuchiCheck( UIDocument uidoc )
    {
      ...
      if ( m_ShoriType == ShoriType.Manual ) {
        ...
        if ( m_HaraokoshiBaseID1 == null && m_HaraokoshiBaseID2 == null ) {
          ElementId id1 = null ;
          if ( ! ClsHaraokoshiBase.PickBaseObject( uidoc, ref id1,
                "隅火打ベースを配置する腹起ベース" ) ) //ClsRevitUtil.PickObject(uidoc, "隅火打ベースを配置する腹起ベース　1本目", "腹起ベース", ref id1))
          {
            return false ;
          }
          ...
          try {
            ...
            if ( GetBaseStartEndPoint( doc, ref pntS, ref pntE ) ) {
              CreateCornerHiuchi( doc, pntS, pntE, levelID,
                ClsRevitUtil.GetParameter( doc, m_HaraokoshiBaseID1, "段" ) ) ;
            }
    ...
    public void SetParameter( Document doc, ElementId id )
    {
      ...
      m_Kousei = ClsRevitUtil.GetParameter( doc, id, "構成" ) == "シングル"
