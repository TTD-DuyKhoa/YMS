  ...
  public class ClsAtamaTsunagi
  {
    ...
    public bool CreateAtamaTsunagiZai( Document doc, ElementId oyakuiIdSt, ElementId oyakuiIdEd )
    {
      ...
      if ( oyaSymName.Contains( "杭" ) ) {
        oyaSize = ClsRevitUtil.CovertToAPI(
          ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou1( oyaSymName, 1 ) ) / 2 ) ;
      }
      else if ( oyaSymName.Contains( "鋼矢板" ) ) {
        oyaSize = ClsRevitUtil.CovertToAPI(
          ClsCommonUtils.ChangeStrToDbl( Master.ClsKouyaitaCsv.GetHeight( oyaSymName ) ) ) ;
      ...
      double atamaSize =
        ClsRevitUtil.CovertToAPI( ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou( familyName, 1 ) ) /
                                  2 ) ;
      if ( m_KouzaiType == "アングル" ) {
        atamaSize = atamaSize - atamaSize ;
      }
      ...
      if ( m_KouzaiType.Contains( "H形鋼" ) || m_KouzaiType == "主材" ) {
        if ( m_ConfigurationDirection == ConfigurationDirection.Front ) {
          ptAtamaSt = new XYZ( ptOyaSt.X + ( offset * instOyaStFacingOrientation.X ),
            ptOyaSt.Y + ( offset * instOyaStFacingOrientation.Y ), insertPointZ1 ) ;
          ptAtamaEd = new XYZ( ptOyaEd.X + ( offset * instOyaEdFacingOrientation.X ),
            ptOyaEd.Y + ( offset * instOyaEdFacingOrientation.Y ), insertPointZ2 ) ;
        }
        else {
          ptAtamaSt = new XYZ( ptOyaSt.X - ( offset * instOyaStFacingOrientation.X ),
            ptOyaSt.Y - ( offset * instOyaStFacingOrientation.Y ), insertPointZ1 ) ;
        ...
        if ( m_ChannelDirection.Equals( ChannelDirection.DirectionVertical ) ) {
          ptAtamaSt = new XYZ( ptOyaSt.X + ( oyaSize * instOyaStFacingOrientation.X ),
            ptOyaSt.Y + ( oyaSize * instOyaStFacingOrientation.Y ), insertPointZ1 ) ;
          ptAtamaEd = new XYZ( ptOyaEd.X + ( oyaSize * instOyaEdFacingOrientation.X ),
            ptOyaEd.Y + ( oyaSize * instOyaEdFacingOrientation.Y ), insertPointZ2 ) ;
        }
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        if ( m_CreateType == CreateType.FromToP ) {
          ...
          if ( m_KouzaiType == "アングル" ) {
            ...
            atamaSize = ClsRevitUtil.CovertToAPI(
              ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou( familyName, 1 ) ) ) ;
    ...
    public bool CreateAtamaTsunagiZaiSyuzai( Document doc, ElementId oyakuiIdSt, ElementId oyakuiIdEd )
    {
      ...
      while ( true ) {
        ...
        // 頭ツナギ材のサイズ分だけオフセットする
        double atamaSize = ClsRevitUtil.CovertToAPI(
          ( ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetSyuzaiSize( sym.FamilyName ).Replace( "HA", "" ) ) * 10 ) /
          2 ) ;
        XYZ ptAtamaSt = XYZ.Zero ;
        XYZ ptAtamaEd = XYZ.Zero ;
        if ( m_ConfigurationDirection == ConfigurationDirection.Front ) {
          ptAtamaSt = new XYZ( ptOyaSt.X + ( atamaSize * instOyaStFacingOrientation.X ),
            ptOyaSt.Y + ( atamaSize * instOyaStFacingOrientation.Y ), insertPointZ1 ) ;
          ptAtamaEd = new XYZ( ptOyaEd.X + ( atamaSize * instOyaEdFacingOrientation.X ),
            ptOyaEd.Y + ( atamaSize * instOyaEdFacingOrientation.Y ), insertPointZ2 ) ;
        }
        else {
          ptAtamaSt = new XYZ( ptOyaSt.X - ( atamaSize * instOyaStFacingOrientation.X ),
            ptOyaSt.Y - ( atamaSize * instOyaStFacingOrientation.Y ), insertPointZ1 ) ;
        ...
        using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
          ...
          if ( createdId != null ) {
            ...
            // 作図地点の更新
            newPoint = ptAtamaSt + ( vecAtama *
                                     ClsRevitUtil.ConvertDoubleGeo2Revit(
                                       ( dlgCreateAtamatsunagiSyuzai.m_Length * 1000 ) ) ) ;
          }
        }
      }
    ...
    /// <returns></returns>
    public bool CreateAtamaTsunagiHojoZai( Document doc, ElementId idOyakui, ElementId idAtama, bool isAuto = false,
      double interval = 0.0, bool isSide = false )
    {
      // 頭ツナギ材の情報を取得する
        ...
        atamaHeight =
          ClsRevitUtil.CovertToAPI(
            ( ClsCommonUtils.ChangeStrToDbl(
              ClsYMSUtil.GetSyuzaiSize( instAtama.Symbol.FamilyName ).Replace( "HA", "" ) ) * 10 ) / 2 ) ;
      }
      else if ( m_KouzaiType == "アングル" ) {
        atamaHeight =
          ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou( instAtama.Symbol.FamilyName, 1 ) ) ) ;
      }
      else {
        atamaHeight =
          ClsRevitUtil.CovertToAPI(
            ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetKouzaiSizeSunpou( instAtama.Symbol.FamilyName, 1 ) ) / 2 ) ;
      }
      ...
      if ( m_Bracket != "" ) {
        ...
        if ( isAuto ) {
          ...
          if ( oyaSymName.Contains( "杭" ) ) {
            ...
            oyaSize = ClsRevitUtil.CovertToAPI(
              ClsCommonUtils.ChangeStrToDbl( Master.ClsKouyaitaCsv.GetWidth( oyaSymName ) ) / 2 ) ;
          }
          ...
          atamaPileSpacingLength = atamaTotalLength - ( ClsRevitUtil.CovertToAPI( m_TsukidashiSt ) +
                                                        ClsRevitUtil.CovertToAPI( m_TsukidashiEd ) + ( oyaSize * 2 ) ) ;
            ...
            if ( m_Bracket == BL30 || m_Bracket == L75X75X6X150 || m_Bracket == L75X75X6X300 ) {
              using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
                ...
                ElementTransformUtils.RotateElement( doc, createdId,
                  Line.CreateBound( insertPoint, insertPoint + XYZ.BasisZ ),
                  angle + danmenAngleLow ) ; //#33967 ブラケットは断面固定
              ...
              if ( m_KouzaiType == "主材" ) {
                extLength = ClsRevitUtil.CovertToAPI(
                  ( ClsCommonUtils.ChangeStrToDbl( ClsYMSUtil.GetSyuzaiSize( instAtama.Symbol.FamilyName )
                    .Replace( "HA", "" ) ) * 10 ) + 100.0 ) ;
              }
              else {
                extLength = ClsRevitUtil.CovertToAPI(
        ...
        foreach ( var id in oyakuiIds ) {
          ...
          if ( IsPointOnLine( trgInsertPoint, lineAtama ) ) {
            using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
              ...
              ElementTransformUtils.RotateElement( doc, createdId,
                Line.CreateBound( trgInsertPoint, trgInsertPoint + XYZ.BasisZ ), angle + danmenAngle ) ;
        ...
        foreach ( var id in oyakuiIds ) {
          ...
          if ( IsPointOnLine( trgInsertPoint, lineAtama ) ) {
            using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
              ...
              ElementTransformUtils.RotateElement( doc, createdId,
                Line.CreateBound( trgInsertPoint, trgInsertPoint + XYZ.BasisZ ), angle ) ;
    ...
    private Line GetLineAtama( XYZ ptAtamaSt, XYZ ptAtamaEd, XYZ ptOyakui, double oyaSize, double size )
    {
      ...
      double atamaTotalLength = ptAtamaSt.DistanceTo( ptAtamaEd ) ;
      double atamaPileSpacingLength = atamaTotalLength - ( ClsRevitUtil.CovertToAPI( m_TsukidashiSt ) +
                                                           ClsRevitUtil.CovertToAPI( m_TsukidashiEd ) +
                                                           ( oyaSize * 2 ) ) ;
    ...
    private static XYZ GetInsertPoint_Kouyaita( Document doc, FamilyInstance instKouyaita, string kouyaitaSize,
      ref XYZ centerPoint )
    {
      XYZ ptKouyaita = instKouyaita.GetTransform().Origin ;
      XYZ vecKouyaita = instKouyaita.FacingOrientation ;
    ...
    /// <returns></returns>
    private static XYZ GetInsertPoint_Kouyaita( Document doc, FamilyInstance instKouyaita, XYZ ptSt, XYZ ptEd,
      string kouyaitaSize )
    {
      XYZ ptKouyaita = instKouyaita.GetTransform().Origin ;
      XYZ vecKouyaita = instKouyaita.FacingOrientation ;
    ...
    private static bool IsPointOnLine( XYZ point, Line line )
    {
      ...
      // ポイントから線分への最短距離を計算
      double distanceToLine = Math.Abs( startPoint.Subtract( point ).CrossProduct( lineDirection ).GetLength() ) /
                              lineDirection.GetLength() ;
