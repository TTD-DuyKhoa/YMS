...
namespace YMS.Parts
{
    public class ClsOyakui
    {
        ...
        public bool CreateOyakui(Document doc, XYZ picPointS, XYZ picPointE, ElementId levelId, bool bIrizumi = true, ElementId kabeShinId = null, bool defVoid = true)
        {
            ...
            try
            {
                ...
                picPointS = new XYZ(picPointS.X + (ClsRevitUtil.CovertToAPI(refPDist) * direction.X),
                                    picPointS.Y + (ClsRevitUtil.CovertToAPI(refPDist) * direction.Y),
                                    picPointS.Z);
                length -= refPDist;
                ...
                if (m_bYokoyaita)
                {
                    for (int i = 0; i < oyaIdList.Count - 1; i++)
                    {
                        ...
                        clsYokoyaita.CreateMokuyaita(doc, oyaIdList[i], oyaIdList[i + 1], m_putPitch, ClsRevitUtil.CovertToAPI(m_void), levelId, ClsRevitUtil.CovertToAPI(m_HTop));
        ...
        public bool CreateSingleOyakui(Document doc, XYZ picPointS, XYZ direction, ElementId levelId, bool bIrizumi = true, bool bCorner = false, ElementId kabeShinId = null, double apexAngle = 0.0)
        {
            try
            {
                string symbolFolpath = ClsZumenInfo.GetYMSFolder();
                ...
                using (Transaction t = new Transaction(doc, Guid.NewGuid().GetHashCode().ToString()))
                {
                    ...
                    if (bCorner)
                    {
                        ...
                        else if (!bIrizumi && m_putPosFlag == 0)//出隅かつ杭面
                        {
                            ...
                            ClsRevitUtil.MoveFamilyInstance(inst, -(oneSide + dEx), inst.FacingOrientation);// - dEx * 2, inst.FacingOrientation);
                        }
        ...
        public bool CreateCornerOyakui(Document doc, XYZ picPointS, XYZ direction, ElementId levelId, bool bIrizumi = true, bool bCorner = false, ElementId kabeShinId = null)
        {
            try
            {
                string symbolFolpath = ClsZumenInfo.GetYMSFolder();
