...
namespace YMS.Parts
{
  public class ClsDesumibuHokyouzai
  {
    ...
    /// <returns></returns>
    public bool CreateDesumibuHokyouzai( Document doc, ElementId levelId, XYZ insertPoint, double sumikiriLength,
      double length1, double length2, double voidDep, double angle, double rotation )
    {
      try {
        ...
        using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
          ...
          for ( int i = 0 ; i < count ; i++ ) {
            ...
            ClsRevitUtil.SetParameter( doc, CreatedId, ClsGlobal.m_refLvTop,
              -ClsRevitUtil.CovertToAPI( m_Top ) - ( ClsRevitUtil.CovertToAPI( pitch * i ) ) ) ;

            sym = ClsRevitUtil.ChangeTypeID( doc, sym, CreatedId,
              familyName + ClsGeo.FloorAtDigitAdjust( 0, ClsRevitUtil.CovertFromAPI( length1 ) ).ToString() + "+" +
