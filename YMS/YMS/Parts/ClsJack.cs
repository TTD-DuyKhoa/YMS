...
namespace YMS.Parts
{
  public class ClsJack
  {
    ...
    /// <returns></returns>
    public bool CreateJack( Document doc, ElementId baseId, XYZ baseSp, XYZ baseEp, XYZ insertPoint,
      bool useAngle = false, double angle = 0.0, double remainingLength = 0.0, string flagName = "通常" )
    {
        ...
        using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
          ...
          if ( instBase.Host is ReferencePlane referencePlane ) {
            ...
            ClsRevitUtil.SetParameter( doc, createdId, "集計レベル",
              ClsRevitUtil.GetParameterElementId( doc, baseId, "集計レベル" ) ) ;
    ...
    /// <param name="flagName"></param>
    /// <returns></returns>
    public bool CreateJacka( Document doc, ElementId baseId, XYZ baseSp, XYZ baseEp, XYZ insertPoint,
      bool useAngle = false, double angle = 0.0, double remainingLength = 0.0, string flagName = "通常" )
    {
      FamilyInstance instBase = doc.GetElement( baseId ) as FamilyInstance ;
