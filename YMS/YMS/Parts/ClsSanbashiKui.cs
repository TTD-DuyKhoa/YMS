...
namespace YMS.Parts
{
  public class ClsSanbashiKui
  {
    ...
    /// <returns></returns>
    public bool CreateSanbashiKui( Document doc, XYZ insertPoint, ElementId levelId, bool renzokushori,
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        ...
        if ( m_PileType == PileType.DanmenHenka ) {
          ...
          for ( int i = 0 ; i < m_TsugiteCount ; i++ ) {
            ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "上杭" + ( i + 1 ),
          ...
          for ( int i = 0 ; i < m_TsugiteCount2 ; i++ ) {
            ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "下杭" + ( i + 1 ),
              ClsRevitUtil.CovertToAPI( m_PileLengthList2[ i ] ) ) ;
          }

          ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "下杭長さ",
            ClsRevitUtil.CovertToAPI( m_PileTotalLength_Bottom ) ) ;

          ClsRevitUtil.SetTypeParameter( duplicatedSymbol, "エンドプレート厚さ",
            ClsRevitUtil.CovertToAPI( m_EndPlateThickness ) ) ;
        }
        else {
    ...
    private void SetCustomData( Document doc, ElementId id )
    {
      ...
      ClsYMSUtil.SetBoltFlange( doc, id, m_TsugiteBoltSize_Flange2,
        ClsCommonUtils.ChangeStrToInt( m_TsugiteBoltQuantity_Flange2 ) ) ;
      ClsYMSUtil.SetBoltWeb( doc, id, m_TsugiteBoltSize_Web,
        ClsCommonUtils.ChangeStrToInt( m_TsugiteBoltQuantity_Web ) ) ;
      ClsYMSUtil.SetBoltWeb( doc, id, m_TsugiteBoltSize_Web2,
        ClsCommonUtils.ChangeStrToInt( m_TsugiteBoltQuantity_Web2 ) ) ;
    }
