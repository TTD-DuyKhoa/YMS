...
namespace YMS.Parts
{
  public class ClsKiribariTsugiBase
  {
    ...
    public bool CreateKiribariTsugiBase( Document doc, List<ElementId> modelLineIdList, List<string> danList,
      ElementId levelID )
    {
      ...
      foreach ( ElementId modelLineId in modelLineIdList ) {
        ...
        using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
          ...
          if ( m_Kousei == Kousei.Double ) {
            ...
            ClsRevitUtil.SetParameter( doc, CreatedID, "切梁側/切梁繋ぎ長さ",
              ClsRevitUtil.CovertToAPI( m_KiriSideTsunagiLength ) ) ;
            ...
            ClsRevitUtil.SetParameter( doc, CreatedID, "腹起側/切梁繋ぎ長さ",
              ClsRevitUtil.CovertToAPI( m_HaraSideTsunagiLength ) ) ;
    ...
    public bool CreateTanbuParts( Document doc, ref XYZ pS, ref XYZ pE ) //自在火打ち対応
    {
      ...
      double sizeA = ClsRevitUtil.CovertToAPI( Master.ClsHiuchiCsv.GetWidth( m_KiriSideParts, tanbuPieceSize ) / 2 ) +
                     diff ;
      double atumiSHG = -ClsRevitUtil.CovertToAPI( 12.0 ) ;
      //シンボル読込※タイプを持っている
      if ( m_KiriSideParts == Master.ClsHiuchiCsv.TypeNameFVP ||
           m_KiriSideParts == Master.ClsHiuchiCsv.TypeNameKaitenVP ) {
        //↑ここには入らないのでは？
      ...
      double sizeB = ClsRevitUtil.CovertToAPI( Master.ClsHiuchiCsv.GetWidth( m_HaraSideParts, tanbuPieceSize2 ) / 2 ) +
                     diff2 ;
      if ( m_HaraSideParts == Master.ClsHiuchiCsv.TypeNameFVP ||
           m_HaraSideParts == Master.ClsHiuchiCsv.TypeNameKaitenVP ) {
        //自在火打ちの処理は保留
        ...
        if ( BaseEndIsKiribaribase ) {
          ...
          if ( sym2 != null ) {
            if ( m_HaraSideParts == Master.ClsHiuchiCsv.TypeNameFVP ) {
              ...
              double dW = RevitUtil.ClsRevitUtil.CovertToAPI( ClsYMSUtil.GetKouzaiHabaFromBase( doc, baseID ) ) ;
              if ( m_Kousei == Kousei.Double )
                dW = RevitUtil.ClsRevitUtil.CovertToAPI(
                  ClsYMSUtil.GetKouzaiHabaFromBase( doc, baseID, bDoubleUNorHara: true ) ) ;
              XYZ dirMove = new XYZ( -dW / 2, 0, 0 ) ; //移動長さ
              ElementTransformUtils.MoveElement( doc, CreatedID, dirMove ) ;
    ...
    /// <returns></returns>
    public double GetTotalLength( double dStartAngle, double dEndAngle, bool BaseEndIsKiribaribase, ref double dMain,
      ref double dOffsetA, ref double dOffsetB )
    {
