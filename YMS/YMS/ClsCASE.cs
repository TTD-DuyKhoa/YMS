...
namespace YMS
{
  public class ClsCASE
  {
    ...
    public class stCASEKoyaita
    {
      ...
      public static List<stCASEKoyaita> OverLapCASE( List<stCASEKoyaita> stCASEKoyaitaList, bool list = false )
      {
        ...
        for ( int i = 0 ; i < stCASEKoyaitaList.Count ; i++ ) {
          ...
          for ( int j = 0 ; j < dataList.Count ; j++ ) {
            if ( dataList[ j ].CASE == stCASEKoyaitaList[ i ].CASE &&
                 dataList[ j ].Edaban == stCASEKoyaitaList[ i ].Edaban ) {
              bFlag = true ;
              break ;
            }
          }
          ...
          for ( int j = 0 ; j < stCASEKoyaitaList.Count ; j++ ) {
            if ( stCASEKoyaitaList[ i ].CASE == stCASEKoyaitaList[ j ].CASE &&
                 stCASEKoyaitaList[ i ].Edaban == stCASEKoyaitaList[ j ].Edaban ) {
          ...
          foreach ( ElementId id in allKouyaitaList ) {
            ...
            if ( stCASEKoyaita.CASE == clsKouyaita.m_case.ToString() && stCASEKoyaita.Edaban == clsKouyaita.m_edaNum ) {
              ...
              using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
                ...
                sym = ClsRevitUtil.ChangeTypeID( doc, sym, id,
                  name + "_" + stCASEKoyaita.NewCASE + "_" + stCASEKoyaita.NewEdaban ) ;
                ...
                if ( stCASEKoyaita.zanchi == "一部埋め殺し" )
                  ClsRevitUtil.SetTypeParameter( sym, "残置/引抜",
                    stCASEKoyaita.zanchi + "(" + stCASEKoyaita.zanchiLength + ")" ) ;
                else
                ...
                for ( int i = 0 ; i < PileJointMax ; i++ ) {
                  ...
                  ClsRevitUtil.SetTypeParameter( sym, "杭" + ( i + 1 ).ToString(),
      ...
      public static void CreateCASE( Document doc, List<stCASEKoyaita> stCASEKoyaitaList,
        List<ElementId> allKouyaitaList )
      {
    ...
    public class stCASEOyagui
    {
      ...
      public static List<stCASEOyagui> OverLapCASE( List<stCASEOyagui> stCASEOyaguiList, bool list = false )
      {
        ...
        for ( int i = 0 ; i < stCASEOyaguiList.Count ; i++ ) {
          bool bFlag = false ;
          for ( int j = 0 ; j < dataList.Count ; j++ ) {
            if ( dataList[ j ].CASE == stCASEOyaguiList[ i ].CASE &&
                 dataList[ j ].Edaban == stCASEOyaguiList[ i ].Edaban ) {
              bFlag = true ;
              break ;
            }
          }
          ...
          for ( int j = 0 ; j < stCASEOyaguiList.Count ; j++ ) {
            if ( stCASEOyaguiList[ i ].CASE == stCASEOyaguiList[ j ].CASE &&
                 stCASEOyaguiList[ i ].Edaban == stCASEOyaguiList[ j ].Edaban ) {
      ...
      public static List<stCASESMW> OverLapCASE( List<stCASESMW> stCASESMWList, bool list = false )
      {
        ...
        for ( int i = 0 ; i < stCASESMWList.Count ; i++ ) {
          ...
          for ( int j = 0 ; j < stCASESMWList.Count ; j++ ) {
            if ( stCASESMWList[ i ].CASE == stCASESMWList[ j ].CASE &&
                 stCASESMWList[ i ].Edaban == stCASESMWList[ j ].Edaban ) {
        ...
        foreach ( stCASESMW stCASESMW in stCASESMWList ) {
          ...
          foreach ( ElementId soilId in soilList ) {
            ...
            if ( stCASESMW.CASE == CASE && stCASESMW.Edaban == edaNum ) {
              ClsSMW.ChangeSoil( doc, soilId, stCASESMW.NewCASE, stCASESMW.NewEdaban, (int) stCASESMW.soilLength,
                (int) stCASESMW.soilKei ) ;
            }
        ...
        foreach ( ElementId id in allSMWList ) {
          ...
          if ( stCASESMW.CASE == clsSMW.m_case.ToString() && stCASESMW.Edaban == clsSMW.m_edaNum ) {
            ...
            foreach ( ElementId soilId in soilList ) {
              ...
              //ソイルのCASEと枝番号が一致するものからデータを取得する
              if ( stCASESMW.CASE == CASE && stCASESMW.Edaban == edaNum ) {
                if ( ClsGeo.GEO_EQ( ( inst.Location as LocationPoint ).Point,
                      ( soilInst.Location as LocationPoint ).Point ) ) {
                  ClsSMW.ChangeSoil( doc, soilId, stCASESMW.NewCASE, stCASESMW.NewEdaban, (int) stCASESMW.soilLength,
                    (int) stCASESMW.soilKei ) ;
                  break ;
                }
    ...
    public class stCASERenzokuKabe
    {
      ...
      public static List<stCASERenzokuKabe> OverLapCASE( List<stCASERenzokuKabe> stCASERenzokuKabeList,
        bool list = false )
      {
        ...
        for ( int i = 0 ; i < stCASERenzokuKabeList.Count ; i++ ) {
          bool bFlag = false ;
          for ( int j = 0 ; j < dataList.Count ; j++ ) {
            if ( dataList[ j ].CASE == stCASERenzokuKabeList[ i ].CASE &&
                 dataList[ j ].Edaban == stCASERenzokuKabeList[ i ].Edaban ) {
              bFlag = true ;
              break ;
            }
          }
          ...
          for ( int j = 0 ; j < stCASERenzokuKabeList.Count ; j++ ) {
            if ( stCASERenzokuKabeList[ i ].CASE == stCASERenzokuKabeList[ j ].CASE &&
                 stCASERenzokuKabeList[ i ].Edaban == stCASERenzokuKabeList[ j ].Edaban ) {
      ...
      public static void CreateCASE( Document doc, List<stCASERenzokuKabe> stCASERenzokuKabeList,
        List<ElementId> allRenzokuKabeList )
      {
        ...
        foreach ( stCASERenzokuKabe stCASERenzokuKabe in stCASERenzokuKabeList ) {
          foreach ( ElementId id in allRenzokuKabeList ) {
            ClsRenzokukabe clsRenzokuKabe = new ClsRenzokukabe() ;
            clsRenzokuKabe.SetParameter( doc, id ) ;
            if ( stCASERenzokuKabe.CASE == clsRenzokuKabe.m_case.ToString() &&
                 stCASERenzokuKabe.Edaban == clsRenzokuKabe.m_edaNum ) {
          ...
          foreach ( ElementId kabeId in kabeList ) {
            ...
            if ( stCASERenzokuKabe.CASE == CASE ) {
              ClsRenzokukabe.ChangeKabeParameter( doc, kabeId, stCASERenzokuKabe.NewCASE,
                (int) stCASERenzokuKabe.kabeThickness, (int) stCASERenzokuKabe.kabeLength ) ;
            }
          }
        }
      ...
      public static void CreateCASE( Document doc, stCASERenzokuKabe stCASERenzokuKabe,
        List<ElementId> allRenzokuKabeList )
      {
        List<ElementId> kabeList = ClsRenzokukabe.GetAllKabeList( doc ) ;
    ...
    /// <returns></returns>
    public enCASE_EdabanHit ExistCaseEdaban( enKabeshu kabe, string caseName, string edaban, ref string nextCASE,
      ref string nextEdaban, bool checkOtherKabeshu = false )
    {
    ...
    public static void BackOriginalColor( Document doc, DLG.DlgCASE dlg )
    {
      if ( dlg.m_originalSettingsIdList != null && dlg.m_originalSettingsList != null ) {
        for ( int i = 0 ; i < dlg.m_originalSettingsIdList.Count ; i++ ) {
          ClsRevitUtil.SetElementOriginalColor( doc, dlg.m_originalSettingsIdList[ i ],
            dlg.m_originalSettingsList[ i ] ) ;
        }
      }
    }
