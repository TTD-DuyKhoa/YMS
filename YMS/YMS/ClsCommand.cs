...
namespace YMS
{
  ...
  public class ClsCommand : IExternalCommand
  {
    public Result Execute( ExternalCommandData commandData, ref string message, ElementSet elements )
    {
      ...
      FilteredElementCollector col = new FilteredElementCollector( doc ).WhereElementIsNotElementType()
        .OfCategory( BuiltInCategory.INVALID ).OfClass( typeof( Wall ) ) ;
    ...
    public static Result CreateBase( UIApplication uiapp, UIDocument uidoc, Document doc )
    {
      try {
        ...
        for ( int i = 0 ; i < num ; i++ ) {
          ...
          rotateTheta0[ i ] = SMatrix3d.Rotation( -basisYprime2[ i ], e0prime[ i ], theta0Set[ i ] ) *
                              rotateTheta1[ i ] ;
  ...
  public class TEST5 : IExternalCommand
  {
    public Result Execute( ExternalCommandData commandData, ref string message, ElementSet elements )
    {
      try {
        ...
        var scaleIds =
          ClsWarituke.CreateScale( uidoc, tmpStPoint, tmpEdPoint, WaritukeDist.warituke100mm, cross: true ) ;
        //削除フラグ無し
      }
      catch ( Exception ex ) {
  ...
  public class TEST8 : IExternalCommand
  {
    ...
    private Level GetElementLevel( Document doc, Element element )
    {
      ...
      BuiltInParameter[] levelParams = new[] { BuiltInParameter.INSTANCE_SCHEDULE_ONLY_LEVEL_PARAM } ;
      ...
      try {
        ...
        if ( testDialogResult == TaskDialogResult.CommandLink1 ) {
          ...
          foreach ( var pickResult in pickResultSet ) {
            ...
            // 斜梁鋼材の断面の端点を算出
            var points =
              ClsYMSUtil.CalcShabariSection( shabariBaseLine, shabariBasePlane, shabariW, shabariH, sectionPlane ) ;
          ...
          foreach ( var pickResult in pickResultSet ) {
            ...
            var points = Enumerable.Empty<double>().Append( ukeWhalf ).Append( -ukeWhalf )
              .Select( x => Plane.CreateByNormalAndOrigin( sectionPlane.Normal,
                sectionPlane.Origin + x * sectionPlane.Normal.Normalize() ) ).Select( x =>
                ClsYMSUtil.CalcShabariSection( shabariBaseLine, shabariBasePlane, shabariW, shabariH, x ) ) ;
            ...
            var sectionTop = ClsYMSUtil.CalcShabariSectionBottom( shabariBaseLine, shabariBasePlane, shabariW, shabariH,
              sectionPlane, ukezaiW ) ;
            if ( sectionTop != null ) {
              topPoints.Add( sectionTop ) ;
            }
            else {
