  ...
  public partial class FrmPreview : Form
  {
    ...
    private void drawPlateDanmen( Bitmap canvas )
    {
      ...
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        for ( int i = 0 ; i < FlatData.FukkoubanPitch.Count ; i++ ) {
          ...
          g.FillRectangle( Brushes.LightYellow, pX, pY, ( (float) FlatData.FukkoubanPitch[ i ] ) * widthCoef,
            PlateThick ) ;
          g.DrawRectangle( Pens.Black, pX, pY, ( (float) FlatData.FukkoubanPitch[ i ] ) * widthCoef, PlateThick ) ;

          //寸法線描画
          drawDim( canvas, new DPoint( pX, pY ),
            new DPoint( pX + ( ( (float) FlatData.FukkoubanPitch[ i ] ) * widthCoef ), pY ),
            ( (float) FlatData.FukkoubanPitch[ i ] ), 10, 5 ) ;
        ...
        if ( FlatData.BaseLevel == DefineUtil.eBaseLevel.FukkouTop ) {
          ...
          //文字列を赤色で表示
          g.DrawString( "▽" + FlatData.SelectedLevel, fnt, Brushes.DarkRed, defaultX + ( koudaiw * widthCoef ) + 15,
            lineY + txtHei ) ;
      ...
      //前長寸法作図
      //寸法線描画
      drawDim( canvas, new DPoint( defaultX, defaultY + _currentThick ),
        new DPoint( defaultX + currentWidth, defaultY + _currentThick ), FlatData.FukkoubanPitch.Sum(), 22, 5 ) ;
      _currentThick += defaultY + ( PlateThick ) ;
    }
    ...
    private void drawOhbikiDanmen( Bitmap canvas )
    {
      if ( FlatData.ohbikiData.isHkou ) {
        using ( Graphics g = Graphics.FromImage( canvas ) ) {
          float mx = ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ) + ( ( koudaiw * widthCoef ) +
            ( (float) FlatData.ohbikiData.exOhbikiEndLeng * widthCoef ) ) ;
          g.FillRectangle( Brushes.DarkGray, defaultX - ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ),
            _currentThick, mx, oobikiThick ) ;
          g.DrawRectangle( Pens.Black, defaultX - ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ),
            _currentThick, mx, oobikiThick ) ;
        ...
        //大引突き出し長さ寸法
        drawDim( canvas,
          new DPoint( defaultX - ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ),
            _currentThick - oobikiThick ), new DPoint( defaultX, _currentThick - oobikiThick ),
          (float) FlatData.ohbikiData.exOhbikiStartLeng, oobikiThick + 30, 5 ) ;
        float Mazleng = ( (float) FlatData.ohbikiData.exOhbikiEndLeng * widthCoef ) +
                        ( (float) ( FlatData.HukuinLength * widthCoef ) ) ;
        drawDim( canvas,
          new DPoint( Mazleng - ( (float) FlatData.ohbikiData.exOhbikiEndLeng * widthCoef ) + defaultX,
            _currentThick - oobikiThick ), new DPoint( Mazleng + defaultX, _currentThick - oobikiThick ),
          (float) FlatData.ohbikiData.exOhbikiEndLeng, oobikiThick + 30, 5 ) ;
        //幅員方向長さ
        drawDim( canvas, new DPoint( defaultX, _currentThick - oobikiThick ),
          new DPoint( defaultX + (float) FlatData.HukuinLength * widthCoef, _currentThick - oobikiThick ),
          FlatData.HukuinLength, oobikiThick + 30, 5 ) ;
    ...
    private void drawNedaDanmen( Bitmap canvas )
    {
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        ...
        for ( int i = 0 ; i < FlatData.NedaPitch.Count ; i++ ) {
          ...
          //根太描画
          drawHkou( canvas, new Point( (int) pX, (int) ( pY + ( PlateThick * highthCoef ) ) ), 10, (int) nedaHi, 2,
            Brushes.LightSalmon ) ;
        }

        drawHkou( canvas,
          new Point( (int) ( defaultX + currentWidth ), (int) ( defaultY + _currentThick + PlateThick * highthCoef ) ),
          10, (int) nedaHi, 2, Brushes.LightSalmon ) ;
      }
    ...
    private void drawPillarDanmen( Bitmap canvas )
    {
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        ...
        for ( int i = 0 ; i < FlatData.HukuinPillarPitch.Count ; i++ ) {
          ...
          if ( i == 0 ) {
            drawDim( canvas, new DPoint( pX, _currentThick ), new DPoint( pX, _currentThick + 200 ),
              FlatData.pilePillerData.PillarLength, 15, 3, false ) ;
          }

          if ( i != FlatData.HukuinPillarPitch.Count - 1 ) {
            float nextP = pX + (float) FlatData.HukuinPillarPitch[ i + 1 ] * widthCoef ;
            drawDim( canvas, new DPoint( pX + 5, _currentThick + 200 ), new DPoint( nextP + 5, _currentThick + 200 ),
              (float) FlatData.HukuinPillarPitch[ i + 1 ], 10, 5, true, true ) ;
        ...
        if ( ! FlatData.ohbikiData.isHkou ) {
          ...
          for ( int i = 0 ; i < FlatData.ohbikiData.OhbikiDan ; i++ ) {
            ...
            g.FillRectangle( Brushes.DarkGray,
              currentWidth - ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ), thick + ( i * 15 ), mx,
              15 ) ;
            g.DrawRectangle( Pens.Black, currentWidth - ( (float) FlatData.ohbikiData.exOhbikiStartLeng * widthCoef ),
              thick + ( i * 15 ), mx, 15 ) ;
            //thick += oobikiThick / 2;
          }
        }
    ...
    private void drawPlateSokumen( Bitmap canvas )
    {
      ...
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        ...
        if ( FlatData.BaseLevel == DefineUtil.eBaseLevel.FukkouTop ) {
          ...
          //文字列を青色で表示
          g.DrawString( "▽" + FlatData.SelectedLevel, fnt, Brushes.DarkRed, defaultX + ( koudaiw * widthCoef ) + 17,
            lineY + txtHei ) ;
      ...
      //根太突き出し長さ寸法
      drawDim( canvas,
        new DPoint( defaultX - ( (float) FlatData.nedaData.exNedaStartLeng * widthCoef ), defaultY + _currentThick ),
      ...
      drawDim( canvas, new DPoint( currentWidth + defaultX, defaultY + _currentThick ),
        new DPoint( currentWidth + defaultX + ( (float) FlatData.nedaData.exNedaEndLeng * widthCoef ),
          defaultY + _currentThick ), (float) FlatData.nedaData.exNedaEndLeng, 10, 5 ) ;


      //前長寸法作図
      //寸法線描画
      drawDim( canvas, new DPoint( defaultX, defaultY + _currentThick ),
        new DPoint( defaultX + currentWidth, defaultY + _currentThick ), FlatData.KyoutyouLength, 22, 5 ) ;
      _currentThick += defaultY + ( PlateThick ) ;
    }
    ...
    private void drawOhbikiSokumen( Bitmap canvas )
    {
      if ( FlatData.ohbikiData.isHkou ) {
        using ( Graphics g = Graphics.FromImage( canvas ) ) {
          ...
          drawHkou( canvas,
            new Point( (int) ( defaultX + currentWidth ),
              (int) ( defaultY + _currentThick + PlateThick * highthCoef ) ), 10, (int) nedaHi, 2, Brushes.LightGray ) ;
    ...
    {
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        g.FillRectangle( Brushes.LightSalmon, defaultX - ( (float) FlatData.nedaData.exNedaStartLeng * widthCoef ),
          _currentThick,
          ( koudaiw + (float) FlatData.nedaData.exNedaStartLeng + (float) FlatData.nedaData.exNedaEndLeng ) * widthCoef,
          oobikiThick ) ;
        g.DrawRectangle( Pens.Black, defaultX - ( (float) FlatData.nedaData.exNedaStartLeng * widthCoef ),
          _currentThick,
          ( koudaiw + (float) FlatData.nedaData.exNedaStartLeng + (float) FlatData.nedaData.exNedaEndLeng ) * widthCoef,
          oobikiThick ) ;
      }
    ...
    private void drawPillarSokumen( Bitmap canvas )
    {
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        ...
        for ( int i = 0 ; i < FlatData.KyoutyouPillarPitch.Count ; i++ ) {
          ...
          if ( ( i == 0 && FlatData.IsFirstShikigeta ) ||
               ( i == FlatData.KyoutyouPillarPitch.Count - 1 && FlatData.IsLastShikigeta ) ) {
            //敷桁位置に柱不要
          }
          else {
            //柱描画
          ...
          if ( i != FlatData.KyoutyouPillarPitch.Count - 1 ) {
            ...
            drawDim( canvas, new DPoint( pX + 5, _currentThick + 200 ), new DPoint( nextP + 5, _currentThick + 200 ),
              (float) FlatData.KyoutyouPillarPitch[ i + 1 ], 10, 5, true, true ) ;
    ...
    private void drawCannel( Bitmap canvas, Point drawPoint, int width, int hight, int thick, Brush brush,
      bool isRight )
    {
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        if ( ! isRight ) {
    ...
    private void drawDim( Bitmap canvas, DPoint start, DPoint end, double value, float dist, float margin,
      bool ishol = true, bool reverse = false, DPoint adjustText = null )
    {
      ...
      using ( Graphics g = Graphics.FromImage( canvas ) ) {
        ...
        if ( ishol ) {
          ...
          //文字列を青色で表示
          g.DrawString( value.ToString(), fnt, Brushes.Blue, Math.Abs( ( start.x + end.x ) / 2 ) + txtWid / 2,
            baseY + stDif ) ;
        }
        else {
          ...
          //文字列を位置(0,0)、青色で表示
          g.DrawString( value.ToString(), fnt, Brushes.Blue, baseX + d + m + txtWid - stDif,
            Math.Abs( ( start.y + end.y ) / 2 ) + stDif ) ;
        }
      }
    }
