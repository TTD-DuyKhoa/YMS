...
namespace YMS_gantry.UI
{
  public partial class FrmCreateSlope : Form
  {
    ...
    private bool InputValueChck()
    {
      ...
      try {
        ...
        for ( int i = 0 ; i < gridSlopeStyle.RowCount ; i++ ) {
          ...
          SlopeType sType = ( type == "スロープ無" ) ? SlopeType.none :
            ( type == "スロープ" ) ? SlopeType.slope : SlopeType.noriire ;
          ...
          if ( sType == SlopeType.none ) {
            ...
            if ( row.Index == 0 || row.Index == gridSlopeStyle.RowCount - 1 || isFNoriire ) {
              ...
              for ( int j = i ; j < gridSlopeStyle.RowCount - 1 ; j++ ) {
                ...
                SlopeType bsType = ( btype == "スロープ無" ) ? SlopeType.none :
                  ( btype == "スロープ" ) ? SlopeType.slope : SlopeType.noriire ;
    ...
    {
      var vm = ViewModel ;
      comboBox1.DataBindings.Add( nameof( System.Windows.Forms.ComboBox.DataSource ), vm,
        nameof( FrmCreateSlopeViewModel.KodaiSet ), true, DataSourceUpdateMode.OnPropertyChanged ) ;
      comboBox1.DisplayMember = nameof( FrmCreateSlopeViewModel.KodaiSlopeModel.Name ) ;
      comboBox1.DataBindings.Add( nameof( System.Windows.Forms.ComboBox.SelectedValue ), vm,
        nameof( FrmCreateSlopeViewModel.SelectedKodai ), true, DataSourceUpdateMode.OnPropertyChanged ) ;
      ...
      gridSlopeStyle.DataBindings.Add( nameof( DataGridView.DataSource ), vm,
        nameof( FrmCreateSlopeViewModel.StyleRowSet ), true, DataSourceUpdateMode.OnPropertyChanged ) ;
      ...
      gridSupportParts.DataBindings.Add( nameof( DataGridView.DataSource ), vm,
        nameof( FrmCreateSlopeViewModel.SupportRowSet ), true, DataSourceUpdateMode.OnPropertyChanged ) ;
