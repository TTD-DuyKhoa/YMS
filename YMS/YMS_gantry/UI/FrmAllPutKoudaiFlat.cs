...
namespace YMS_gantry.UI
{
  public partial class FrmAllPutKoudaiFlat : System.Windows.Forms.Form
  {
    ...
    private void button1_Click( object sender, EventArgs e )
    {
      if ( RbtKuiKyoutyouPitch.Checked ) {
        ...
        if ( DgvKuiKyoutyouPitch.Rows.Count > 0 &&
             MessageUtil.YesNo( "追加した橋長方向の杭ピッチをすべて更新します。\r\n続行しますか?", "杭追加", this ) != DialogResult.Yes ) {
          return ;
        }
        ...
        if ( this.DgvKuiKyoutyouPitch.SelectedRows.Count > 0 && this.DgvKuiKyoutyouPitch.SelectedRows[ 0 ].Index !=
            this.DgvKuiKyoutyouPitch.Rows.Count - 1 ) {
          row.Cells[ 0 ].Value = this.DgvKuiKyoutyouPitch.SelectedRows[ 0 ].Index + 2 ;
          row.Cells[ 1 ].Value = "0" ;
          for ( int i = this.DgvKuiKyoutyouPitch.SelectedRows[ 0 ].Index + 1 ;
               i < this.DgvKuiKyoutyouPitch.Rows.Count ;
               i++ ) {
    ...
    private void button2_Click( object sender, EventArgs e )
    {
      if ( RbtKuiHukuinPitch.Checked ) {
        ...
        if ( DgvKuiHukuinPitch.Rows.Count > 0 &&
             MessageUtil.YesNo( "追加した幅員方向の杭ピッチをすべて更新します。\r\n続行しますか?", "杭追加", this ) != DialogResult.Yes ) {
          return ;
        }
        ...
        if ( this.DgvKuiHukuinPitch.SelectedRows.Count > 0 &&
             this.DgvKuiHukuinPitch.SelectedRows[ 0 ].Index != this.DgvKuiHukuinPitch.Rows.Count - 1 ) {
          row.Cells[ 0 ].Value = this.DgvKuiHukuinPitch.SelectedRows[ 0 ].Index + 2 ;
          row.Cells[ 1 ].Value = "0" ;
          for ( int i = this.DgvKuiHukuinPitch.SelectedRows[ 0 ].Index + 1 ;
               i < this.DgvKuiHukuinPitch.Rows.Count ;
               i++ ) {
    ...
    private void InitComboBox()
    {
      ...
      lstStr = new List<string>()
      {
        Master.ClsPileTsugiteCsv.KoteiHohoBolt, Master.ClsPileTsugiteCsv.KoteiHohoYousetsu
      } ;
    ...
    private void comboBox2_SelectedIndexChanged( object sender, EventArgs e )
    {
      ...
      //    return;
      //}
      bool isHkou = ! string.IsNullOrEmpty( this.CmbOhbikiType.Text ) &&
                    ( ! this.CmbOhbikiType.SelectedItem.ToString().Equals( Master.ClsAngleCsv.TypeAngle ) &&
    ...
    private void CntrlToData()
    {
      ...
      data.ohbikiData.isHkou = CmbOhbikiType.Text.Equals( Master.ClsHBeamCsv.TypeHiro ) ||
                               CmbOhbikiType.Text.Equals( Master.ClsHBeamCsv.TypeHoso ) ||
                               CmbOhbikiType.Text.Equals( Master.ClsHBeamCsv.TypeNaka ) ||
                               CmbOhbikiType.Text.Equals( Master.ClsYamadomeCsv.TypeShuzai ) ||
                               CmbOhbikiType.Text.Equals( Master.ClsYamadomeCsv.TypeHighShuzai ) ;
      ...
      data.pilePillerData.PileCutLength = data.pilePillerData.PillarLength = (double) NmcKuiLng.Value ;
      ...
      data.pilePillerData.extensionPileData.attachType =
        this.txtExPileJointType.Text == "ボルト" ? eJoinType.Bolt : eJoinType.Welding ;
      JointPlateData jpd = new JointPlateData() ;
      jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( txtExJointPlSizeFO1.Text, txtExJointPlSizeFO2.Text,
        txtExJointPlSizeFO3.Text ) ;
      ...
      jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( txtExJointPlSizeFI1.Text, txtExJointPlSizeFI2.Text,
        txtExJointPlSizeFI3.Text ) ;
      ...
      jpd = new JointPlateData() ;
      jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( txtExJointPlSizeW1.Text, txtExJointPlSizeW2.Text,
        txtExJointPlSizeW3.Text ) ;
      ...
      data.pilePillerData.jointDetailData.JointType =
        CmbKuiJointType.Text == "ボルト" ? eJoinType.Bolt : eJoinType.Welding ;
      jpd = new JointPlateData() ;
      jpd.PlateSize =
        Master.ClsPileTsugiteCsv.CreatePlateName( txtJointPlSizeFO1.Text, txtJointPlSizeFO2.Text,
          txtJointPlSizeFO3.Text ) ;
      ...
      jpd = new JointPlateData() ;
      jpd.PlateSize =
        Master.ClsPileTsugiteCsv.CreatePlateName( txtJointPlSizeFI1.Text, txtJointPlSizeFI2.Text,
          txtJointPlSizeFI3.Text ) ;
      ...
      jpd = new JointPlateData() ;
      jpd.PlateSize =
        Master.ClsPileTsugiteCsv.CreatePlateName( txtJointPlSizeW1.Text, txtJointPlSizeW2.Text,
          txtJointPlSizeW3.Text ) ;
    ...
    private void DataToCntrl()
    {
      ...
      this.txtExPileJointType.Text = /*data.pilePillerData.extensionPileData.attachType == eAttachType.Bolt ? "ボルト" :*/
        "溶接" ;
    ...
    private bool CheckInPutValues( bool isPreview = false )
    {
      ...
      try {
        ...
        //敷桁
        if ( ( ChckSikiketaStart.Checked || ChckSikiketaEnd.Checked ) &&
             string.IsNullOrEmpty( CmbShikiketaSize.Text ) ) {
        ...
        //大引
        if ( string.IsNullOrEmpty( CmbShikiketaMaterial.Text ) &&
             ( ChckSikiketaStart.Checked || ChckSikiketaEnd.Checked ) ) {
    ...
    private void button5_Click( object sender, EventArgs e )
    {
      ...
      nPitch = new List<string[]>
      {
        new string[] { $"1", "0" }, new string[] { $"2", "5500" }, new string[] { $"3", "5500" }
      } ;
      DgvKuiHukuinPitch.Rows.Clear() ;
      for ( int i = 0 ; i < nPitch.Count ; i++ ) {
    ...
    private void CalcPileLength()
    {
      ...
      double nedaHeight = ( this.CmbNedaSize.Text != "" )
        ? GantryUtil.GetKouzaiSize( this.CmbNedaSize.Text ).Height
        : 0 ;
      double ohbikiHeight = ( this.CmbOhbikiSize.Text != "" )
        ? GantryUtil.GetKouzaiSize( this.CmbOhbikiSize.Text ).Height
        : 0 ;
      double topPLthick = ( this.CmbKuiTopPLSize.Text != "" )
        ? GantryUtil.GetKouzaiSize( this.CmbKuiTopPLSize.Text ).Thick
        : 0 ;
    ...
    private void button4_Click( object sender, EventArgs e )
    {
      ...
      if ( RbtNedaKobetsu.Checked ) {
        ...
        if ( this.DgvNedaPitch.SelectedRows.Count > 0 &&
             this.DgvNedaPitch.SelectedRows[ 0 ].Index != this.DgvNedaPitch.Rows.Count - 1 ) {
      ...
      //覆工板直下配置
        ...
        if ( DgvNedaPitch.Rows.Count > 0 &&
             MessageUtil.YesNo( "根太サイズをすべて更新します。\r\r続行しますか?", "根太追加", this ) != DialogResult.Yes ) {
          return ;
        }
        ...
        foreach ( DataGridViewRow row in DgvFukkouban.Rows ) {
          ...
          double w = RevitUtil.ClsCommonUtils.ChangeStrToDbl( row.Cells[ 1 ].Value.ToString()
            .Substring( row.Cells[ 1 ].Value.ToString().LastIndexOf( "X" ) + 1 ) ) ;
    ...
    private void button7_Click( object sender, EventArgs e )
    {
      ...
      if ( RbtFukkouKobetsu.Checked ) {
        ...
        if ( this.DgvFukkouban.SelectedRows.Count > 0 &&
             this.DgvFukkouban.SelectedRows[ 0 ].Index != this.DgvFukkouban.Rows.Count - 1 ) {
        ...
        double w = RevitUtil.ClsCommonUtils.ChangeStrToDbl(
          CmbFukkouSize.Text.Substring( CmbFukkouSize.Text.LastIndexOf( "X" ) + 1 ) ) ;
    ...
    private void BtnDelete_Click( object sender, EventArgs e )
    {
      ...
      List<ElementId> lstId =
        ( from data in unionMaterials where data.m_KodaiName == kodaiName select data.m_ElementId ).ToList() ;
    ...
    private void BtnCalcFileDataLoad_Click( object sender, EventArgs e )
    {
      ...
      if ( ( data.KoujiType == eKoujiType.Kenchiku && calcFileData.Base.Type == "建築" ) ||
           ( ( data.KoujiType == eKoujiType.Doboku && calcFileData.Base.Type == "土木" ) ) ) {
        // 計算書取込データを画面データに読込
  ...
  public class AllKoudaiFlatFrmData
  {
    ...
    public (XYZ Origin, XYZ BasisH, XYZ BasisK, XYZ BasisZ) GetCoordinate( Document doc )
    {
      ...
      var baseInstance = MaterialSuper.Collect( doc ).Select( x => x as KoudaiReference ).Where( x => x != null )
    ...
    public (XYZ Origin, XYZ BasisH, XYZ BasisK, XYZ BasisZ) GetBasis( Document doc, bool isSlope = false )
    {
      ...
      var origin = coordinate.Origin +
                   RevitUtil.ClsRevitUtil.CovertToAPI( hukuinPitches.FirstOrDefault() ) * coordinate.BasisH +
                   RevitUtil.ClsRevitUtil.CovertToAPI( kyochoPitches.FirstOrDefault() ) * coordinate.BasisK ;
    ...
    public double[] PillarHSetFt()
    {
      ...
      return Enumerable.Range( 0, this.HukuinPillarPitch.Count() )
        .Select( x => this.HukuinPillarPitch.Skip( 1 ).Take( x ).Sum() )
        .Select( x => ClsRevitUtil.CovertToAPI( x ) + diff ).ToArray() ;
    }
    ...
    /// </summary>
    /// <returns></returns>
    public double[] PillarKSetFt() =>
      Enumerable.Range( 0, this.KyoutyouPillarPitch.Count() )
        .Select( x => this.KyoutyouPillarPitch.Skip( 1 ).Take( x ).Sum() ).Select( x => ClsRevitUtil.CovertToAPI( x ) )
        .ToArray() ;
    ...
    {
      this.KoujiType = calcFileData.Base.Type == "建築" ? eKoujiType.Kenchiku :
        calcFileData.Base.Type == "土木" ? eKoujiType.Doboku : eKoujiType.None ;
      ...
      // 基本情報 構台種別
      this.KoujiType = calcFileData.Base.Type == "建築" ? eKoujiType.Kenchiku :
        calcFileData.Base.Type == "土木" ? eKoujiType.Doboku : eKoujiType.Kenchiku ;
      ...
      // 根太 ピッチ
      var nedaPitchValue = calcFileData.Girder.GirderPitch.Pitches.OrderBy( x => x.No )
        .Select( x => double.Parse( x.Value.ToString() ) ).ToList() ;
      ...
      this.ohbikiData.OhbikiDan =
        obikiDanValue == "シングル" ? 1 : obikiDanValue == "ダブル" ? 2 : obikiDanValue == "トリプル" ? 3 : 1 ;
      // 大引 取付方法
      var ohbikiAttachTypeValue = calcFileData.CatchBeam.FitMeth ;
      this.ohbikiData.OhbikiAttachType = ohbikiAttachTypeValue == "ボルト" ? eJoinType.Bolt :
        ohbikiAttachTypeValue == "溶接" ? eJoinType.Welding : eJoinType.Bolt ;
      // 大引 ボルト本数
      var ohbikiBoltCountValue = int.TryParse( calcFileData.CatchBeam.BoltNum, out int ohbikiBoltCountConvertValue )
        ? ohbikiBoltCountConvertValue
        : 0 ;
      ...
      // 杭 ピッチ
      var pillarPitchValue = calcFileData.Pile.PilePitch.Pitches.OrderBy( x => x.No )
        .Select( x => double.Parse( x.Value.ToString() ) ).ToList() ;
      // 計算書取込データには先頭ピッチ(0)が無いので挿入
      ...
      // 覆工板 幅員方向 サイズ
      this.FukkoubanSizeList = calcFileData.CoverPlate.CoverPlatePitch.Sizes.OrderBy( x => x.No )
        .Select( x => GetFukkoubanSizeByInputCalcFileData( x.Value, calcFileData.CoverPlate.Kind ) ).ToList() ;
    }
