...
namespace YMS_gantry.UI
{
  public partial class FrmEditLocationChange : System.Windows.Forms.Form
  {
    ...
    public FrmEditLocationChange( ExternalEvent exEvent, RequestHandler handler, UIApplication uiApp,
    ...
    private void InitComponent( FamilyInstance targetFi )
    {
      ...
      var intersectElementIdList =
        ClsRevitUtil.GetIntersectFamilys( doc, _targetElementId, ignorelist: new List<string>() { targetFi.Name } ) ;
      ...
      foreach ( var intersectElementId in intersectElementIdList ) {
        ...
        if ( ! existsBunrui ) {
          ...
          if ( string.IsNullOrWhiteSpace( daiBunrui ) && string.IsNullOrWhiteSpace( chuuBunrui ) &&
               string.IsNullOrWhiteSpace( shouBunrui ) ) return ;
    ...
    private List<RowData> CreateRowDataList( ElementId targetElementId )
    {
      ...
      var intersectElementIdList =
        ClsRevitUtil.GetIntersectFamilys( doc, targetElementId, ignorelist: new List<string>() { targetFi.Name } ) ;
    ...
    private RowData GetPileData( Document doc, ElementId exPileId )
    {
      ...
      try {
        ...
        var intersectElementIdList =
          ClsRevitUtil.GetIntersectFamilys( doc, exPileId, ignorelist: new List<string>() { targetFi.Name } ) ;
        foreach ( var intersectElementId in intersectElementIdList ) {
          var intersectFi = GantryUtil.GetFamilyInstance( doc, intersectElementId ) ;
          if ( intersectFi == null ) continue ;
