...
namespace YMS_gantry.UI
{
  public partial class FrmEditLengthChange : System.Windows.Forms.Form
  {
    ...
    public FrmEditLengthChange( ExternalEvent exEvent, RequestHandler handler, UIApplication uiApp,
    ...
    private void InitComponent()
    {
      foreach ( var elementUniqueId in _targetElementUniqueIdList ) {
        ...
        if ( string.IsNullOrEmpty( lengthByParam ) &&
             ( string.IsNullOrEmpty( lengthByTypeParam ) || double.Parse( lengthByTypeParam ) == 0.0 ) ) continue ;
    ...
    public void InitData()
    {
      ...
      foreach ( var rowData in rowDataList ) {
        this.DgvEditLengthChangeList.Rows.Add( new object[]
        {
          rowData.Id, rowData.ElementId, false, rowData.KoudaiName, rowData.DaiBunrui, rowData.ChuuBunrui,
          rowData.ShouBunrui, rowData.FamilyName, rowData.Length,
        } ) ;
      }
    }
    ...
    private void DgvEditLengthChangeList_CellValueChanged( object sender, DataGridViewCellEventArgs e )
    {
      ...
      var cellEditingCheck =
        this.DgvEditLengthChangeList.Rows[ e.RowIndex ].Cells[ "ColLengthChangeListSelected" ] as
      ...
      for ( var i = 0 ; i < this.DgvEditLengthChangeList.RowCount ; i++ ) {
        var cellCheck =
          this.DgvEditLengthChangeList.Rows[ i ].Cells[ "ColLengthChangeListSelected" ] as DataGridViewCheckBoxCell ;

        // 編集中のセルを除くかつ、チェックが有効な場合
        if ( ( cellEditing.RowIndex != cellCheck.RowIndex ) && (bool) cellCheck.Value ) {
          var cellEditLength =
            this.DgvEditLengthChangeList.Rows[ i ]
              .Cells[ "ColLengthChangeListAfterLength" ] as DataGridViewTextBoxCell ;
