...
namespace YMS_gantry
{
    ...
    public sealed class Shikigeta : OhbikiSuper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static Dictionary<XYZ, string> CalcArrangementPoint(AllKoudaiFlatFrmData kData, XYZ basePnt, XYZ vecKyoutyou)
        {
            Dictionary<XYZ, string> retList = new Dictionary<XYZ, string>();
            XYZ newP = basePnt;
            for (int hC = 0; hC < kData.KyoutyouPillarPitch.Count; hC++)
            {
                newP = newP + vecKyoutyou.Normalize() * (RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(kData.KyoutyouPillarPitch[hC]));
                if ((hC == 0 && kData.IsFirstShikigeta) || (hC == kData.KyoutyouPillarPitch.Count - 1 && kData.IsLastShikigeta))
                {
                    if (kData.shikigetaData.ShikigetaSize.StartsWith("H")|| kData.shikigetaData.ShikigetaSize.Contains("HA")|| kData.shikigetaData.ShikigetaSize.Contains("SMH"))
                    {
                        retList.Add(newP, $"{hC}");
                    }
                    else
                    {
                        ...
                        retList.Add(newP + vecKyoutyou.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((pS.Height / 2)), $"F-{hC}");//橋長方向側
                        retList.Add(newP - vecKyoutyou.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((pS.Height / 2)), $"B-{hC}");//原点側
                    }
                }
            }
            return retList;
        ...
        /// </summary>
        /// <returns></returns>
        public static List<ElementId> CreateShikiGeta(Document doc,OhbikiData oData, ShikigetaData data, XYZ start, XYZ end, Level level ,string koudainame, double rotate=0, double leveloffset = 0,FamilySymbol oriSym=null)
        {
            ...
            using (SubTransaction tr = new SubTransaction(doc))
            {
                ...
                {
                    CreatedID = MaterialSuper.PlaceWithTwoPoints(sym, level.GetPlaneReference(), start, end, RevitUtil.ClsRevitUtil.CovertToAPI(leveloffset));
