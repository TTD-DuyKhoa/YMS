...
namespace YMS_gantry
{
  public abstract class MaterialSuper
  {
      ...
      get
      {
        ...
        if ( elem is FamilyInstance familyInstance ) {
          if ( familyInstance.Location is LocationCurve locationCurve ) {
            ...
            var lengthParam = familyInstance.Parameters.Cast<Parameter>()
              .FirstOrDefault( x => x.Definition.Name == DefineUtil.PARAM_LENGTH ) ;
    ...
    public static ElementId PlaceWithTwoPoints( FamilySymbol symbol, Reference reference, XYZ startPt, XYZ endPt,
      double levelOffsetFeet, bool noOffset = false )
    {
      ...
      if ( family.FamilyPlacementType == FamilyPlacementType.WorkPlaneBased ) {
        ...
        if ( element is Level level ) {
          ...
          var lineSeg = Line.CreateBound( startPt, endPt ) ;
          var instance = doc.Create.NewFamilyInstance( lineSeg, symbol, (Level) doc.GetElement( element.LevelId ),
    ...
    private static MaterialSuper FromDictionary( Dictionary<string, string> map )
    {
      try {
        ...
        foreach ( var property in GetPropertiesRecursive( resultType, BindingFlags.Instance ) ) {
          ...
          if ( ! map.TryGetValue( key, out var valueString ) ) {
            continue ;
          }
    ...
    private static Dictionary<string, string> ToDictionary( MaterialSuper material )
    {
      ...
      try {
        ...
        var properties = GetPropertiesRecursive( materialType, BindingFlags.Instance ).ToArray() ;
    ...
    public static void WriteToElement( MaterialSuper material, Document doc )
    {
      ...
      RevitUtil.ClsRevitUtil.SetMojiParameter( doc, material.m_ElementId, DefineUtil.PARAM_MATERIAL_DETAIL,
        lineString ) ;
    }
    ...
    {
      return new FilteredElementCollector( document ).OfClass( typeof( FamilyInstance ) )
        .Select( x => ReadFromElement( x.Id, document ) ).Where( x => x != null ).ToArray() ;
    }
    ...
    {
      return new FilteredElementCollector( document ).OfClass( typeof( FamilyInstance ) ).Select( x => x.Id )
        .Where( x => IsFromOtherElement( x, document ) ).ToList() ;
    }
