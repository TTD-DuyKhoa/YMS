...
namespace YMS_gantry.Material
{
  ...
  public sealed class JifukuZuredomezai : MaterialSuper
  {
    ...
    public JifukuZuredomezai( string koudainame, string size, string material, ElementId id ) : base( id, koudainame,
      material, size )
    {
    }

    public ElementId CreateJifuku( JifukuZuredomezai jifuku, Document doc, string type, Reference refer, XYZ point1,
      XYZ point2, double offset = 0, double rotate = 0, bool adjust = false )
    {
        ...
        if ( retId != ElementId.InvalidElementId ) {
          ...
          if ( adjust ) {
            ...
            if ( ts.BasisZ.Z > 0 && XYZ.BasisZ.Z != ts.BasisZ.Z && rad != 0 ) {
              ...
              ElementTransformUtils.MoveElement( doc, retId,
                vec * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( dist2 ) ) ;
            }
          }

          //高さ
          RevitUtil.ClsRevitUtil.SetParameter( doc, retId, DefineUtil.PARAM_HOST_OFFSET,
