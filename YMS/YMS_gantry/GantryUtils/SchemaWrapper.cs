...
namespace YMS_gantry
{
  ...
  public class SchemaWrapper
  {
    ...
    public static SchemaWrapper FromSchema( Schema schema )
    {
      ...
      foreach ( Field currentField in schema.ListFields() ) {
        ...
        MethodInfo addFieldmethod = typeof( SchemaWrapper ).GetMethod( "AddField",
          new Type[] { typeof( string ), typeof( ForgeTypeId ), typeof( SchemaWrapper ) } ) ;
        ...
        if ( currentField.ContainerType == ContainerType.Simple )
          ...
          {
            typeof( IList<int> ).GetGenericTypeDefinition().MakeGenericType( new Type[] { currentField.ValueType } )
          } ;
        else
          methodGenericParameters = new Type[]
          {
            typeof( Dictionary<int, int> ).GetGenericTypeDefinition()
              .MakeGenericType( new Type[] { currentField.KeyType, currentField.ValueType } )
          } ;
    ...
    public void FinishSchema()
    {
      ...
      foreach ( FieldData currentFieldData in m_SchemaDataWrapper.DataList ) {
        ...
        if ( fieldType.IsGenericType ) {
          ...
          genericParams =
            fieldType.GetGenericArguments() ; //Get the actual data type(s) stored in the field's IList<> or IDictionary<>
        ...
        if ( //if we're dealing with an Entity as a simple field, map field, or list field and need to do recursion...
            ( fieldType == ( typeof( Entity ) ) ) || ( fieldType.IsGenericType &&
                                                       ( ( genericParams[ 0 ] == ( typeof( Entity ) ) ) ||
                                                         ( ( genericParams.Length > 1 ) &&
                                                           ( genericParams[ 1 ] == typeof( Entity ) ) ) ) ) ) {
    ...
    public void ToXml( string xmlDataPath )
    {
      ...
      Stream streamXmlOut =
        new FileStream( xmlDataPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite ) ;
      ...
      StringBuilder strBuilder )
    {
      strBuilder.AppendLine( "Schema/Entity Name: " + "" + ", Description: " + schema.Documentation + ", Id: " +
                             schema.GUID.ToString() + ", Read Access: " + schema.ReadAccessLevel.ToString() +
                             ", Write Access: " + schema.WriteAccessLevel.ToString() ) ;
      foreach ( Field currentField in schema.ListFields() ) {
        ...
        ParameterModifier[]
          pmodifiers = new ParameterModifier[ 0 ] ; //a Dummy parameter needed for GetMethod() call, empty

        //Get the method.
        MethodInfo getFieldDataAsStringMethod = typeof( SchemaWrapper ).GetMethod( "GetFieldDataAsString",
          BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.InvokeMethod, System.Type.DefaultBinder,
      ...
      if ( field.ContainerType == ContainerType.Simple ) {
        ...
        if ( fieldType == typeof( Entity ) ) {
          ...
          strBuilder.AppendLine( "Field: " + field.FieldName + ", Type: " + field.ValueType.ToString() + ", Value: " +
                                 " {SubEntity} " + ", Unit: " + field.GetSpecTypeId().TypeId + ", ContainerType: " +
        ...
        if ( fieldType == ( typeof( Entity ) ) ) {
          ...
          strBuilder.AppendLine( "Field: " + field.FieldName + ", Type: " + field.ValueType.ToString() + ", Value: " +
                                 " {Array} " + ", Unit: " + field.GetSpecTypeId().TypeId + ", ContainerType: " +
                                 field.ContainerType.ToString() ) ;
          foreach ( FieldType fa in listRetval ) {
