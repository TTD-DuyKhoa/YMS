...
{
    public static class UtilAlgebra
    {
        ...
        public static bool IsReal(Complex z) => Eq(z.Imaginary, 0.0);
        ...
        public static Complex[] SolveQuadraticEquation(Complex a, Complex b, Complex c)
        {
            ...
            var result = Enumerable.Empty<Complex>()
                .Append((-b + discriminantSqrt) / (2 * a))
                .Append((-b - discriminantSqrt) / (2 * a))
                .ToArray();
#if DEBUG
            var checkResult = result
                .Select(x => CalcPolynomial(new Complex[] { a, b, c }, x))
                .ToArray();
#endif
            return result;
        }
        ...
        public static Complex[] SolveCubicEquation(double a, double b, double c, double d)
        {
            ...
            var checkResults = results
                .Select(x => CalcPolynomial(new Complex[] { a, b, c, d }, x))
                .ToArray();
#endif
            return results;
        }
        ...
        private static Complex[] CardanoMethod(double a, double b, double c)
        {
            ...
            return Enumerable.Empty<Complex>()
                .Append(u + v)
                .Append(u * omega + v * omega2)
                .Append(u * omega2 + v * omega)
                .Select(y => -a / 3 + y)
                .ToArray();
        }
...
#if DEBUG
            var checkResults = results
                .Select(x => CalcPolynomial(new Complex[] { a, b, c, d, e }, x))
                .ToArray();
#endif
            return results;
        }
        ...
        private static Complex[] FerrariMethod(double a, double b, double c, double d)
        {
            ...
            return Enumerable.Empty<Complex>()
                .Concat(SolveQuadraticEquation(1, -m, lamda - n))
                .Concat(SolveQuadraticEquation(1, m, lamda + n))
                .Select(y => -a / 4 + y)
                .ToArray();
        }
        ...
        {
            {
                ...
                for (int i = 0; i < 1000; i++)
                {
                    ...
                    var checkResult = roots
                        .Select(x => CalcPolynomial(coefSet.Select(y => y * Complex.One), x))
                        .ToArray();
        ...
        public static double ToFt(this double mm) => mm / 304.8;
    }
}
