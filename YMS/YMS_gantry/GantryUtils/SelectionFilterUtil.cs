    ...
    public bool Selection( string prmpt, out List<FamilyInstance> retList )
    {
      ...
      while ( flag ) {
        ...
        try {
          ...
          Reference reference =
            uiDoc.Selection.PickObject( ObjectType.Element, pickFilter, "部材を選択して下さい　選択終了はEscキーを押下してください" ) ;
  ...
  class YMSPickFilter : ISelectionFilter
  {
    ...
    public bool AllowElement( Element elem )
    {
      ...
      try {
        ...
        if ( string.IsNullOrEmpty( cate ) || cate == null ) {
          return false ;
        }

        if ( CategoryList.Contains( cate ) ) {
          return true ;
        }
        else {
          return false ;
  ...
  class YMSWaritsukeFilter : ISelectionFilter
  {
    ...
    public bool AllowElement( Element elem )
    {
      ...
      try {
        ...
        if ( ms.GetType() == typeof( Ohbiki ) || ms.GetType() == typeof( Ohbiki ) ||
             ms.GetType() == typeof( Shikigeta ) || ms.GetType() == typeof( PutFukkougeta ) ||
             ms.GetType() == typeof( HorizontalJoint ) || ms.GetType() == typeof( Tesuri ) ||
             ms.GetType() == typeof( Neda ) ) {
          return true ;
        }
