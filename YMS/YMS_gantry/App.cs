...
namespace YMS_gantry
{
  class App : IExternalApplication
  {
    ...
    public void AddRibbonMenu( UIControlledApplication app )
    {
      ...
      // [一括配置] ブレース・ツナギ配置
      CreatePushButton( panelAllPut, "CmdAllPut4", "ブレース・ツナギ", "AllPutBraceTsunagi", "ブレース・ツナギ材を一括配置します。",
        "3_ブレース・ツナギ" ) ;
      ...
      // [個別配置] 大引(桁受)
      // [個別配置] 根太(主桁)
      PushButtonDef pushButtonDef1 =
        new PushButtonDef( "CmdPut2", "大引(桁受)", "PutOobikiKetauke", "大引(桁受)を配置します。", "6_大引" ) ;
      PushButtonDef pushButtonDef2 =
        new PushButtonDef( "CmdPut3", "根太(主桁)", "PutNedaShugeta", "根太(主桁)を配置します。", "7_根太" ) ;
      ...
      // [個別配置] 地覆・覆工板ズレ止め材配置
      CreatePushButton( panelPut, "CmdPut10", "覆工板ズレ止め", "PutJifukuFukkoubanZuredome", "地覆・覆工板ズレ止め材を配置します。",
        "14_覆工板ズレ止め" ) ;
      ...
      // [個別配置] 垂直ブレース配置
      // [個別配置] 水平ツナギ配置
      PushButtonDef pushButtonDef5 =
        new PushButtonDef( "CmdPut12", "水平ブレース", "PutHorizontalBrace", "水平ブレースを配置します。", "16_水平ブレース" ) ;
      PushButtonDef pushButtonDef6 =
        new PushButtonDef( "CmdPut13", "垂直ブレース", "PutVerticalBrace", "垂直ブレースを配置します。", "17_垂直ブレース" ) ;
      PushButtonDef pushButtonDef7 =
        new PushButtonDef( "CmdPut14", "水平ツナギ", "PutHorizontalTsunagi", "水平ツナギを配置します。", "18_水平ツナギ" ) ;
    ...
    private void CreatePushButton( RibbonPanel panel, string name, string text, string className, string toolTipText,
      string iconName )
    {
    ...
    private void CreateStackedPushButton( RibbonPanel panel, List<PushButtonDef> pushButtonDefList )
    {
      ...
      foreach ( var pushButtonDef in pushButtonDefList ) {
        PushButtonData pushButtonData = new PushButtonData( pushButtonDef.Name, pushButtonDef.Text, _dllPath,
          $"YMS_gantry.{pushButtonDef.ClassName}" ) ;
      ...
      if ( pushButtonDataList.Count == 3 ) {
        IList<RibbonItem> addedButtons =
          panel.AddStackedItems( pushButtonDataList[ 0 ], pushButtonDataList[ 1 ], pushButtonDataList[ 2 ] ) ;
        List<PushButton> pushBtnList = new List<PushButton>() ;
        foreach ( var item in addedButtons ) {
          pushBtnList.Add( item as PushButton ) ;
        }
