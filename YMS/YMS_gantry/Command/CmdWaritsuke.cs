...
namespace YMS_gantry.Command
{
  class CmdWaritsuke
  {
    ...
    private WaritsukeResult WaritukeMain( UIApplication uiapp )
    {
      ...
      if ( ! isSichu ) {
        ...
        if ( rs == DialogResult.Yes ) {
          tmpStPoint = ( cvBase.GetEndPoint( 0 ).Z > cvBase.GetEndPoint( 1 ).Z )
            ? cvBase.GetEndPoint( 0 )
            : cvBase.GetEndPoint( 1 ) ;
          tmpEdPoint = ( cvBase.GetEndPoint( 0 ).Z > cvBase.GetEndPoint( 1 ).Z )
            ? cvBase.GetEndPoint( 1 )
            : cvBase.GetEndPoint( 0 ) ;
        }
      ...
      while ( Continue ) {
        ...
        scaleIds = CreateScale( uidoc, m_InsertPoint, m_EndPoint, instBase, WaritukeDist.warituke100mm, maxLength,
          minLength ) ;
    ...
    private static bool WaritsukeKetaZai( Document doc, FamilyInstance instBase, FamilySymbol kouzaiSym, string size,
      double length, double rotate = 0 )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          //タイプを複製
          FamilySymbol creatSym = ( kouzaiSym.Name == "ﾂﾅｷﾞ" )
            ? kouzaiSym
            : GantryUtil.DuplicateTypeWithNameRule( doc, ms.m_KodaiName, kouzaiSym, kouzaiSym.Name ) ;
    ...
    /// <param name="size"></param>
    /// <param name="length"></param>
    private static bool WaritsukeShichu( Document doc, FamilyInstance instBase, FamilySymbol kouzaiSym, string size,
      double length )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins =
            GantryUtil.CreateInstanceWith1point( doc, end, refer, creatSym, instBase.HandOrientation ) ;
          if ( ins != null ) {
            newId = ins.Id ;
          }
    ...
    /// <returns></returns>
    private static bool WaritsukeCoverPL( Document doc, FamilyInstance instBase, FamilySymbol coverPLSym,
      string parentSize, string size, bool isShichu )
    {
      bool retBool = true ;
    ...
    private static bool WaritsukePieace( Document doc, FamilyInstance instBase, FamilySymbol PieaceSym, string size )
    {
      ...
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          XYZ vec = ( m_EndPoint - m_InsertPoint ).Normalize() ;
          FamilyInstance ins = GantryUtil.CreateInstanceWith1point( doc, m_InsertPoint, refer, PieaceSym,
            vec.Normalize(), followWithHostType: false ) ;
    ...
    /// <param name="length"></param>
    /// <returns></returns>
    private static bool WaritsukePieaceToShicu( Document doc, FamilyInstance instBase, FamilySymbol PieaceSym,
      string size )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins = GantryUtil.CreateInstanceWith1point( doc, newP, refer, PieaceSym,
            instBase.HandOrientation, followWithHostType: true ) ;
          if ( ins != null ) {
            AttachMaterialSuper( doc, ins.Id, ms.m_KodaiName, ms, size, FrmWaritsukeElement.WaritsukeType.Pieace ) ;
            t.Commit() ;
            m_createdIds.Add( ins.Id ) ;
    ...
    /// <returns></returns>
    private static bool WaritsukePlate( Document doc, FamilyInstance instBase, FamilySymbol plateSym, string size,
      (double h, double d, double w) plateSize )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins = GantryUtil.CreateInstanceWith1point( doc, m_InsertPoint, refer, creatSym,
            vec.Normalize(), followWithHostType: false ) ;
          if ( ins != null ) {
    ...
    /// <param name="plateSize"></param>
    /// <returns></returns>
    private static bool WaritsukePlateToShicu( Document doc, FamilyInstance instBase, FamilySymbol plateSym,
      string size, (double h, double d, double w) plateSize )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins = GantryUtil.CreateInstanceWith1point( doc, newP, refer, creatSym,
            instBase.HandOrientation, followWithHostType: true ) ;
          if ( ins != null ) {
            AttachMaterialSuper( doc, ins.Id, ms.m_KodaiName, ms, size, FrmWaritsukeElement.WaritsukeType.Plate ) ;
            t.Commit() ;
    ...
    /// <param name="length"></param>
    /// <returns></returns>
    private static bool WaritsukeJack( Document doc, FamilyInstance instBase, FamilySymbol jackSym, string size,
      double length )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins =
            GantryUtil.CreateInstanceWith1point( doc, m_InsertPoint, refer, jackSym, vec.Normalize() ) ;
          if ( ins != null ) {
    ...
    /// <param name="length"></param>
    /// <returns></returns>
    private static bool WaritsukeJackToSichu( Document doc, FamilyInstance instBase, FamilySymbol jackSym, string size,
      double length )
    {
      bool retBool = true ;
      using ( Transaction t = new Transaction( doc, Guid.NewGuid().GetHashCode().ToString() ) ) {
        try {
          ...
          FamilyInstance ins = GantryUtil.CreateInstanceWith1point( doc, newP, refer, jackSym, instBase.HandOrientation,
            followWithHostType: false ) ;
          if ( ins != null ) {
    ...
    /// <param name="size"></param>
    private static void AttachMaterialSuper( Document doc, ElementId id, string koudaiName, MaterialSuper materialSuper,
      string size, FrmWaritsukeElement.WaritsukeType wType )
    {
    ...
    /// <returns></returns>
    public static List<ElementId> CreateScale( UIDocument uidoc, XYZ tmpStPoint, XYZ tmpEdPoint, FamilyInstance ins,
      WaritukeDist eDist, double maxLength = 7000.0, double minLength = 1000.0 )
    {
      List<ElementId> scaleIds = new List<ElementId>() ;
        ...
        try {
          ...
          for ( int i = 0 ; i <= scaleMax / dist ; i++ ) {
            ...
            if ( i % 10 == 0 ) {
              ...
              XYZ txtPoint = cv.GetEndPoint( 1 ) + dir.Negate() * ClsRevitUtil.CovertToAPI( 100 ) +
                             txtDir * ClsRevitUtil.CovertToAPI( 200 ) ;
              TextNote tx = TextNote.Create( doc, vActv.Id, txtPoint, maxWidth, ( i * dist / 1000.0 ).ToString() + ".0",
                opts ) ;
