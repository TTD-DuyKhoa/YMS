  ...
  class CmdCreatePile
  {
    ...
    public bool Excute()
    {
      try {
        ...
        string
          plateSize = frm.CmbTopPlateSize
            .Text ; /* (frm.CmbTopPlateSize.Text != "任意サイズ") ?  : $"PL-{frm.NmcPLH.Value}x{frm.NmcPLW.Value}x{frm.NmcPLD}";*/
        ...
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtExJointPlSizeFO1.Text,
          frm.txtExJointPlSizeFO2.Text, frm.txtExJointPlSizeFO3.Text ) ;
        ...
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtExJointPlSizeFI1.Text,
          frm.txtExJointPlSizeFI2.Text, frm.txtExJointPlSizeFI3.Text ) ;
        jpd.PlateCount = ClsCommonUtils.ChangeStrToInt( frm.txtExJointPlSizeFI4.Text ) ;
        pData.extensionPileData.PlateFlangeInSide = jpd ;
        jpd = new JointPlateData() ;
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtExJointPlSizeW1.Text,
          frm.txtExJointPlSizeW2.Text, frm.txtExJointPlSizeW3.Text ) ;
        ...
        jpd = new JointPlateData() ;
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtJointPlSizeFO1.Text,
          frm.txtJointPlSizeFO2.Text, frm.txtJointPlSizeFO3.Text ) ;
        jpd.PlateCount = ClsCommonUtils.ChangeStrToInt( frm.txtJointPlSizeFO4.Text ) ;
        pData.jointDetailData.PlateFlangeOutSide = jpd ;
        jpd = new JointPlateData() ;
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtJointPlSizeFI1.Text,
          frm.txtJointPlSizeFI2.Text, frm.txtJointPlSizeFI3.Text ) ;
        jpd.PlateCount = ClsCommonUtils.ChangeStrToInt( frm.txtJointPlSizeFI4.Text ) ;
        pData.jointDetailData.PlateFlangeInSide = jpd ;
        jpd = new JointPlateData() ;
        jpd.PlateSize = Master.ClsPileTsugiteCsv.CreatePlateName( frm.txtJointPlSizeW1.Text, frm.txtJointPlSizeW2.Text,
          frm.txtJointPlSizeW3.Text ) ;
        ...
        pData.topPlateData = new TopPlateData { PlateSize = plateSize } ;
        ...
        while ( ! cancel ) {
          using ( Transaction tr = new Transaction( _doc ) ) {
            ...
            try {
              ...
              ElementId eId = PilePillerSuper.CreatePile( _doc, point, 0, pData, frm.CmbKoudaiName.Text, XYZ.Zero,
                level, pOff ) ;
              //個別情報追加
              ...
              if ( frm.ChkExPile.Checked ) {
                //継足し杭配置
                eId = PilePillerSuper.CreateExtraPile( _doc, point, 0, pData, frm.CmbKoudaiName.Text, XYZ.Zero, level,
                  PilePillerSuper.CalcLevelOffsetExtensionPileLevel( level, kData.BaseLevel,
                    (double) frm.NmcOffset.Value, ohbikiSize.Height, nedaSize.Height ) ) ;
