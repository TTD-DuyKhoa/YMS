            ...
            try
            {
                ...
                switch (gid)
                {
                    ...
                    case FrmGrouping.GroupingId.GroupView:
                        ...
                        {
                            List<ElementId> ids = (from data in unionMaterials where data.m_KodaiName == koudaiName select data.m_ElementId).ToList();
                    ...
                    case FrmGrouping.GroupingId.KoudaiView:
                        List<ElementId> lstId = (from data in unionMaterials where data.m_KodaiName == f.selectKoudaiName select data.m_ElementId).ToList();
                        List<ElementId> lstId2 = f.GetEditModelData().GetKoudaiData(f.selectKoudaiName).GetListWariateElementId();
