...
namespace YMS_gantry.Command
{
    class CmdCreateStiffener
    {
        ...
        public Result Excute()
        {
            ...
            if (frm.ChkNeda.Checked) { filterList.Add("根太"); filterList.Add("主桁"); }
            if (frm.ChkOhbiki.Checked) { filterList.Add("大引"); filterList.Add("桁受"); }
            if (frm.ChkShikigeta.Checked) { filterList.Add("敷桁"); }
            ...
            try
            {
                ...
                if (frm.RbtAuto.Checked)
                {
                    ...
                    MaterialSuper[] materials = MaterialSuper.Collect(_doc).Where(x => ids.Contains(x.m_ElementId) && x.m_KodaiName == frm.CmbKoudaiName.Text).ToArray();
                    ...
                    x.m_KodaiName == frm.CmbKoudaiName.Text && (x.GetType().Equals(typeof(Ohbiki))|| x.GetType().Equals(typeof(Shikigeta)))).Select(x => x.m_ElementId).ToList();

                    List<ElementId> nedaId = MaterialSuper.Collect(_doc).Where(x => x.m_ElementId != null &&
                    x.m_KodaiName == frm.CmbKoudaiName.Text && x.GetType().Equals(typeof(Neda))).Select(x => x.m_ElementId).ToList();

                    List<ElementId> pileId = MaterialSuper.Collect(_doc).Where(x => x.m_ElementId != null &&
                    x.m_KodaiName == frm.CmbKoudaiName.Text && x.GetType().Equals(typeof(PilePiller))).Select(x => x.m_ElementId).ToList();
                    ...
                    using (Transaction tr = new Transaction(_doc))
                    {
                        ...
                        if (ohbikiId.Count > 0)
                        {
                            ...
                            {
                                List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, neda.m_ElementId,serchIds:ohbikiId);
                                ...
                                foreach (XYZ pnt in intPnts)
                                {
                                    ...
                                    if(frm.CmbSizeType.Text=="プレート")
                                    {
                                        ...
                                        {
                                            XYZ point = c.GetEndPoint(0) + ts.BasisY.Negate().Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                            ...
                                            point = c.GetEndPoint(0) + ts.BasisY.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                                norm * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10);
                                        ...
                                        else if (pnt.DistanceTo(c.GetEndPoint(1)) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10))
                                        {
                                            ...
                                            XYZ point = c.GetEndPoint(1) + ts.BasisY.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                                norm * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10);
                                            CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm, sym, sym.Name, -ms.Height / 2);
                                            point = c.GetEndPoint(1) + ts.BasisY.Negate().Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                                norm * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10);
                                            CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm.Negate(), sym, sym.Name, -ms.Height / 2);
                                        }
                                        else
                                        {
                                            XYZ point = pnt + ts.BasisY.Negate().Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff);
                                            CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm, sym, sym.Name, -ms.Height / 2);
                                    ...
                                    {
                                        horOff = (ms.Width / 2);
                                        string jSize = System.IO.Path.GetFileNameWithoutExtension(path).Replace("スチフナージャッキ_","").Replace("_V","");
                                        double jPL = jSize.Contains("DWJ") ? jSize.Contains("50") ? 100 : 200 : jSize.Contains("30") ? 175 : 75;
                                        ...
                                        if (pnt.DistanceTo(c.GetEndPoint(0)) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10))
                                        {
                                            ...
                                            CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm, sym, sym.Name, -ms.Height + sS.FrangeThick,true);
                                            point = c.GetEndPoint(0) + ts.BasisY.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                                lineD * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(off2);
                                            CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm.Negate(), sym, sym.Name, -ms.Height + sS.FrangeThick, true);
                                        }
                                        else if (pnt.DistanceTo(c.GetEndPoint(1)) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10))
                                        {
                                            XYZ point = c.GetEndPoint(1) + ts.BasisY.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                        ...
                        //大引　敷桁
                        List<MaterialSuper> ohbikis = materials.Where(x => x != null&&(x.GetType() == typeof(Ohbiki)|| x.GetType() == typeof(Shikigeta))).ToList();
                        if (ohbikiId.Count > 0)
                        {
                            foreach (MaterialSuper ohbiki in ohbikis)
                            {
                                List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, ohbiki.m_ElementId,serchIds:nedaId);
                ...
                {
                    bool end = false;
                    YMSPickFilter filter = new YMSPickFilter(_doc, new List<string> { "根太", "主桁", "大引", "桁受", "敷桁" });
                    using (Transaction tr = new Transaction(_doc))
                    {
                        ...
                        while (!end)
                        {
                            ...
                            if (pickId == ElementId.InvalidElementId) { continue; }
                            MaterialSuper material = MaterialSuper.ReadFromElement(pickId, _doc);
                            if (material.m_Size == "") { continue; }
                            ...
                            pnt = GantryUtil.GetClosestPointOnVector(c.GetEndPoint(1) - c.GetEndPoint(0), c.GetEndPoint(0),pnt);
                            double horOff = (ms.Thick / 2) + 1;
                            ...
                            if(frm.CmbSizeType.Text.Contains("プレート"))
                            {
                                ...
                                else if (pnt.DistanceTo(c.GetEndPoint(1)) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10))
                                {
                                    ...
                                    point = pnt + ts.BasisY.Negate().Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(horOff) +
                                        norm * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(10);
                                    CreateStiffener(frm.CmbKoudaiName.Text, f, point, norm.Negate(), sym, sym.Name, -ms.Height / 2);
                                }
                                else
        ...
        private void CreateStiffener(string koudaiName,Face f,XYZ point,XYZ normal,FamilySymbol sym,string size,double offset, bool isJack=false)
        {
            FamilyInstance ins=_doc.Create.NewFamilyInstance(f, point, normal, sym);
            if (ins == null) { return; }
            RevitUtil.ClsRevitUtil.SetParameter(_doc, ins.Id, DefineUtil.PARAM_HOST_OFFSET, RevitUtil.ClsRevitUtil.CovertToAPI(offset));
