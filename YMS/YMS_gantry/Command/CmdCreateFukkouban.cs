...
namespace YMS_gantry.Command
{
  class CmdCreateFukkouban
  {
        ...
        if ( f.RbtFace.Checked ) {
          ...
          using ( Transaction tr = new Transaction( _doc ) ) {
            ...
            try {
              ...
              int hukuinCnt = (int) f.NmcHukuinCnt.Value ;
              int kyoutyouCnt = (int) f.NmcKyoutyouCnt.Value ;
              double w = RevitUtil.ClsCommonUtils.ChangeStrToDbl(
                f.CmbSize.Text.Substring( f.CmbSize.Text.LastIndexOf( "X" ) + 1 ) ) ;
              double hukuinSize = w != 0 ? w : ( f.CmbSize.Text.EndsWith( "2000" ) ) ? 2000 : 3000 ;
              List<XYZ> FukkouList =
                calcFukkoubanPlacement( origin, vecHukuin, vecKyoutyou, hukuinCnt, kyoutyouCnt, hukuinSize ) ;
              XYZ adVec = ( ! isLeft )
                ? vecKyoutyou.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( 1000 )
                : XYZ.Zero ;
              foreach ( XYZ pnt in FukkouList ) {
                DefineUtil.eFukkoubanType hType = ( hukuinSize == 2000 ) ? eFukkoubanType.TwoM : eFukkoubanType.Three ;
                ElementId idnew = Fukkouban.CreateFukkoubanOnFace( _doc, f.CmbMaterial.Text, pnt, hType, f.CmbSize.Text,
                  adjustAngle, refer, vecHukuin, adVec ) ;
                //個別情報追加
          ...
          try {
            ...
            using ( Transaction tr = new Transaction( _doc ) ) {
              ...
              foreach ( XYZ pnt in FukkouList ) {
                DefineUtil.eFukkoubanType hType = ( hukuinSize == 2000 ) ? eFukkoubanType.TwoM : eFukkoubanType.Three ;
                ElementId idnew = Fukkouban.CreateFukkouban( _doc, f.CmbMaterial.Text, pnt, hType, f.CmbSize.Text,
                  adjustAngle, adVec, level: baselevel, levelOffset: offset ) ;
                //個別情報追加
                Fukkouban fukkouban = new Fukkouban() ;
                fukkouban.m_ElementId = idnew ;
                fukkouban.m_KodaiName = f.CmbKoudaiName.Text ;
    ...
    private List<XYZ> calcFukkoubanPlacement( XYZ origin, XYZ vecHukuin, XYZ vecKyoutyou, int hukuinCnt,
      int kyoutyouCnt, double hukuinSize )
    {
