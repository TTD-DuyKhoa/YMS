    ...
    class CmdCreateTeiketsuHojo
    {
        ...
        public bool Excute()
        {
            try
            {
                ...
                if (frm.RbtAuto.Checked)
                {
                    ...
                    using (Transaction tr = new Transaction(_doc))
                    {
                       ...
                       var ms = MaterialSuper.Collect(_doc).Where(x => x.m_KodaiName == frm.CmbKoudaiName.Text);
                        if (ms==null||ms.Count() < 1) { return false; }
                        ...
                        if (frm.ChkUp.Checked)
                        {
                            ...
                            //根太-大引、根太-敷桁                           
                            if (!CreateKetazaiHojo(materilas, frm.CmbKoudaiName.Text, frm.CmbUpSize.Text, frm.RbtBulmanUp.Checked))
                            {
                                retMs.Add("上部工締結補助材の配置に失敗しました");
                            }

                            //水平ブレス
                            if (!CreateHorizontalHojo(materilas, frm.CmbKoudaiName.Text, frm.CmbUpSize.Text, frm.RbtBulmanUp.Checked))
                            {
                        ...
                        {
                            //垂直ブレス
                            if (!CreateVerticalBraceHojo(materilas, frm.CmbKoudaiName.Text, frm.CmbBtmSize.Text, frm.RbtBulmanBtm.Checked))
                            {
                                retMs.Add("垂直ブレース用締結補助材の配置に失敗しました");
                            }

                            //水平ツナギ
                            if (!CreateHorizontalJointHojo(materilas, frm.CmbKoudaiName.Text, frm.CmbBtmSize.Text, frm.RbtBulmanBtm.Checked))
                            {
                        ...
                        {
                            if (!CreateNeko(materilas, frm.CmbHolBrace.Text, frm.CmbHolJoint_Web.Text,frm.CmbHolJoint_Frange.Text, frm.CmbVertBrace_Web.Text,frm.CmbVertBrace_Frange.Text,frm.CmbKoudaiName.Text))
                            {
                ...
                {
                    string type = frm.RbtNekoFree.Checked ? Master.ClsTeiketsuHojoCsv.TypeNeko : frm.RbtBulFree.Checked ? Master.ClsTeiketsuHojoCsv.TypeBulman : Master.ClsTeiketsuHojoCsv.TypeRikiman;
                    string size = frm.RbtNekoFree.Checked ? frm.CmbNekoFree.Text : frm.RbtBulFree.Checked ? frm.CmbBulFree.Text : frm.CmbRikiFree.Text;
                    using (TransactionGroup tG=new TransactionGroup(_doc))
                    {
                        FamilySymbol sym = null;
                        string path = Master.ClsTeiketsuHojoCsv.GetFamilyPath(type, size);
                        type = frm.RbtNekoFree.Checked ? Master.ClsTeiketsuHojoCsv.HojoType_B : frm.RbtBulFree.Checked ? Master.ClsTeiketsuHojoCsv.TypeBulman : Master.ClsTeiketsuHojoCsv.TypeRikiman;
        ...
        private bool CreateKetazaiHojo(List<MaterialSuper> materials, string koudaiName, string size, bool isBulman)
        {
            try
            {
                string teiketsuType = Master.ClsTeiketsuHojoCsv.GetTeiketsuType(isBulman ? Master.ClsTeiketsuHojoCsv.TypeBulman : Master.ClsTeiketsuHojoCsv.TypeRikiman, size);
                if (teiketsuType != Master.ClsTeiketsuHojoCsv.TeiketsuC) { return true; }

                //根太
                List<Neda> nedas = materials.Select(x => x as Neda)
                    .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).ToList();
                //大引
                List<ElementId> ohbikis = materials.Select(x => x as Ohbiki)
                   .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).Select(x => x.m_ElementId).ToList();
                //敷桁
                List<ElementId> shikigetas = MaterialSuper.Collect(_doc).Where(x => x.m_ElementId != null &&
                     x.m_KodaiName == koudaiName && x.GetType().Equals(typeof(Shikigeta))).Select(x => x.m_ElementId).ToList();
                ...
                FamilySymbol sym = null;
                string path = Master.ClsTeiketsuHojoCsv.GetFamilyPath(isBulman ? Master.ClsTeiketsuHojoCsv.TypeBulman : Master.ClsTeiketsuHojoCsv.TypeRikiman, size);
                ...
                foreach (Neda neda in nedas)
                {
                    ...
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, neda.m_ElementId, ignorelist: teikesuList());
                    ...
                    foreach ((XYZ, ElementId) itm in ohbikiIntPnts)
                    {
                        ...
                        (itm.Item1+(vecN*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nS.Width/2))+(crsN*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(oS.Width/2))),
                        (itm.Item1+(vecN.Negate()*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nS.Width/2))+(crsN*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(oS.Width/2))),
                        (itm.Item1+(vecN*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nS.Width/2))+(crsN.Negate()*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(oS.Width/2))),
                        (itm.Item1+(vecN.Negate()*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nS.Width/2))+(crsN.Negate()*RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(oS.Width/2)))
                    });

                        foreach (XYZ p in pnts)
                        {
                            if (GantryUtil.IsXYZInList(p, intersectPnt)) { continue; }
        ...
        private bool CreateHorizontalHojo(List<MaterialSuper> materials, string koudaiName, string size, bool isBulman)
        {
            try
            {
                ...
                {
                    List<HorizontalBrace> horBraces = materials.Select(x => x as HorizontalBrace)
                        .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).ToList();
                    if (horBraces.Count < 1) { return true; }
                    ...
                    foreach (HorizontalBrace brace in horBraces)
                    {
                        ...
                        if (brace.MaterialSize().Shape != MaterialShape.L) { continue; }
                        List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, brace.m_ElementId, ignorelist: teikesuList());
                        ...
                        if (ketaIds.Count > 0)
                        {
                            foreach (ElementId id in ketaIds)
                            {
                                ...
                                XYZ nearP = (pnts[0].DistanceTo(c.GetEndPoint(0)) < pnts[0].DistanceTo(c.GetEndPoint(1))) ? c.GetEndPoint(0) : c.GetEndPoint(1);
                                ...
                                if (teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuC && (isCType))
                                {
                                    ...
                                    XYZ placeP = p + bTs.BasisY.Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(ms.Height / 2);
                                ...
                                else if (teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuG && (!isCType))
                                {
                                    ...
                                    XYZ placeP = p + bTs.BasisY.Negate().Normalize() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(ms.Height / 2);
                                    dataList.Add(new FamilyInstanceCreationData(f, placeP, normal, sym));
                                }
                            }
                        }
        ...
        private bool CreateVerticalBraceHojo(List<MaterialSuper> materials, string koudaiName, string size, bool isBulman)
        {
            ...
            {
                //垂直ブレース
                List<VerticalBrace> verBraces = materials.Select(x => x as VerticalBrace)
                    .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).ToList();
                if (verBraces.Count < 1) { return true; }
                ...
                foreach (VerticalBrace brace in verBraces)
                {
                    ...
                    foreach (ElementId id in pillerIds)
                    {
                        ...
                        bool isFlange = GantryUtil.AreComponentsSameSign(pillerHandle, bracePlaneVec) || GantryUtil.AreComponentsSameSign(pillerHandle.Negate(), bracePlaneVec);
                        if (pnts.Count > 1)
                        {
                            XYZ nearP = (pnts[0].DistanceTo(c.GetEndPoint(0)) < pnts[0].DistanceTo(c.GetEndPoint(1))) ? c.GetEndPoint(0) : c.GetEndPoint(1);
                            bool isUpSize = nearP.DistanceTo(c.GetEndPoint(1)) < 0.1;
                            XYZ p = isUpSize ? pnts.OrderBy(x => x.DistanceTo(c.GetEndPoint(0))).First() : pnts.OrderBy(x => x.DistanceTo(c.GetEndPoint(1))).ToList().ElementAt(1);
                            ...
                            else if (!isFlange && teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuLA)
                            {
                                ...
                                p = isUpSize ? laList.OrderBy(x => x.DistanceTo(c.GetEndPoint(0))).First() : laList.OrderBy(x => x.DistanceTo(c.GetEndPoint(0))).ToList().Last();
                                double moveDist = (bS.Height / 4) / Math.Cos(bRotate);
                                XYZ pnt = p + XYZ.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(moveDist);
                                //面
                                Face f = GantryUtil.GetSpecifyFaceOfFamilyInstance(braceIns, ts.BasisZ.Negate());
        ...
        /// <param name="isBulman"></param>
        /// <returns></returns>
        private bool CreateHorizontalJointHojo(List<MaterialSuper> materials, string koudaiName, string size, bool isBulman)
        {
            try
            {
                //水平ツナギ
                List<ElementId> joints = materials.Select(x => x as HorizontalJoint)
                    .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).Select(x => x.m_ElementId).ToList();
                if (joints.Count < 1) { return true; }

                //柱、杭
                List<PilePillerSuper> pillers = materials.Select(x => x as PilePillerSuper)
                    .Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName).ToList();
                ...
                foreach (PilePillerSuper pil in pillers)
                {
                    ...
                    List<ElementId> intsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, pil.m_ElementId, ignorelist: teikesuList(), serchIds: joints);
                    //柱に接するツナギ材だけを収集
                    ...
                    foreach (ElementId jId in jointIds)
                    {
                        ...
                        //ツナギ材に接する別のツナギ材を取得
                        List<ElementId> jintsId = RevitUtil.ClsRevitUtil.GetIntersectFamilys(_doc, jId, ignorelist: teikesuList(), serchIds: jointIds);
                        List<ElementId> jjIds = jintsId.Where(x => joints.Contains(x) && intsId.Contains(x)).ToList();
                        //このツナギがフランジ方向に平行か
                        bool isFlange = GantryUtil.AreComponentsSameSign(pillerIns.HandOrientation, jointVec) || GantryUtil.AreComponentsSameSign(pillerIns.HandOrientation, jointVec.Negate());

                        //ツナギと柱の交点
                        List<XYZ> pnts = GantryUtil.FindIntersectionPnts(_doc, pil.m_ElementId, jId);
                        if (pnts.Count < 1) { continue; }
                        XYZ nearP = (pnts[0].DistanceTo(jC.GetEndPoint(0)) < pnts[0].DistanceTo(jC.GetEndPoint(1))) ? jC.GetEndPoint(0) : jC.GetEndPoint(1);
                        //交点群が近いほうの点からのベクトル
                        ...
                        if (jins.Symbol.Name.Contains("切梁受"))
                        {
                            ...
                            jC = Line.CreateBound(jC.GetEndPoint(0) + XYZ.BasisZ.Normalize() * kiribariH, jC.GetEndPoint(1) + XYZ.BasisZ.Normalize() * kiribariH);
                        ...
                        bool needNTForjoint = false; bool isNeedWebNtUp = true; bool needNtcN = false; bool needNtcF = false;
                        bool hasCrossUp = false; bool noCross = true;
                        ...
                        foreach (ElementId id in jjIds)
                        {
                            ...
                            if (pntsJ.Count < 1) { continue; }
                            XYZ nearPJ = (pntsJ[0].DistanceTo(jjC.GetEndPoint(0)) < pntsJ[0].DistanceTo(jjC.GetEndPoint(1))) ? jjC.GetEndPoint(0) : jjC.GetEndPoint(1);
                            ...
                            if (jjins.Symbol.Name.Contains("切梁受"))
                            {
                                ...
                                jjC = Line.CreateBound(jjC.GetEndPoint(0) + XYZ.BasisZ.Normalize() * kiribariH, jjC.GetEndPoint(1) + XYZ.BasisZ.Normalize() * kiribariH);
                            ...
                            else if (isFlange && intIns.Id.Equals(jId))
                            {
                                if (teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuC)
                                {
                                    ...
                                    if (f == null) { continue; }
                                    XYZ p = pntsJ.Where(x => nearPJ.Z < x.Z).OrderBy(x => x.DistanceTo(nearPJ)).ToList().OrderBy(x => x.DistanceTo(pillerP)).First();
                                    XYZ plP = p + jins.GetTransform().BasisY * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(jH.MaterialSize().Width * 0.75)
                                               + jjins.GetTransform().BasisY * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(jH.MaterialSize().Width * 0.75);
                                    dataList.Add(new FamilyInstanceCreationData(f, plP, (pillerP - p).Normalize(), sym));
                                }
                                noCross = false;
                                needNTForjoint = true;
                                if (GantryUtil.AreComponentsSameSign(jointVec.Normalize(), jjins.GetTotalTransform().BasisY.Negate().Normalize()))
                                {
                                    needNtcN = true;
                                }
                                else
                        ...
                        if (isFlange)
                        {
                            ...
                            else if (needNTForjoint && teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuNT || teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuC)
                            {
                                XYZ p = pnts.Where(x => nearP.Z < x.Z).OrderBy(x => x.DistanceTo(nearP)).ToList().Last();
                        ...
                        {
                            //ウェブ側で上部にNTが来る
                            if (!isNeedWebNtUp && (teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuC || teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuNT))
                            {
                            ...
                            //下にＮＴが来る
                            else if ((teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuC || teiketsuType == Master.ClsTeiketsuHojoCsv.TeiketsuNT))
                            {
                                XYZ p = pnts.Where(x => nearP.Z < x.Z).OrderBy(x => x.DistanceTo(nearP)).ToList().First();
                                XYZ plP = p + jointVec * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(13);
        ...
        /// <returns></returns>
        private bool CreateNeko(List<MaterialSuper> materials, string horBraceSize, string holTsunagiSize_Web, string holTsunagiSize_Frange, string vertBraceSize_Web, string vertBraceSize_Frange, string koudaiName)
        {
            try
            {
                List<FamilyInstanceCreationData> dataList = new List<FamilyInstanceCreationData>();
                ...
                if (!string.IsNullOrEmpty(horBraceSize) && Holsym != null&&horBraceSize!="なし")
                {
                    ...
                    foreach (HorizontalBrace hBrace in horBraces)
                    {
                        ...
                        XYZ p1 = c.GetEndPoint(0); XYZ p2 = c.GetEndPoint(1);
                        XYZ vec = (p2 - p1).Normalize();
                        if (!RevitUtil.ClsGeo.IsLeft(ts.BasisY.Normalize(), vec))
                        {
                            p1 = c.GetEndPoint(1); p2 = c.GetEndPoint(0);
                            vec = (p2 - p1).Normalize();
                        }
                        ...
                        if (f != null)
                        {
                            ...
                            dataList.Add(new FamilyInstanceCreationData(f, p2 + vec.Negate() * (nekoSizeHol), vec.Normalize(), Holsym));
                        }
                    }
                    ...
                    foreach (VerticalBrace brace in verBraces)
                    {
                        ...
                        foreach (ElementId id in pillerIds)
                        {
                            ...
                            if (pnts.Count > 1)
                            {
                                ...
                                if (isFlange&&vertBraceSize_Frange != "なし" && !string.IsNullOrEmpty(vertBraceSize_Frange) && VerFSym != null)
                                {
                                    foreach (XYZ pn in pnts)
                                    {
                                        if (RevitUtil.ClsRevitUtil.CovertFromAPI(c.Distance(pn)) < bS.Thick)
                                    ...
                                    if (pList.Count >=2)
                                    {
                                        ...
                                        {
                                            ps = pntMid + vecP.Normalize().Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nekoSizeF / 2);
                                            vecP = vecP.Negate();
                                        }
                                        Face f = GantryUtil.GetSpecifyFaceOfFamilyInstance(braceIns, ts.BasisZ.Negate());
                                        if (f != null)
                                    ...
                                    }else if(pList.Count==1)
                                    {
                                        ...
                                        if (f != null)
                                        {
                                            XYZ p = pList.First() + braceVec.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(nekoSizeF / 2);
                                            dataList.Add(new FamilyInstanceCreationData(f, p, braceIns.HandOrientation, VerFSym));
                                        }
                                    }
                                   
                                }
                                else if(!isFlange&& vertBraceSize_Web != "なし" && !string.IsNullOrEmpty(vertBraceSize_Web) && VerWSym != null)
                                {
                                    //上部の点のみ対象
                                    foreach (XYZ pn in pnts)
                                    {
                                    ...
                                    if (pList.Count >= 2)
                                    {
                                        ...
                                        XYZ pntMid = ps + vecP / 2;
                                        Face Hf = GantryUtil.GetSpecifyFaceOfFamilyInstance(pillerIns, new XYZ(vecP.X, vecP.Y, 0).Normalize());
                                        Face noHF = GantryUtil.GetSpecifyFaceOfFamilyInstance(pillerIns, new XYZ(vecP.X, vecP.Y, 0).Negate());
                                        XYZ pPi = ((pillerIns.Location) as LocationPoint).Point;
                                        if (Hf != null && noHF != null)
                                        {
                                            if (!RevitUtil.ClsGeo.IsLeft(ts.BasisZ.Normalize(), braceIns.HandOrientation.Normalize()) || RevitUtil.ClsGeo.IsLeft(ts.BasisZ.Normalize(), vecP.Normalize()))
                                            {
                                                ps = ps - XYZ.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((nekoSizeW / 2) + (bS.Height / 2));
                                                dataList.Add(new FamilyInstanceCreationData(noHF, ps, XYZ.BasisZ, VerWSym));
                                                if (pList.Count >= 4)
                                                {
                                                    pe = pe - XYZ.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((nekoSizeW / 2) + (bS.Height / 2));
               ...
               //水平ツナギ
                ...
                FamilySymbol tWSym=null;FamilySymbol tFSym=null;
                ...
                {
                    //継材
                    List<ElementId> joints = materials.Select(x => x as HorizontalJoint).Where(x => x != null && x.m_ElementId != ElementId.InvalidElementId && x.m_KodaiName == koudaiName)
                    ...
                    if(joints.Count>0)
                    {
                        foreach (PilePillerSuper pil in pillers)
                        {
                            ...
                            foreach (ElementId jId in jointIds)
                            {
                                ...
                                //ツナギ材のマテリアルクラス
                                HorizontalJoint mainJ = HorizontalJoint.ReadFromElement(jId, _doc) as HorizontalJoint;
                                if (mainJ == null) { continue; }
                                //接するものをすべて考慮し、あとから配置するためフラグを持っておく
                                bool hasCrossNear = false; bool hasCrossFar = false;
                                bool hasCrossUp = false; bool noCross = true; bool hasNext = false;
                                ...
                                if (isFlange && !string.IsNullOrEmpty(holTsunagiSize_Frange)&& holTsunagiSize_Frange != "なし"&&tFSym!=null)
                                {
                                    Face topFace = GantryUtil.GetTopFaceOfFamilyInstance(jins);
                                    Face btmFace = GantryUtil.GetBtmFaceOfFamilyInstance(jins);
                                    XYZ up = jins.GetTotalTransform().BasisZ.Z > 0 ? jins.GetTotalTransform().BasisZ : jins.GetTotalTransform().BasisZ.Negate();
                                    double leng = holTsunagiSize_Frange.StartsWith("B") ? 100 : 75;
                                    XYZ pntUp = null; XYZ pntBtm = null; XYZ normTop = null; XYZ normBtm = null;
                                    //継材中央位置にネコ材が来る
                                    if (pnts.Count >= 16 && (noCross || (hasCrossFar || hasCrossNear)))
                                    {
                                        ...
                                        if (RevitUtil.ClsGeo.IsLeft(jins.FacingOrientation.Normalize(), jins.HandOrientation.Normalize()))
                                        {
                                            norm = norm.Negate();
                                        }
                                        List<XYZ> topPnts = pnts.Where(x => x.Z > nearP.Z).OrderBy(x => x.DistanceTo(nearP)).ToList();
                                        List<XYZ> btmPnts = pnts.Where(x => x.Z < nearP.Z).OrderBy(x => x.DistanceTo(nearP)).ToList();
                                        if (topPnts.Count < 2) { continue; }
                                        XYZ mid = (topPnts.First() + topPnts.Last()) / 2;
                                        pntUp = mid; normTop = norm.Negate();
                                        pntUp = pntUp + jins.FacingOrientation.Normalize() * RevitUtil.ClsRevitUtil.CovertToAPI(pil.SteelSize.FrangeThick / 2);
                                        if (btmPnts.Count < 2) { continue; }
                                        mid = (btmPnts.First() + btmPnts.Last()) / 2;
                                        pntBtm = mid; normBtm = norm;
                                        pntBtm = pntBtm + jins.FacingOrientation.Normalize() * RevitUtil.ClsRevitUtil.CovertToAPI(pil.SteelSize.FrangeThick / 2);
                                    ...
                                    else if (hasNext ||isHlf)
                                    {
                                        ...
                                        XYZ mid = nearP + up * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((mainJ.MaterialSize().Height) / 2);
                                        pntUp = mid; normTop = norm.Negate();
                                        //pntUp = pntUp + jins.GetTotalTransform().BasisY.Normalize() * RevitUtil.ClsRevitUtil.CovertToAPI(pil.SteelSize.FrangeThick/2);
                                        mid = nearP + up.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit((mainJ.MaterialSize().Height) / 2);
                                        pntBtm = mid; normBtm = norm;
                                    ...
                                    if (pntUp != null && normTop != null && topFace != null)
                                    {
                                        ...
                                        foreach ((XYZ, XYZ, Face,bool) p in lstOrderP)
                                        {
                                            if (p.Item1.DistanceTo(pntUp) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(leng) &&
                                                (GantryUtil.AreComponentsSameSign(p.Item2, normTop) || GantryUtil.AreComponentsSameSign(p.Item2, normTop.Negate())))
                                            {
                                                has = true;
                                                break;
                                            }
                                        }
                                        if (!has) { lstOrderP.Add((pntUp, normTop, topFace,isFlange)); }
                                    }
                                    ...
                                    if (pntBtm != null && normBtm != null && btmFace != null)
                                    {
                                        ...
                                        foreach ((XYZ, XYZ, Face,bool) p in lstOrderP)
                                        {
                                            if (p.Item1.DistanceTo(pntBtm) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(leng) &&
                                                (GantryUtil.AreComponentsSameSign(p.Item2, normBtm) || GantryUtil.AreComponentsSameSign(p.Item2, normBtm.Negate())))
                                            {
                                                has = true;
                                                break;
                                            }
                                        }
                                        if (!has) { lstOrderP.Add((pntBtm, normBtm, btmFace,isFlange)); }
                                    }
                                }
                                else if(!isFlange&& !string.IsNullOrEmpty(holTsunagiSize_Web) && holTsunagiSize_Web != "なし" && tWSym != null)
                                {
                                    double thicks = holTsunagiSize_Web.StartsWith("B") ? 10 : holTsunagiSize_Web.StartsWith("A") ? 9 : 6;
                                    Face Hf = GantryUtil.GetSpecifyFaceOfFamilyInstance(pillerIns, new XYZ(jointVec.X, jointVec.Y, 0).Normalize());
                                    Face noHF = GantryUtil.GetSpecifyFaceOfFamilyInstance(pillerIns, new XYZ(jointVec.X, jointVec.Y, 0).Negate());
                                    XYZ vecUP = (jins.GetTransform().BasisZ.Z > 0) ? jins.GetTransform().BasisZ : jins.GetTransform().BasisZ.Negate();
                                    Curve lC = Line.CreateBound(jC.GetEndPoint(0) + vecUP.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2), jC.GetEndPoint(1) + vecUP.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2));
                                    List<XYZ> lowerPnts = pnts.Where(x => ToCurve(jC, x).Z > 0 && RevitUtil.ClsRevitUtil.CovertFromAPI(lC.Distance(x)) < RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(thicks / 2)).ToList();
                                    XYZ vecToC = ToCurve(jC, pillerP);
                                    bool isLeft = RevitUtil.ClsGeo.IsLeft(new XYZ(jointVec.X, jointVec.Y, 0).Normalize(), new XYZ(vecToC.X, vecToC.Y, 0));
                                    XYZ pntN = null, pntF = null, normN = isLeft ? XYZ.BasisZ : XYZ.BasisZ.Negate(), normF = isLeft ? XYZ.BasisZ.Negate() : XYZ.BasisZ;
                                    double half = Math.Abs((nekoSizeW / 2) - RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2));

                                    //フランジの片側にしか接していない
                                    if (isHlf)
                                    {
                                        pntF = lowerPnts.OrderBy(x => x.DistanceTo(nearP)).ThenBy(x => x.Z).Last() + vecUP * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2);
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            pntF = lowerPnts.OrderBy(x => x.DistanceTo(nearP)).ThenBy(x => x.Z).Last() + vecUP * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2);
                                            pntN = lowerPnts.OrderBy(x => x.DistanceTo(nearP)).ThenBy(x => x.Z).First() + vecUP * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(mainJ.MaterialSize().Height / 2);
                                        }
                                    }
