...
namespace YMS_gantry
{
  public class CmdCreateSlope
  {
    ...
    private static string StiffenerNameTableRawText =>
      @"StiffenerType,OhbikiSize,PlateFamilyName,SymbolName
...
JackDwj,H500x500,DWJ-50,50H(構台)" ;
    ...
    private static Lazy<StiffenerPlateRec[]> StiffenerTable { get ; } = new Lazy<StiffenerPlateRec[]>( () =>
    {
      ...
      foreach ( var line in textArray ) {
        if ( ! Enum.TryParse<StiffenerType>( line.ElementAtOrDefault( 0 ), out var type ) ) {
          continue ;
        }
        ...
        result.Add( new StiffenerPlateRec
        {
          ...
          SymbolName = string.IsNullOrWhiteSpace( line.ElementAtOrDefault( 3 ) )
            ? line.ElementAtOrDefault( 2 )
            : line.ElementAtOrDefault( 3 ),
        } ) ;
      }
      ...
      using ( var transaction = new Transaction( doc ) ) {
        ...
        #endif
        {
          ...
          if ( slopeInputData.StyleList.All( x => x.SlopeType == SlopeStyle.Entrance ) ) {
            ...
            taskDialog.AddCommandLink( TaskDialogCommandLinkId.CommandLink1, /*$"スパン1 : 上 → スパン{n} : 下",*/
              $"スパン1 ↘ スパン{n}" ) ;
            taskDialog.AddCommandLink( TaskDialogCommandLinkId.CommandLink2, /*$"スパン1 : 下 ← スパン{n} : 上",*/
              $"スパン1 ↙ スパン{n}" ) ;
          ...
          //foreach (var slopeInputData in targetSlopeData)
          {
            ...
            var kodaiData = data.ListKoudaiData
              .FirstOrDefault( x => x.AllKoudaiFlatData.KoudaiName == slopeInputData.Name )?.AllKoudaiFlatData ;

            // 現状のスロープ角度情報
            var originalSlopeInputData = originalData.ListKoudaiData
              .FirstOrDefault( x => x.AllKoudaiFlatData.KoudaiName == slopeInputData.Name )?.SlopeData ;
            var originalAreaRangeSet =
              ConvertAreaRangeFromInputData( originalSlopeInputData, materials, kodaiData, inverse: true ) ;

            // 一度フラットに戻す
            TransformSloping( doc, kodaiData, slopeInputData, originalAreaRangeSet, true ) ;

            var flatMaterials = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == slopeInputData.Name )
              .ToArray() ;
            // 変更後のスロープ角度情報
            var areaRangeSet =
              ConvertAreaRangeFromInputData( slopeInputData, flatMaterials, kodaiData, inverse: false ) ;
    ...
    private static void TransformSloping( Document doc, AllKoudaiFlatFrmData kodaiData, KodaiSlopeModel slopeModel,
      AreaRange[] areaRangeSet, bool inverse )
    {
      ...
      foreach ( var ohbikiBottom in ohbikiBottoms ) {
        ...
        var ohbikis = materials.Select( x => x as OhbikiSuper ).Where( x => x != null && x.m_K == k ).ToArray() ;

        if ( ohbikis.Any() ) {
          //
          var ohbikiIds = ohbikis.Where( x => x != null && x.m_K == k ).Select( x => x.m_ElementId ).ToArray() ;
          ElementTransformUtils.MoveElements( doc, ohbikiIds, transform ) ;

          //H鋼以外
          if ( ohbikis.FirstOrDefault().m_Size.StartsWith( "[" ) ||
               ohbikis.FirstOrDefault().m_Size.StartsWith( "L" ) ) {
            var arearange =
              areaRangeSet.Where( x => x is AreaRangeSloped && x._OhbikiBottoms.Where( y => y._K == k ).Any() ) ;
            if ( arearange.Any() ) {
              ...
              //B
              var ohbikiB = ohbikis.Where( x => x != null && x.m_K == k && x.m_AttachSide == "B" )
                .Select( x => x.m_ElementId ).ToArray() ;
              //F
              var ohbikiF = ohbikis.Where( x => x != null && x.m_K == k && x.m_AttachSide == "F" )
                .Select( x => x.m_ElementId ).ToArray() ;
              ...
              if ( toB ) {
                if ( cArea._Type == SlopeStyle.Entrance ||
                     ( cArea._Type == SlopeStyle.Slope && k != cArea._StartOhbikiK ) ) {
                  ElementTransformUtils.MoveElements( doc, ohbikiB, dTra ) ;
                }

                if ( ( cArea._Type == SlopeStyle.Entrance && k != cArea._EndOhbikiK ) ||
                     ( cArea._Type == SlopeStyle.Slope && k != cArea._EndOhbikiK ) ) {
                  ElementTransformUtils.MoveElements( doc, ohbikiF, uTra ) ;
                }
              }
              else {
                if ( ( cArea._Type == SlopeStyle.Entrance && k != cArea._StartOhbikiK ) ||
                     ( cArea._Type == SlopeStyle.Slope && k != cArea._StartOhbikiK ) ) {
        ...
        // 杭の対応
        if ( ! inverse ) {
          var pileSetAtK = materials.Select( x => x as PilePillerSuper ).Where( x => x != null )
            .Where( x => x.m_KyoutyouNum == k ).ToArray() ;

          var levelSet = new FilteredElementCollector( doc ).OfClass( typeof( Level ) ).Cast<Level>().ToArray() ;
          ...
          if ( ! hasExtraPile && pillarData.ExtensionPile ) {
            ...
            mainPileData.PileCutLength =
              Math.Abs( ( extraPileData.extensionPileData.Length + ohbikiBottomMm ) - pillarData.PileCutLength ) ;
          }
          else if ( ! hasExtraPile && ! pillarData.ExtensionPile ) {
            if ( Math.Sign( ohbikiBottomMm ) == 1 && pillarData.PileCutLength < ohbikiBottomMm &&
                 kodaiData.BaseLevel != eBaseLevel.FukkouTop ) {
          ...
          foreach ( var srcPile in mainPileSet ) {
            ...
            if ( pillarData.IsCut ) {
              if ( hasExtraPile ) {
                ...
                var extraLevelOffset = PilePillerSuper.CalcLevelOffsetExtensionPileLevel( baseLevel,
                  kodaiData.BaseLevel, kodaiData.LevelOffset, ohbikiSize.Height, nedaSize.Height ) ;
                if ( ! kodaiData.ohbikiData.isHkou ) {
                  extraLevelOffset += ohbikiSize.Height ;
                }

                var extraId = PilePillerSuper.CreateExtraPile( doc, location.Point, location.Rotation, extraPileData,
                  kodaiData.KoudaiName, XYZ.Zero, baseLevel, extraLevelOffset + ohbikiBottomMm ) ;
              ...
              if ( kodaiData.BaseLevel == eBaseLevel.FukkouTop ) {
                double.TryParse( ClsRevitUtil.GetParameter( doc, srcInstance.Id, DefineUtil.PARAM_BASE_OFFSET ),
                  out var offsetMm ) ;
            ...
            var mainId = PilePillerSuper.CreatePile( doc, location.Point, location.Rotation, mainPileData,
              kodaiData.KoudaiName, XYZ.Zero, baseLevel, mainLevelOffset ) ;
            ...
            if ( pillarData.IsCut && ! hasExtraPile && ! pillarData.ExtensionPile ) {
              if ( Math.Sign( kodaiData.LevelOffset ) == -1 && kodaiData.BaseLevel == eBaseLevel.OhbikiBtm ) {
                ClsRevitUtil.SetParameter( doc, mainId, DefineUtil.PARAM_HOST_OFFSET,
                  RevitUtil.ClsRevitUtil.CovertToAPI( baseLevelMm ) ) ;
              }
            }
      ...
      if ( ! inverse ) {
        ...
        // 根太を分割すべき K
        var nedaSplitKSet = areaRangeSet.SelectMany( x => new int[] { x._StartOhbikiK, x._EndOhbikiK } ).Distinct()
          .Where( x => x != 0 && x != ohbikiSpanNum ).ToArray() ;

        // 分割対象の根太集合
        var splitingNedaSet = materials.Select( x => x as Neda ).Where( x => x != null )
          .Where( x => x.m_KodaiName == kodaiData.KoudaiName ).ToArray() ;

        var kodaiOrigin = BasePoint( doc, kodaiData ) ;
        var valueKSet = Enumerable.Range( 0, kodaiData.KyoutyouPillarPitch.Count() ).Select( x =>
          RevitUtil.ClsRevitUtil.CovertToAPI( kodaiData.KyoutyouPillarPitch.Skip( 1 ).Take( x ).Sum() ) ).ToArray() ;
        ...
        //    .ToArray();
        var valueHSet = Enumerable.Range( 0, kodaiData.NedaPitch.Count() ).Select( x =>
          RevitUtil.ClsRevitUtil.CovertToAPI( kodaiData.NedaPitch.Skip( 1 ).Take( x ).Sum() ) ).ToArray() ;
        ...
        foreach ( var neda in splitingNedaSet ) {
          ...
          var nedaPtSet = splitKSet.Append( neda.m_MinK ).Append( neda.m_MaxK ).OrderBy( x => x ).Select( x =>
              new { K = x, Pt = kodaiOrigin + nedaValueH * axisH + valueKSet.ElementAtOrDefault( x ) * axisK } )
            .ToArray() ;
          ...
          for ( int i = 0 ; i < nedaPtSet.Length - 1 ; i++ ) {
            ...
            using ( var sourceInstance = doc.GetElement( neda.m_ElementId ) as FamilyInstance ) {
              ...
              instanceId = MaterialSuper.PlaceWithTwoPoints( sourceInstance.Symbol, level.GetPlaneReference(), start,
                end, neda.LevelOffsetFeet ) ;
        ...
        splitedNedaSet = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == kodaiData.KoudaiName )
          .Select( x => x as Neda ).Where( x => x != null ).ToList() ;
      }

      // 根太と覆工板
      var fukkobanSet = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == kodaiData.KoudaiName )
        .Select( x => x as Fukkouban ).Where( x => x != null ).ToArray() ;
      foreach ( var areaRange in areaRangeSet ) {
        var minKFeet =
          ClsRevitUtil.CovertToAPI( kodaiData.KyoutyouPillarPitch.Skip( 1 ).Take( areaRange._StartOhbikiK ).Sum() ) ;
        var maxKFeet =
          ClsRevitUtil.CovertToAPI( kodaiData.KyoutyouPillarPitch.Skip( 1 ).Take( areaRange._EndOhbikiK ).Sum() ) ;
        ...
        var targetNedaSet = splitedNedaSet
          .Where( x => areaRange._StartOhbikiK <= x.m_MinK && x.m_MaxK <= areaRange._EndOhbikiK ).ToArray() ;
        ...
        var targetFukkobanSet = fukkobanSet
          ...
          .Where( x => minKFeet <= x.FeetK && ClsGeo.GEO_LT( x.FeetK, maxKFeet ) ).Select( x => x.X ).ToArray() ;
        ...
        if ( areaRange is AreaRangeFlat flatArea ) {
          ...
          if ( ! inverse ) {
            ...
            var slopePlane = doc.Create.NewReferencePlane( rotateAxisOrigin, rotateAxisOrigin + 10 * slopePlaneDir,
              cutDir, doc.ActiveView ) ;
            ...
            foreach ( var neda in targetNedaSet ) {
              ...
              using ( var familyInstance = doc.GetElement( neda.m_ElementId ) as FamilyInstance ) {
                ...
                nedaSlopedId = MaterialSuper.PlaceWithTwoPoints( symbol, slopePlane.GetReference(), nedaSlopedStart,
                  nedaSlopedEnd, RevitUtil.ClsRevitUtil.CovertToAPI( 0.5 * nedaHeight ) ) ;
            ...
            foreach ( var fukkoban in targetFukkobanSet ) {
              ...
              var instance = doc.Create.NewFamilyInstance( slopePlane.GetReference(), positionSloped, direction,
                familyInstance.Symbol ) ;

              ClsRevitUtil.SetParameter( doc, instance.Id, DefineUtil.PARAM_HOST_OFFSET,
                RevitUtil.ClsRevitUtil.CovertToAPI( nedaHeight ) ) ;
            ...
            var slopePlane = new FilteredElementCollector( doc ).OfClass( typeof( ReferencePlane ) )
              .Cast<ReferencePlane>().FirstOrDefault( x => x.Name == planeName ) ;
            if ( slopePlane != null ) {
              ClsRevitUtil.Delete( doc, slopePlane.Id ) ;
            }

            var evenLevel = new FilteredElementCollector( doc ).OfClass( typeof( Level ) ).Cast<Level>()
              .FirstOrDefault( x => x.Name == kodaiData.SelectedLevel ) ;
            ...
            var ohbikiTopSet = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == kodaiData.KoudaiName )
              .Select( x => x as OhbikiSuper ).Where( x => x != null )
              .Where( x => slopeRange._StartOhbikiK <= x.m_K && x.m_K <= slopeRange._EndOhbikiK )
              .Select( x => x.LevelOffsetFeet + 0.5 * x.SteelSize.HeightFeet ).ToArray() ;
            ...
            foreach ( var neda in targetNedaSet ) {
              ...
              var evenInstanceId = MaterialSuper.PlaceWithTwoPoints( familyInstance.Symbol,
                evenLevel.GetPlaneReference(), evenStart, evenEnd, 0.5 * nedaHeightFeet + ohbikiTopFeet ) ;
            ...
            foreach ( var fukkoban in targetFukkobanSet ) {
              ...
              var instance = doc.Create.NewFamilyInstance( evenLevel.GetPlaneReference(), evenPosition, evenDirection,
                familyInstance.Symbol ) ;
      ...
      if ( inverse ) {
        // フラットに戻すときは全て削除
        var slopeSupportIds = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == kodaiData.KoudaiName )
          .Select( x => x as SlopeSupport ).Where( x => x != null ).Select( x => x.m_ElementId ).ToList() ;
        ClsRevitUtil.Delete( doc, slopeSupportIds ) ;
      }

      if ( ! inverse ) {
        var materialSet = MaterialSuper.Collect( doc ).Where( x => x.m_KodaiName == kodaiData.KoudaiName ).ToArray() ;
        var ohbikiSet = materialSet.Select( x => x as OhbikiSuper ).Where( x => x != null ).ToArray() ;
        var nedaSet = materialSet.Select( x => x as Neda ).Where( x => x != null ).ToArray() ;
        ...
        foreach ( var a in slopeModel.SupportList.Where( x => x.HasTensetsuban != YMSGridNames.Nil ) ) {
          ...
          var rec = ClsMasterCsv.Shared.Select( x => x as ClsStiffenerCsv ).Where( x => x != null )
          ...
          var familyPath = Directory
            .EnumerateFiles( PathUtil.GetYMSFolder(), familyFileName, SearchOption.AllDirectories ).FirstOrDefault() ;
            ...
            //foreach (var elemH in elemHSet)
          {
            ...
            foreach ( var coef in new int[] { 1, -1 } ) {
              ...
              var instance = doc.Create.NewFamilyInstance( ohbikiHost.GetPlaneReference(),
                ptOnXY - 0.5 * nedaWebThickFt * dirH, dirK, symbol ) ;
        ...
        foreach ( var a in slopeModel.SupportList.Where( x => x.Stiffener != YMSGridNames.Nil ) ) {
          ...
          if ( a.Stiffener == YMSGridNames.Plate ) {
            isJack = false ;
            var stiffenerRec = StiffenerTable.Value.FirstOrDefault( x =>
              x.StiffenerType == StiffenerType.Plate && ohbikiSizeText.Contains( x.OhbikiSize.ToLower() ) ) ;
            if ( stiffenerRec == null ) {
              _logger.Error( $"StiffenerType:\"{a.Stiffener}\", OhbikiSize:\"{ohbikiSizeText}\" に適合するスチフナが見つかりません。" ) ;
              continue ;
            }
            ...
            var stiffenerRec = ClsMasterCsv.Shared.Select( x => x as ClsStiffenerCsv ).Where( x => x != null )
          ...
          var slopedRange = areaRangeSet.Select( x => x as AreaRangeSloped ).Where( x => x != null )
          ...
          if ( slopedRange != null ) {
            ...
            if ( slopedRange._Type == SlopeStyle.Slope ) {
              if ( ( ! thetaIsNegative && k == slopedRange._StartOhbikiK ) ||
                   ( thetaIsNegative && k == slopedRange._EndOhbikiK ) ) {
                dirSign *= -1 ;
              }
            }
          }
          ...
          for ( int h = 0 ; h < elemHSet.Length ; h++ ) {
            ...
            if ( isJack ) {
              ...
              ElementTransformUtils.RotateElement( doc, instance.Id, Line.CreateUnbound( ptOnXY, XYZ.BasisZ ),
                Math.PI ) ;
        ...
        foreach ( var a in slopeModel.SupportList.Where( x => x.Madumezai != YMSGridNames.Nil ) ) {
          ...
          var rec = ClsMasterCsv.Shared.Select( x => x as ClsAngleCsv ).Where( x => x != null )
          ...
          var slopedRange = areaRangeSet.Select( x => x as AreaRangeSloped ).Where( x => x != null )
            .FirstOrDefault( x =>
              ( x._Radian > 0 && x._EndOhbikiK == k ) || ( x._Radian < 0 && x._StartOhbikiK == k ) ) ;
          if ( slopedRange == null ) {
            continue ;
          }
          ...
          if ( ! kodaiData.ohbikiData.isHkou && ohbiki.MaterialSize().Shape != MaterialShape.H ) {
            ohbiki = ohbikiSet.Where( x => x.m_K == k ).OrderBy( x => x.Position.Z ).Select( x => x as OhbikiSuper )
              .FirstOrDefault() ;
            startPt = origin + elemHSet.ElementAtOrDefault( startH ) * axisH +
                      XYZ.BasisZ * ( nedaHeight + DefineUtil.FukkouBAN_THICK ) ;
            endPt = origin + elemHSet.ElementAtOrDefault( endH ) * axisH +
                    XYZ.BasisZ * ( nedaHeight + DefineUtil.FukkouBAN_THICK ) ;
            if ( ohbiki != null ) {
              ...
              var dir = ( ( ohbiki.m_AttachSide == "F" ) ? -axisK : axisK ) *
                        ClsRevitUtil.CovertToAPI( pSize.Height / 2 ) ;
              var pnts = GantryUtil.GetCurvePoints( doc, ohbiki.m_ElementId ) ;
              var ps = ( ( pnts[ 0 ].DistanceTo( origin ) < pnts[ 1 ].DistanceTo( origin ) ) ? pnts[ 0 ] : pnts[ 1 ] ) +
                       dir ;
              var pe = ( ( pnts[ 0 ].DistanceTo( origin ) < pnts[ 1 ].DistanceTo( origin ) ) ? pnts[ 1 ] : pnts[ 0 ] ) +
                       dir ;
            ...
            instanceId = MaterialSuper.PlaceWithTwoPoints( symbol, reference: negativePlane.GetReference(), startPt,
              endPt, levelOffsetFeet: levelOffsetFt ) ;
          }
          else {
            instanceId = MaterialSuper.PlaceWithTwoPoints( symbol, reference: negativePlane.GetReference(), startPt,
              endPt, levelOffsetFeet: levelOffsetFt ) ;
          ...
          if ( instanceId != null ) {
            ...
            {
              m_Document = doc, m_ElementId = instanceId, m_K = k, m_KodaiName = kodaiData.KoudaiName,
            } ;
    ...
    public static ReferencePlane NegativePlane( Document doc )
    {
      ...
      var existedPlane = new FilteredElementCollector( doc ).OfClass( typeof( ReferencePlane ) ).Cast<ReferencePlane>()
    ...
    private static XYZ BasePoint( Document doc, AllKoudaiFlatFrmData kodaiData )
    {
      ...
      return coordinate.Origin + ClsRevitUtil.CovertToAPI( hukuinPitches.FirstOrDefault() ) * coordinate.BasisH +
             ClsRevitUtil.CovertToAPI( kyochoPitches.FirstOrDefault() ) * coordinate.BasisK ;
    }
    ...
    private static AreaRange CreateAreaRangeFrom( FrmCreateSlopeViewModel.StyleRow a, int startK, int endK )
    {
      if ( a.SlopeType == FrmCreateSlopeViewModel.SlopeStyle.Nil ) {
        ...
        {
          _StyleRow = a, _Type = a.SlopeType, _StartOhbikiK = startK, _EndOhbikiK = endK,
        } ;
      }
    }

    private static AreaRange[] ConvertAreaRangeFromInputData( FrmCreateSlopeViewModel.KodaiSlopeModel slopeInputData,
      IEnumerable<MaterialSuper> materials, AllKoudaiFlatFrmData kodaiData, bool inverse )
    {
    ...
    abstract class AreaRange
    {
      ...
      public virtual void Calculate( IEnumerable<AreaRange> unionSet, IEnumerable<MaterialSuper> materials,
        AllKoudaiFlatFrmData kodaiData, bool inverse, KodaiSlopeModel slopeInputData )
      {
      }
    }
    ...
    class AreaRangeFlat : AreaRange
    {
      ...
      public override void Calculate( IEnumerable<AreaRange> unionSet, IEnumerable<MaterialSuper> materials,
        AllKoudaiFlatFrmData kodaiData, bool inverse, KodaiSlopeModel slopeInputData )
      {
        _OhbikiBottom = _StyleRow.Level ;
        _OhbikiBottoms = Enumerable.Range( _StartOhbikiK, _EndOhbikiK - _StartOhbikiK + 1 )
          .Select( x => new OhbikiBottomLevel { _K = x, _Z = _OhbikiBottom } ).ToArray() ;
      }
    }
    ...
    class AreaRangeSloped : AreaRange
    {
      ...
      public override void Calculate( IEnumerable<AreaRange> unionSet, IEnumerable<MaterialSuper> materials,
        AllKoudaiFlatFrmData kodaiData, bool inverse, KodaiSlopeModel slopeInputData )
      {
        var flatAreas = unionSet.Select( x => x as AreaRangeFlat ).Where( x => x != null ).ToArray() ;
        ...
        var axisOhbiki = materials.Select( x => x as OhbikiSuper ).OrderByDescending( x => x?.Position.Z )
          .FirstOrDefault( x => x?.m_K == axisIndex ) ;
        if ( axisOhbiki.MaterialSize().Shape == MaterialShape.C ||
             axisOhbiki.MaterialSize().Shape == MaterialShape.L ) {
          if ( axisIndex == _StartOhbikiK ) {
            axisOhbiki = materials.Select( x => x as OhbikiSuper ).OrderByDescending( x => x?.Position.Z )
              .FirstOrDefault( x => x?.m_K == axisIndex && x?.m_AttachSide == "F" ) ;
          }
          else {
            axisOhbiki = materials.Select( x => x as OhbikiSuper ).OrderByDescending( x => x?.Position.Z )
              .FirstOrDefault( x => x?.m_K == axisIndex && x?.m_AttachSide == "B" ) ;
          }
        }
        ...
        var rotateAxisOrigin = ohbikiPt + signAxisK * 0.5 * axisOhbikiWidthFeet * axisK +
                               ( 0.5 * axisOhbikiSize.HeightFeet + levelValue ) * axisZ ;

        if ( ! kodaiData.ohbikiData.isHkou ) {
          var p = materials.Select( x => x as PilePillerSuper ).OrderByDescending( x => x?.Position.Z )
            .FirstOrDefault( x => x?.m_KyoutyouNum == axisIndex ) ;
          if ( p != null ) {
            ohbikiPt = p.Position ;
            ...
            rotateAxisOrigin = ohbikiPt + signAxisK * 0.5 * ClsRevitUtil.CovertToAPI( ms.Width ) * axisK +
        ...
        if ( _StyleRow.SlopeType == FrmCreateSlopeViewModel.SlopeStyle.Slope ) {
          if ( startSideFlat != null && endSideFlat != null ) {
            var ohbikiPitch = /*ClsRevitUtil.CovertToAPI*/( kodaiData.KyoutyouPillarPitch.Skip( 1 )
              .Skip( _StartOhbikiK ).Take( _EndOhbikiK - _StartOhbikiK ).Sum() ) ;
            var hypotenuse = ohbikiPitch - 0.5 * ( axisOhbikiSize?.FrangeWidth ?? 0.0 ) ; //) axisOhbikiWidthFeet;
            var height = /*ClsRevitUtil.CovertToAPI*/
              ( Math.Abs( startSideFlat._StyleRow.Level - endSideFlat._StyleRow.Level ) ) ;
            ...
            if ( ! kodaiData.ohbikiData.isHkou ) {
              var p = materials.Select( x => x as PilePillerSuper ).OrderByDescending( x => x?.Position.Z )
                .FirstOrDefault( x => x?.m_KyoutyouNum == axisIndex ) ;
              if ( p != null ) {
                ohbikiPitch = ( kodaiData.KyoutyouPillarPitch.Skip( 1 ).Skip( _StartOhbikiK )
                  .Take( _EndOhbikiK - _StartOhbikiK ).Sum() ) ;
        ...
        var positionKSet = Enumerable.Range( 0, kodaiData.KyoutyouPillarPitch.Count() )
          .Select( x => kodaiData.KyoutyouPillarPitch.Skip( 1 ).Take( x ).Sum() ).ToArray() ;
        ...
        _OhbikiBottoms = Enumerable.Range( _StartOhbikiK, _EndOhbikiK - _StartOhbikiK + 1 )
          .Where( x => ! ( isSlope && x == lowerEndK ) ).Select( x =>
          {
            ...
            return new OhbikiBottomLevel { _K = x, _Z = baseOhbikiLevel - depthFromBase } ;
          } ).ToArray() ;
