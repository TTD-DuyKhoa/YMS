    ...
    public Result Excute()
    {
      try {
        ...
        if ( kData.KoujiType == DefineUtil.eKoujiType.Kenchiku ||
             frm.CmbSizeType.Text == Master.ClsJifukuZuredomeCsv.TypeL ) {
        ...
        if ( frm.RbtAuto.Checked ) {
          ...
          MaterialSuper[] materials = MaterialSuper.Collect( _doc )
            .Where( x => ids.Contains( x.m_ElementId ) && x.m_KodaiName == jifuku.m_KodaiName ).ToArray() ;
        ...
        //全周配置
          ...
          MaterialSuper[] materials = MaterialSuper.Collect( _doc ).Where( x => x.m_KodaiName == jifuku.m_KodaiName )
            .ToArray() ;

          //始点側端部にある根太
          Neda[] startNeda = materials.Select( x => x as Neda ).Where( x => x != null && x.m_MinK == 0 )
            .OrderBy( x => x.m_H ).ToArray() ;
          //終点側端部にある根太
          Neda[] endNeda = materials.Select( x => x as Neda ).Where( x => x != null && x.m_MaxK == maxKInd )
            .OrderBy( x => x.m_H ).ToArray() ;
          //構台左側にある根太
          Neda[] leftNeda = materials.Select( x => x as Neda ).Where( x => x != null && x.m_H == 0 )
            .OrderBy( x => x.m_MinK ).ToArray() ;
          //構台右側にある根太
          Neda[] rightNeda = materials.Select( x => x as Neda ).Where( x => x != null && x.m_H == maxHInd )
            .OrderBy( x => x.m_MinK ).ToArray() ;
          ...
          using ( Transaction tr = new Transaction( _doc ) ) {
            ...
            if ( startNeda.Length > 1 ) {
              ...
              XYZ point1 = pntsF[ 0 ] +
                           vecLine * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( startNeda.Last().m_ExStartLen ) +
                           ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) +
            ...
            if ( endNeda.Length > 1 ) {
              ...
              XYZ point1 = pntsF[ 0 ] +
                           vecLine * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( endNeda.Last().m_ExEndLen ) +
                           ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) +
                           vecCross * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( length ) ;
              XYZ point2 = pntsE[ 0 ] +
                           vecLine * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( endNeda.Last().m_ExEndLen ) +
            ...
            if ( leftNeda.Length >= 1 ) {
              for ( int i = 0 ; i < leftNeda.Length ; i++ ) {
                ...
                while ( same && cnt < leftNeda.Length ) {
                  Transform nextts =
                    ( _doc.GetElement( leftNeda[ cnt ].m_ElementId ) as FamilyInstance ).GetTransform() ;
                ...
                XYZ point1 = pntsF[ 0 ] +
                             ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) ;
                ;
                point1 = ( ins.Id == leftNeda.First().m_ElementId )
                  ? point1 + vecLine * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( leftNeda.First().m_ExStartLen )
                  : point1 ;
                XYZ point2 = pntsE[ 1 ] +
                             ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) ;
                point2 = ( nextins.Id == leftNeda.Last().m_ElementId )
                  ? point2 + vecLine.Negate() *
                  RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( leftNeda.Last().m_ExEndLen )
                  : point2 ;
                jifuku.CreateJifuku( jifuku, _doc, type, f.Reference, point1, point2, cOffset, cRotate ) ;
              }
            }
            ...
            if ( rightNeda.Length >= 1 ) {
              for ( int i = 0 ; i < rightNeda.Length ; i++ ) {
                ...
                while ( same && cnt < rightNeda.Length ) {
                  Transform nextts =
                    ( _doc.GetElement( rightNeda[ cnt ].m_ElementId ) as FamilyInstance ).GetTransform() ;
                  if ( nextts.BasisZ == ts.BasisZ ) {
                ...
                XYZ point1 = pntsF[ 0 ] +
                             ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) ;
                ;
                point1 = ( nextins.Id == rightNeda.Last().m_ElementId )
                  ? point1 + vecLine * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( rightNeda.Last().m_ExEndLen )
                  : point1 ;
                XYZ point2 = pntsE[ 1 ] +
                             ts.BasisZ * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( cOffset + ( height / 2 ) ) ;
                point2 = ( ins.Id == rightNeda.First().m_ElementId )
                  ? point2 + vecLine.Negate() *
                  RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit( rightNeda.First().m_ExStartLen )
                  : point2 ;
                jifuku.CreateJifuku( jifuku, _doc, type, f.Reference, point1, point2, cOffset, cRotate ) ;
              }
            }
            ...
            if ( frm.CmbSizeType.Text == Master.ClsJifukuZuredomeCsv.TypeL ) {
              try {
                ...
                using ( Transaction tr = new Transaction( _doc ) ) {
                  ...
                  var newIns = GantryUtil.CreateInstanceWith1point( _doc, origin, level.GetPlaneReference(), sym,
                    orthogolanVec ) ;

                  if ( newIns != null ) {
                    newId = newIns.Id ;
                    if ( offset != 0 ) {
                      RevitUtil.ClsRevitUtil.SetParameter( _uiDoc.Document, newId, DefineUtil.PARAM_BASE_OFFSET,
                        offset ) ;
                    }
              ...
              using ( Transaction tr = new Transaction( _doc ) ) {
                ...
                try {
                  if ( frm.CmbLevel.Text == "部材選択" ) {
                    newId = GantryUtil.PlaceSymbolOverTheSelectedElm( _uiDoc, sym, frm.CmbKoudaiName.Text, level,
                      offset, functionName: "地覆・ズレ止め材配置" ) ;
                  }
                  else {
                    XYZ p1 = selection.PickPoint( "1点目を指定してください" ) ;
                    XYZ p2 = selection.PickPoint( "2点目を指定してください" ) ;
