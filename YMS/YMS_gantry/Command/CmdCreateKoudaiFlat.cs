        ...
        public bool Excute()
        {
            ...
            try
            {
                using (var f = new FrmAllPutKoudaiFlat(_doc))
                {
                    ...
                    Autodesk.Revit.DB.View selView = RevitUtil.ClsRevitUtil.getView(_doc, viewName, GantryUtil.getViewType(_doc, viewName));
                    if (selView != null)
                    {
                    ...
                    using (Transaction tr = new Transaction(_doc))
                    {
                        ...
                        _doc.Regenerate();
                        RevitUtil.ClsRevitUtil.RotateElement(_doc, pointId, Line.CreateBound(origin, origin + XYZ.BasisZ), adjustAngle);
                        RevitUtil.ClsRevitUtil.SetParameter(_doc, pointId, DefineUtil.PARAM_HOST_OFFSET, RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(f.data.LevelOffset));
                        ...
                        {
                            //配置位置算出
                            Dictionary<XYZ, string> FukkoubanPnts = Fukkouban.CalcArrangementPoint(f.data.KyoutyouLength, f.data.HukuinLength, f.data.FukkoubanPitch, origin, vecKyoutyou, vecHukuin);
                            //配置
                            ...
                            for (int FukkouC = 0; FukkouC < FukkoubanPnts.Count; FukkouC++)
                            {
                                ...
                                eFukkoubanType hType = (RevitUtil.ClsGeo.GEO_EQ(f.data.FukkoubanPitch[FukkourC], 2000)) ? eFukkoubanType.TwoM : eFukkoubanType.Three;
                                ElementId hId = Fukkouban.CreateFukkouban(_doc, f.data.FukkoubanData.FukkouMaterial, position, hType, f.data.FukkoubanSizeList[FukkourC], adjustAngle, adjustVec, baselevel,
                                    Fukkouban.CalcLevelOffsetLevel(baselevel, eBase, f.data.LevelOffset, ohbikiSize.Height, nedaSize.Height, f.data.ohbikiData.OhbikiDan, f.data.ohbikiData.isHkou));

                                string[] pitchs = FukkoubanPnts.Values.ElementAt(FukkouC).Split(',');
                                Fukkouban fukkouban = new Fukkouban(hId, f.data.KoudaiName, f.data.FukkoubanSizeList[FukkourC], f.data.FukkoubanData,
                                    RevitUtil.ClsCommonUtils.ChangeStrToInt(pitchs[0]), RevitUtil.ClsCommonUtils.ChangeStrToInt(pitchs[1]));
                        ...
                        //根太配置
                        List<XYZ> NedaPnts = Neda.CalcArrangementPoint(f.data.KyoutyouLength, f.data.HukuinLength, f.data.NedaPitch, origin, vecKyoutyou, vecHukuin);
                        ...
                        if (GantryUtil.GetFamilySymbol(_doc, familyPath, type, out sym, true))
                        {
                            ...
                            {
                                XYZ start = position;
                                XYZ end = position + vecKyoutyou * (RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(f.data.KyoutyouLength));
                                ElementId nId = Neda.CreateNeda(_doc, f.data.nedaData, start, end, baselevel, isDoboku,f.data.KoudaiName,adjustAngle,
                                    Neda.CalcLevelOffsetLevel(baselevel, eBase, f.data.HasFukkouban, f.data.LevelOffset, ohbikiSize.Height, nedaSize.Height, f.data.ohbikiData.OhbikiDan, f.data.ohbikiData.isHkou),sym);
                                //個別情報追加
                                Neda neda = new Neda(nId, f.data.KoudaiName, f.data.nedaData, 0, f.data.KyoutyouPillarPitch.Count - 1, NedaPnts.IndexOf(position));
                        ...
                        if (GantryUtil.GetFamilySymbol(_doc, familyPath, type, out sym, true))
                        {
                            ...
                            for (int OhbikiC = 0; OhbikiC < OhbikiPnts.Count; OhbikiC++)
                            {
                                ...
                                XYZ start = position;
                                XYZ end = position + vecHukuin * (RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(f.data.HukuinLength));
                                double rotate = ohbikiSize.Shape == MaterialShape.C? 270: 180;
                                bool rev = false;
                                if(OhbikiPnts.ElementAt(OhbikiC).Value.Contains("F"))
                                {
                                    end = position;
                                    start = position + vecHukuin * (RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(f.data.HukuinLength));
                                    rev = true;
                                }
                                oId.AddRange(Ohbiki.CreateOhbiki(_doc, f.data.ohbikiData, start, end, baselevel, isDoboku,f.data.KoudaiName, 
                                    OhbikiSuper.CalcLevelOffsetLevel(baselevel, eBase, f.data.LevelOffset, f.data.HasFukkouban, ohbikiSize.Height, nedaSize.Height, f.data.ohbikiData.OhbikiDan, f.data.ohbikiData.isHkou,rotate), rotate, sym,rev));
                        ...
                        {
                            sym = GantryUtil.DuplicateTypeWithNameRule(_doc, f.data.KoudaiName, sym, OhbikiSuper.shikiGetaName, f.data.KoudaiType);
                            for (int shikiC = 0; shikiC < shikiGetaPnts.Count; shikiC++)
                            {
                                ...
                                XYZ end = position + vecHukuin * (RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(f.data.HukuinLength));
                                double rotate = 0;
                                double off = OhbikiSuper.CalcLevelOffsetLevel(baselevel, eBase, f.data.LevelOffset, f.data.HasFukkouban, shikiSize.Height, nedaSize.Height,1, true, rotate);
                                off = (f.data.ohbikiData.isHkou&&ohbikiSize.Height != shikiSize.Height && f.data.BaseLevel == eBaseLevel.OhbikiBtm) ? off +( ohbikiSize.Height - shikiSize.Height) : off;
                                sId.AddRange(Shikigeta.CreateShikiGeta(_doc, f.data.ohbikiData, f.data.shikigetaData, start, end, baselevel,f.data.KoudaiName, rotate, off, sym));

                                string indSt = shikiGetaPnts.ElementAt(shikiC).Value.Contains("-") ? shikiGetaPnts.ElementAt(shikiC).Value.Substring(0, 2) : shikiGetaPnts.ElementAt(shikiC).Value;
                                int ind = RevitUtil.ClsCommonUtils.ChangeStrToInt(indSt);
                        ...
                        //杭、柱配置（継足し杭＋PL等）
                        //柱
                        Dictionary<XYZ, string> pPnts = PilePillerSuper.CalcArrangementPoint(f.data.KyoutyouLength, f.data.HukuinLength, f.data.beginNedaDiff, f.data.HukuinPillarPitch, f.data.KyoutyouPillarPitch, origin, vecKyoutyou, vecHukuin);
                        if (f.data.HasPilePiller)
                        {
                            List<ElementId> pIds = new List<ElementId>();
                            adjustAngle = XYZ.BasisX.AngleTo(vecKyoutyou) * (RevitUtil.ClsGeo.GEO_LT0(vecKyoutyou.Y) ? -1 : 1);
                            //土木は90度回転
                            if (f.data.KoujiType == eKoujiType.Doboku) { adjustAngle += Math.PI / 2; }
                            ...
                            {
                                sym = GantryUtil.DuplicateTypeWithNameRule(_doc, f.data.KoudaiName, sym, PilePillerSuper.typeName, f.data.KoudaiType);
                            }
                            ...
                            {
                                exSym = GantryUtil.DuplicateTypeWithNameRule(_doc, f.data.KoudaiName, exSym, PilePillerSuper.exPileTypeName, f.data.KoudaiType);
                            }

                            if(sym!=null)
                            {
                                double pOffset = PilePillerSuper.CalcLevelOffsetLevel(baselevel, f.data, ohbikiSize.Height, nedaSize.Height);
                                double exPOffset = PilePillerSuper.CalcLevelOffsetExtensionPileLevel(baselevel, eBase, f.data.LevelOffset, ohbikiSize.Height, nedaSize.Height);

                                foreach (KeyValuePair<XYZ, string> kv in pPnts)
                                {
                                    string[] pitchs = kv.Value.Split(',');
                                    //敷桁配置位置には柱を配置しない
                                    if ((f.data.IsFirstShikigeta && pitchs[0] == "0") || (f.data.IsLastShikigeta && pitchs[0] == $"{f.data.KyoutyouPillarPitch.Count - 1}"))
                                    {
                                        continue;
                                    }
                                    ElementId pId = PilePillerSuper.CreatePile(_doc, kv.Key, adjustAngle, f.data.pilePillerData,f.data.KoudaiName,adjustVec,baselevel,pOffset , sym);
                                    PilePiller pile = new PilePiller(pId, f.data.KoudaiName, f.data.pilePillerData,
                                        RevitUtil.ClsCommonUtils.ChangeStrToInt(pitchs[0]), RevitUtil.ClsCommonUtils.ChangeStrToInt(pitchs[1]));
                                    if(f.data.pilePillerData.pJointCount>0&&f.data.pilePillerData.jointDetailData.JointType==eJoinType.Bolt)
                                    {
                                        pile.m_BoltInfo1 = f.data.pilePillerData.jointDetailData.WebBolt.BoltSize;
                                        pile.m_Bolt1Cnt = f.data.pilePillerData.jointDetailData.WebBolt.BoltCount * f.data.pilePillerData.pJointCount;
                                        pile.m_BoltInfo2 = f.data.pilePillerData.jointDetailData.FlangeBolt.BoltSize;
                                        pile.m_Bolt2Cnt = f.data.pilePillerData.jointDetailData.FlangeBolt.BoltCount * f.data.pilePillerData.pJointCount;
                                    }                                    
                                    ...
                                    {
                                        pId = PilePillerSuper.CreateExtraPile(_doc, kv.Key, adjustAngle, f.data.pilePillerData, f.data.KoudaiName, adjustVec,baselevel, exPOffset, exSym);
