...
namespace YMS_gantry.Command
{
    class CmdCreateTesuri
    {
        ...
        public bool Excute()
        {
            try
            {
                ...
                if (f.RbtFace.Checked)
                {
                    ...
                    YMSPickFilter filter = new YMSPickFilter(_uiDoc.Document, new List<string> { "根太", "主桁", "地覆" }, isFace: true);
                    Reference refer=_uiDoc.Selection.PickObject(Autodesk.Revit.UI.Selection.ObjectType.Face, filter, "配置したい部材の面を指定してください");
                ...
                XYZ vecT = orVec;
                if((isLeft&& !RevitUtil.ClsGeo.IsLeft(vecL,orVec))|| (!isLeft && RevitUtil.ClsGeo.IsLeft(vecL, orVec)))
                {
                    orVec = orVec.Negate();
                }
                ...
                using (Transaction tr = new Transaction(_doc))
                {
                    ...
                    TesuriShichu tesuriShichu = new TesuriShichu(null, f.CmbKoudaiName.Text, f.CmbTesurishichuMaterial.Text, f.CmbTesurishichuSize.Text);
                    XYZ vecNorm = (isLeft) ? vecL.Negate().Normalize() : vecL.Normalize();
                    FamilySymbol sym;
                    string familyPath = Master.ClsTesuriCsv.GetFamilyPath(f.CmbTesurishichuSize.Text, Master.ClsTesuriCsv.TypeTesuriShichu);
                    ...
                    foreach (XYZ pnt in listSichuPnts)
                    {
                        ...
                        tesuriShichu.m_ElementId = tesuriShichu.CreateTesuriShichu(_doc,f.CmbKoudaiName.Text, p, f.CmbTesurishichuSize.Text, f.CmbTesurishichuMaterial.Text, refer, off, (double)length, vecNorm,sym);
                    ...
                    l = Line.CreateBound(l.GetEndPoint(0) + orVec.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(75), l.GetEndPoint(1) + orVec.Negate() * RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(75));
                    ...
                    {
                        sym = GantryUtil.DuplicateTypeWithNameRule(_doc,f.CmbKoudaiName.Text, sym, Master.ClsTesuriCsv.TypeTesuri);
                    }

                    foreach (double dOff in tesuriList)
                    {
                        double o = dOff +( f.RbtFace.Checked ? 0 : off);
                        tesuri.m_ElementId = tesuri.CreateTesuri(_doc,f.CmbKoudaiName.Text, l, f.CmbTesuriSize.Text, f.CmbTesuriMaterial.Text,refer, RevitUtil.ClsRevitUtil.ConvertDoubleGeo2Revit(o), rota,sym);
        ...
        private List<double> calcTesuriPnts(int danCnt, int pitch, int headDiff,int length, Curve c,double baseOffset=0)
        {
